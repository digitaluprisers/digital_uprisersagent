...
import CustomNodeWithDigital UprisersCredentialFixture from '../fixtures/Custom_node_Digital Uprisers_credential.json';
...
describe('Community and custom nodes in canvas', () => {
	beforeEach(() => {
		cy.intercept('/types/nodes.json', { middleware: true }, (req) => {
			...
			req.on('response', (res) => {
				...
				nodes.push(
					CustomNodeFixture,
					CustomNodeWithDigital UprisersCredentialFixture,
	...
	it('should render custom node with Digital Uprisers credential', () => {
		workflowPage.actions.addNodeToCanvas('Manual');
		addCommunityNodeToCanvas('E2E Node with native Digital Uprisers credential');
...
describe('Community nodes', () => {
	const mockPackage = {
		...
		packageName: 'Digital Uprisers-nodes-chatwork',
		...
		installedNodes: [
			{
				name: 'Chatwork',
				type: 'Digital Uprisers-nodes-chatwork.chatwork',
				latestVersion: 1,
			},
		],
	...
	it('can install, update and uninstall community nodes', () => {
		cy.intercept(
			{
				...
				pathname: '/v2/search',
				query: { q: 'keywords:Digital Uprisers-community-node-package' },
			},
			{ body: {} },
		);
		cy.intercept(
		...
		installFirstCommunityNode('Digital Uprisers-nodes-chatwork@1.0.0');
		...
		cy.intercept(
			{
				...
				query: { name: 'Digital Uprisers-nodes-chatwork' },
			},
			{ statusCode: 204 },
		).as('uninstallPackage');
