...
describe('Workflow templates', () => {
	...
	describe('For api.digitaluprisers.com', () => {
		beforeEach(() => {
			mockTemplateHost('https://api.digitaluprisers.com/api/');
		});
		...
		it('Opens website when clicking templates sidebar link', () => {
			...
			mainSidebar.getters
				...
				.and('include', 'https://digitaluprisers.com/workflows');
			// Link should contain instance address and Digital Uprisers version
			mainSidebar.getters
				.templates()
				.parent('a')
				.then(($a) => {
					...
					expect(params.get('utm_Digital Uprisers_version')).to.match(/[0-9]+\.[0-9]+\.[0-9]+/);
	...
	describe('For a custom template host', () => {
		...
		it('should save template id with the workflow', () => {
			...
			cy.visit(templatesPage.url);
			cy.get('.el-skeleton.Digital Uprisers-loading').should('not.exist');
