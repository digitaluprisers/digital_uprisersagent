...
export class CredentialsModal extends BasePage {
	getters = {
		...
		connectionParameter: (fieldName: string) =>
			this.getters.credentialInputs().find(`:contains('${fieldName}') .Digital Uprisers-input input`),
		...
		menuItem: (name: string) => this.getters.menu().get('.Digital Uprisers-menu-item').contains(name),
