...
beforeEach(() => {
	cy.window().then((win): void => {
		win.localStorage.setItem('DIGITAL_UPRISERS_THEME', 'light');
		win.localStorage.setItem('DIGITAL_UPRISERS_AUTOCOMPLETE_ONBOARDED', 'true');
		win.localStorage.setItem('DIGITAL_UPRISERS_MAPPING_ONBOARDED', 'true');
	});
	...
	cy.intercept({ pathname: '/api/versions/*' }, [
		{
			...
			documentationUrl: 'https://docs.digitaluprisers.com/release-notes/#Digital Uprisers131',
			nodes: [],
		...
		},
		{
			...
			documentationUrl: 'https://docs.digitaluprisers.com/release-notes/#Digital Uprisers104',
			nodes: [],
