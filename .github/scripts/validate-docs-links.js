...
const packages = ['nodes-base', '@Digital Uprisers/nodes-langchain'];
const concurrency = 20;
let exitCode = 0;

const debug = require('debug')('Digital Uprisers');
	...
	new Promise((resolve, reject) => {
		...
		const url = new URL(
			/^https?:\/\//.test(documentationUrl)
				? documentationUrl
				: `https://docs.digitaluprisers.com/integrations/builtin/${kind}/${documentationUrl.toLowerCase()}/`,
		);
		https
			.request(
				{
