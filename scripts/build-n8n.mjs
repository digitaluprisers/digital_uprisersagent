#!/usr/bin/env node
/**
 * This script is used to build the Digital Uprisers application for production.
 * It will:
...
printHeader('Digital Uprisers Build & Production Preparation');
...
await $`cd ${config.rootDir} && NODE_ENV=production DOCKER_BUILD=true pnpm --filter=Digital Uprisers --prod --legacy deploy --no-optional ./compiled`;
...
echo(chalk.green(`✅ Digital Uprisers built successfully!`));
echo('');
