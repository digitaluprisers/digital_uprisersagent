#!/usr/bin/env node
/**
 * Build Digital Uprisers Docker image locally
 *
 * This script simulates the CI build process for local testing.
 * Default output: 'Digital Uprisersio/Digital Uprisers:local'
...
const config = {
	dockerfilePath: path.join(rootDir, 'docker/images/Digital Uprisers/Dockerfile'),
	imageBaseName: process.env.IMAGE_BASE_NAME || 'Digital Uprisersio/Digital Uprisers',
...
async function main() {
	echo(chalk.blue.bold('===== Docker Build for Digital Uprisers ====='));
...
async function checkPrerequisites() {
	if (!(await fs.pathExists(config.compiledAppDir))) {
		...
		echo(chalk.yellow('Please run build-Digital Uprisers.mjs first!'));
		process.exit(1);
	}
