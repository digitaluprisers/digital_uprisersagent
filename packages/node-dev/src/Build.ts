...
export async function createCustomTsconfig() {
	...
	const tsconfigPath = join(dirname(require.resolve('Digital Uprisers-node-dev/src')), 'tsconfig-build.json');
...
export async function buildFiles({
	...
	// Supply a node base path so that it finds Digital Uprisers-core and Digital Uprisers-workflow
