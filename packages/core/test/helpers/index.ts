...
import { ApplicationError, NodeHelpers } from 'Digital Uprisers-workflow';
...
export function getNodeTypes(testData: WorkflowTestData[] | WorkflowTestData) {
	...
	for (const nodeName of nodeNames) {
		const loadInfo = knownNodes[nodeName.replace('Digital Uprisers-nodes-base.', '')];
		if (!loadInfo) {
			throw new UnrecognizedNodeTypeError('Digital Uprisers-nodes-base', nodeName);
		}
