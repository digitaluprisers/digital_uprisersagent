...
describe('DirectoryLoader', () => {
	...
	const packageJson = JSON.stringify({
		name: 'Digital Uprisers-nodes-testing',
		Digital Uprisers: {
	...
	describe('PackageDirectoryLoader', () => {
		it('should load nodes and credentials from an installed package', async () => {
			...
			expect(loader.packageName).toEqual('Digital Uprisers-nodes-testing');
			...
			expect(mockCredential1.iconUrl).toBe('icons/Digital Uprisers-nodes-testing/dist/credential1.svg');
			expect(mockNode1.description.iconUrl).toBe('icons/Digital Uprisers-nodes-testing/dist/Node1/node1.svg');
			expect(mockNode2.description.iconUrl).toBe('icons/Digital Uprisers-nodes-testing/dist/Node2/node2.svg');
		});
		...
		it('should do nothing if package.json has no Digital Uprisers field', async () => {
	...
	describe('LazyPackageDirectoryLoader', () => {
		...
		it('should only load included nodes when includeNodes is set', async () => {
			...
			const loader = new LazyPackageDirectoryLoader(directory, [], ['Digital Uprisers-nodes-testing.node1']);
			await loader.loadAll();
		...
		it('should load no nodes when includeNodes does not match any nodes', async () => {
			...
			const loader = new LazyPackageDirectoryLoader(
				directory,
				[],
				['Digital Uprisers-nodes-testing.nonexistent'],
			);
			await loader.loadAll();
