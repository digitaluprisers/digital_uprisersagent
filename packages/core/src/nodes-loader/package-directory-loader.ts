...
import type { Digital Uprisers } from './types';
...
export class PackageDirectoryLoader extends DirectoryLoader {
	packageJson: Digital Uprisers.PackageJson;
	...
	override async loadAll() {
		const { Digital Uprisers } = this.packageJson;
		if (!Digital Uprisers) return;

		const { nodes, credentials } = Digital Uprisers;
