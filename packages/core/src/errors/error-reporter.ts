...
import type { ReportingOptions } from 'Digital Uprisers-workflow';
import { ApplicationError, ExecutionCancelledError, BaseError } from 'Digital Uprisers-workflow';
...
export class ErrorReporter {
	...
	async beforeSend(event: ErrorEvent, hint: EventHint) {
		...
		if (originalException instanceof BaseError) {
			...
			this.extractEventDetailsFromDigital UprisersError(event, originalException);
		}
		...
		if (originalException instanceof ApplicationError || originalException instanceof BaseError) {
			if (this.isIgnoredDigital UprisersError(originalException)) return null;
	...
	private isIgnoredDigital UprisersError(error: ApplicationError | BaseError) {
		return error.level === 'warning' || error.level === 'info';
	}

	private extractEventDetailsFromDigital UprisersError(
		event: ErrorEvent,
