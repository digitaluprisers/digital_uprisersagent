...
process.env.DIGITAL_UPRISERS_RUNNERS_ENABLED = 'false';
...
describe('WorkflowExecute', () => {
	...
	describe('v0 hook order', () => {
		...
		test("don't run hooks for siblings of the destination node", async () => {
			// ARRANGE
			const trigger = createNodeData({ name: 'trigger', type: 'Digital Uprisers-nodes-base.manualTrigger' });
	...
	describe('runPartialWorkflow2', () => {
		...
		test('passes filtered run data to `recreateNodeExecutionStack`', async () => {
			...
			const trigger = createNodeData({ name: 'trigger', type: 'Digital Uprisers-nodes-base.manualTrigger' });
			const loop = createNodeData({ name: 'loop', type: 'Digital Uprisers-nodes-base.splitInBatches' });
		...
		it('rewires graph for partial execution of tools', async () => {
			...
			const trigger = createNodeData({ name: 'trigger', type: 'Digital Uprisers-nodes-base.manualTrigger' });
			const tool = createNodeData({ name: 'tool', type: 'Digital Uprisers-nodes-base.toolTest' });
			...
			const expectedToolExecutor: INode = {
				...
				disabled: false,
				type: '@Digital Uprisers/Digital Uprisers-nodes-langchain.toolExecutor',
				parameters: {
					query: {},
					toolName: '',
				},
