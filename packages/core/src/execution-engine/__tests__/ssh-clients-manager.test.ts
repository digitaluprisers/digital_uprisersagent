...
import type { SSHCredentials } from 'Digital Uprisers-workflow';
...
describe('SSHClientsConfig', () => {
	...
	test('allows overriding the default idle timeout', async () => {
		// ARRANGE
		process.env.DIGITAL_UPRISERS_SSH_TUNNEL_IDLE_TIMEOUT = '5';
	...
	test.each(['-5', '0', 'foo'])(
		'fall back to default if DIGITAL_UPRISERS_SSH_TUNNEL_IDLE_TIMEOUT is `%s`',
		async (value) => {
			// ARRANGE
			process.env.DIGITAL_UPRISERS_SSH_TUNNEL_IDLE_TIMEOUT = value;
