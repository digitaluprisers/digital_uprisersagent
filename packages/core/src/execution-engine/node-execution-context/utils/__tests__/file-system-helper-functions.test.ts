...
describe('isFilePathBlocked', () => {
	beforeEach(() => {
		process.env[BLOCK_FILE_ACCESS_TO_DIGITAL_UPRISERS_FILES] = 'true';
	});
	...
	it('should return true for restricted paths', () => {
		const restrictedPath = instanceSettings.Digital UprisersFolder;
	...
	it('should return false when BLOCK_FILE_ACCESS_TO_DIGITAL_UPRISERS_FILES is false', () => {
		process.env[BLOCK_FILE_ACCESS_TO_DIGITAL_UPRISERS_FILES] = 'false';
		const restrictedPath = instanceSettings.Digital UprisersFolder;
...
describe('getFileSystemHelperFunctions', () => {
	...
	describe('getStoragePath', () => {
		it('returns correct path', () => {
			const expectedPath = join(instanceSettings.Digital UprisersFolder, `storage/${node.type}`);
		...
		it('should throw error for blocked file path', async () => {
			...
			await expect(
				helperFunctions.writeContentToFile(
					instanceSettings.Digital UprisersFolder + '/test.txt',
					'content',
					'w',
				),
