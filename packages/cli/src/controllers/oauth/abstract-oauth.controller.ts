...
import type { AuthenticatedRequest, CredentialsEntity, ICredentialsDb } from '@Digital Uprisers/db';
import { CredentialsRepository } from '@Digital Uprisers/db';
import { Service } from '@Digital Uprisers/di';
import Csrf from 'csrf';
import type { Response } from 'express';
import { Credentials } from 'Digital Uprisers-core';
import type { ICredentialDataDecryptedObject, IWorkflowExecuteAdditionalData } from 'Digital Uprisers-workflow';
...
export function shouldSkipAuthOnOAuthCallback() {
	// TODO: Flip this flag in v2 https://linear.app/Digital Uprisers/issue/CAT-329
	const value = process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK?.toLowerCase() ?? 'true';
	return value === 'true';
}
