...
describe('shouldSkipAuthOnOAuthCallback', () => {
	const originalEnv = process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK;

	afterEach(() => {
		// Restore original environment variable after each test
		if (originalEnv === undefined) {
			delete process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK;
		} else {
			process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK = originalEnv;
		}
	});

	describe('when Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK is not set', () => {
		beforeEach(() => {
			delete process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK;
		});
	...
	describe('with various environment variable values', () => {
		...
		test.each(testCases)('"%s" value should return %s', ({ value, expected }) => {
			process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK = value;
