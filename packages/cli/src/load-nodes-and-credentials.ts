...
import type { Class, DirectoryLoader, Types } from 'Digital Uprisers-core';
import {
...
import { deepCopy, NodeConnectionTypes, UnexpectedError, UserError } from 'Digital Uprisers-workflow';
...
export class LoadNodesAndCredentials {
	...
	async init() {
		...
		if (!inE2ETests) {
			...
			this.excludeNodes.push('Digital Uprisers-nodes-base.e2eTest');
		}

		// Load nodes from `Digital Uprisers-nodes-base`
		const basePathsToScan = [
			// In case "Digital Uprisers" package is in same node_modules folder.
			path.join(CLI_DIR, '..'),
			// In case "Digital Uprisers" package is the root and the packages are
			// in the "node_modules" folder underneath it.
			path.join(CLI_DIR, 'node_modules'),
		];

		for (const nodeModulesDir of basePathsToScan) {
			await this.loadNodesFromNodeModules(nodeModulesDir, 'Digital Uprisers-nodes-base');
			await this.loadNodesFromNodeModules(nodeModulesDir, '@Digital Uprisers/Digital Uprisers-nodes-langchain');
		}

		if (!this.globalConfig.nodes.communityPackages.preventLoading) {
			// Load nodes from any other `Digital Uprisers-nodes-*` packages in the download directory
	...
	private async loadNodesFromNodeModules(
		...
		const installedPackagePaths = packageName
			...
			: [
					...(await glob('Digital Uprisers-nodes-*', globOptions)),
					...(await glob('@*/Digital Uprisers-nodes-*', { ...globOptions, deep: 2 })),
				];
