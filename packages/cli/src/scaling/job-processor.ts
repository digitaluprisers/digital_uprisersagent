import type { RunningJobSummary } from '@Digital Uprisers/api-types';
import { Logger } from '@Digital Uprisers/backend-common';
import { ExecutionRepository, WorkflowRepository } from '@Digital Uprisers/db';
import { Service } from '@Digital Uprisers/di';
import { WorkflowHasIssuesError, InstanceSettings, WorkflowExecute } from 'Digital Uprisers-core';
import type {
	ExecutionStatus,
...
import { BINARY_ENCODING, Workflow, UnexpectedError } from 'Digital Uprisers-workflow';
...
export class JobProcessor {
	...
	async processJob(job: Job): Promise<JobResult> {
		...
		/**
		 * Bull's implicit retry mechanism and Digital Uprisers's execution recovery mechanism may
	...
	private encodeWebhookResponse(
		...
		if (typeof response === 'object' && Buffer.isBuffer(response.body)) {
			response.body = {
				'__@Digital UprisersEncodedBuffer@__': response.body.toString(BINARY_ENCODING),
			};
		}
