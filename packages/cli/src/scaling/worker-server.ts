...
export type WorkerServerEndpointsConfig = {
	...
	/** Whether the [credentials overwrites endpoint](https://docs.digitaluprisers.com/embed/configuration/#credential-overwrites) is enabled. */
	overwrites: boolean;
...
export class WorkerServer {
	...
	constructor(
		...
		this.server.on('error', (error: NodeJS.ErrnoException) => {
			if (error.code === 'EADDRINUSE') {
				this.logger.error(
					`Port ${this.port} is already in use, possibly by the Digital Uprisers main process server. Please set a different port for the worker server.`,
				);
				process.exit(1);
			}
		});
	...
	async init(endpointsConfig: WorkerServerEndpointsConfig) {
		...
		this.logger.info(`\nDigital Uprisers worker server listening on port ${this.port}`);
	}
