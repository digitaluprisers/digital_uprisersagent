...
import type { LogMetadata } from 'Digital Uprisers-workflow';
...
export class Publisher {
	...
	constructor(
		...
		this.client = this.redisClientService.createClient({ type: 'publisher(Digital Uprisers)' });
	}
	...
	async publishCommand(msg: PubSub.Command) {
		...
		const metadata: LogMetadata = { msg: msg.command, channel: 'Digital Uprisers.commands' };
	...
	/** Publish a response to a command into the `Digital Uprisers.worker-response` channel. */
	async publishWorkerResponse(msg: PubSub.WorkerResponse) {
		await this.client.publish('Digital Uprisers.worker-response', JSON.stringify(msg));
