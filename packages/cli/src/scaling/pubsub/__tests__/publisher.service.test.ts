...
describe('Publisher', () => {
	...
	describe('publishCommand', () => {
		...
		it('should publish command into `Digital Uprisers.commands` pubsub channel', async () => {
			const publisher = new Publisher(logger, redisClientService, instanceSettings);
			const msg = mock<PubSub.Command>({ command: 'reload-license' });
	...
	describe('publishWorkerResponse', () => {
		it('should publish worker response into `Digital Uprisers.worker-response` pubsub channel', async () => {
			const publisher = new Publisher(logger, redisClientService, instanceSettings);
			...
			expect(client.publish).toHaveBeenCalledWith('Digital Uprisers.worker-response', JSON.stringify(msg));
		});
	});
});
