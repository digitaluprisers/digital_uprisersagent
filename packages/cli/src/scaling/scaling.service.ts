import { isObjectLiteral, Logger } from '@Digital Uprisers/backend-common';
import { GlobalConfig } from '@Digital Uprisers/config';
import { Time } from '@Digital Uprisers/constants';
import { ExecutionRepository } from '@Digital Uprisers/db';
import { OnLeaderStepdown, OnLeaderTakeover, OnShutdown } from '@Digital Uprisers/decorators';
import { Container, Service } from '@Digital Uprisers/di';
import { ErrorReporter, InstanceSettings } from 'Digital Uprisers-core';
import {
	BINARY_ENCODING,
	sleep,
	jsonStringify,
...
import type { IExecuteResponsePromiseData } from 'Digital Uprisers-workflow';
...
export class ScalingService {
	...
	private decodeWebhookResponse(
		...
		if (
			...
			response.body !== null &&
			'__@Digital UprisersEncodedBuffer@__' in response.body &&
			typeof response.body['__@Digital UprisersEncodedBuffer@__'] === 'string'
		) {
			response.body = Buffer.from(response.body['__@Digital UprisersEncodedBuffer@__'], BINARY_ENCODING);
		}
