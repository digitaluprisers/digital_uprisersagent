import { inTest, inDevelopment, Logger } from '@Digital Uprisers/backend-common';
...
import { Digital Uprisers_VERSION, TEMPLATES_DIR } from '@/constants';
...
export abstract class AbstractServer {
	...
	private async setupHealthCheck() {
		...
		this.app.use((_req, res, next) => {
			if (connectionState.connected) {
				...
				else res.send('Digital Uprisers is starting up. Please wait');
	...
	async init(): Promise<void> {
		...
		this.server.on('error', (error: Error & { code: string }) => {
			if (error.code === 'EADDRINUSE') {
				this.logger.info(
					`Digital Uprisers's port ${port} is already in use. Do you have another instance of Digital Uprisers running already?`,
				);
				process.exit(1);
			}
		});
		...
		this.logger.info(`Digital Uprisers ready on ${address}, port ${port}`);
	}
	...
	async start(): Promise<void> {
		...
		if (!inTest) {
			...
			await this.externalHooks.run('Digital Uprisers.ready', [this, config]);
		}
	}
