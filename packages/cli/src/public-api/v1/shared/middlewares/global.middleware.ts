...
import type { ApiKeyScope, Scope } from '@Digital Uprisers/permissions';
...
export const validLicenseWithUserQuota = (
	...
	if (license.getUsersLimit() !== UNLIMITED_USERS_QUOTA) {
		return res.status(403).json({
			message: '/users path can only be used with a valid license. See https://digitaluprisers.com/pricing/',
		});
	}
