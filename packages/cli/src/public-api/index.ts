...
async function createApiRouter(
	...
	const Digital UprisersPath = Container.get(GlobalConfig).path;
	...
	if (!Container.get(GlobalConfig).publicApi.swaggerUiDisabled) {
		...
		apiController.use(
			...
			setup(swaggerDocument, {
				...
				customSiteTitle: 'Digital Uprisers Public API UI',
				customfavIcon: `${Digital UprisersPath}favicon.ico`,
			}),
		);
	}
