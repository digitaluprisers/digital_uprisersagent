...
import type { DirectoryLoader } from 'Digital Uprisers-core';
import type { INodeProperties, INodeTypeDescription } from 'Digital Uprisers-workflow';
...
describe('LoadNodesAndCredentials', () => {
	...
	describe('resolveSchema', () => {
		...
		beforeEach(() => {
			instance = new LoadNodesAndCredentials(mock(), mock(), mock(), mock());
			instance.knownNodes['Digital Uprisers-nodes-base.test'] = {
				className: 'Test',
		...
		it('should return undefined if the node is not known', () => {
			const result = instance.resolveSchema({
				node: 'Digital Uprisers-nodes-base.doesNotExist',
				version: '1.0.0',
				resource: 'account',
				operation: 'get',
			});
		...
		it('should return the correct path if the node is known', () => {
			const result = instance.resolveSchema({
				node: 'Digital Uprisers-nodes-base.test',
				version: '1.0.0',
				resource: 'account',
				operation: 'get',
			});
