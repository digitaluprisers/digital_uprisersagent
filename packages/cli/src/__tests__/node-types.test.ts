...
import { RoutingNode, UnrecognizedNodeTypeError } from 'Digital Uprisers-core';
import type {
	LoadedClass,
	INodeType,
	IVersionedNodeType,
...
describe('NodeTypes', () => {
	...
	const nonVersionedNode: LoadedClass<INodeType> = {
		...
		type: {
			description: mock<INodeTypeDescription>({
				name: 'Digital Uprisers-nodes-base.nonVersioned',
	...
	const versionedNode: LoadedClass<IVersionedNodeType> = {
		...
		type: {
			description: mock<INodeTypeDescription>({
				name: 'Digital Uprisers-nodes-base.versioned',
			}),
			currentVersion: 2,
			nodeVersions: {
				1: v1Node,
	...
	const toolNode: LoadedClass<INodeType> = {
		...
		type: {
			description: mock<INodeTypeDescription>({
				name: 'Digital Uprisers-nodes-base.toolNode',
	...
	const toolSupportingNode: LoadedClass<INodeType> = {
		...
		type: {
			description: mock<INodeTypeDescription>({
				name: 'Digital Uprisers-nodes-base.testNode',
				displayName: 'TestNode',
				usableAsTool: true,
				properties: [],
			}),
	...
	const declarativeNode: LoadedClass<INodeType> = {
		...
		type: {
			description: mock<INodeTypeDescription>({
				name: 'Digital Uprisers-nodes-base.declarativeNode',
	...
	const communityNode: LoadedClass<INodeType> = {
		...
		type: {
			description: mock<INodeTypeDescription>({
				name: 'Digital Uprisers-nodes-community.testNode',
				displayName: 'TestNode',
				usableAsTool: true,
				properties: [],
			}),
	...
	loadNodesAndCredentials.getNode.mockImplementation((fullNodeType) => {
		...
		if (packageName === 'Digital Uprisers-nodes-base') {
		...
		} else if (fullNodeType === 'Digital Uprisers-nodes-community.testNode') return communityNode;
	...
	describe('getByName', () => {
		it('should return node type when it exists', () => {
			const result = nodeTypes.getByName('Digital Uprisers-nodes-base.nonVersioned');
	...
	describe('getByNameAndVersion', () => {
		...
		it('should throw an error if the node-type does not exist', () => {
			expect(() => nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.unknownNode')).toThrow(
				'Unrecognized node type: Digital Uprisers-nodes-base.unknownNode',
			);
		});

		it('should return a regular node-type without version', () => {
			const result = nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.nonVersioned');
			expect(result).toBe(nonVersionedNode.type);
		});

		it('should return a regular node-type with version', () => {
			const result = nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.versioned');
			expect(result).toBe(v2Node);
		});

		it('should throw when a node-type is requested as tool, but does not support being used as one', () => {
			expect(() => nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.nonVersionedTool')).toThrow(
		...
		it('should throw when a node-type is requested as tool, but the original node is already a tool', () => {
			expect(() => nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.toolNodeTool')).toThrow(
				'Node already has a `supplyData` method',
			);
		});

		it('should return the tool node-type when requested as tool', () => {
			const result = nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.testNodeTool');
			expect(result).not.toEqual(toolSupportingNode.type);
			expect(result.description.name).toEqual('Digital Uprisers-nodes-base.testNodeTool');
		...
		it('should return a tool node-type from a community node,  when requested as tool', () => {
			const result = nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-community.testNodeTool');
			expect(result).not.toEqual(toolSupportingNode.type);
			expect(result.description.name).toEqual('Digital Uprisers-nodes-community.testNodeTool');
			expect(result.description.displayName).toEqual('TestNode Tool');
			expect(result.description.codex?.categories).toContain('AI');
			expect(result.description.inputs).toEqual([]);
			expect(result.description.outputs).toEqual(['ai_tool']);
		...
		it('should return a declarative node-type with an `.execute` method', () => {
			const result = nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.declarativeNode');
		...
		it('should return a declarative node-type as a tool with an `.execute` method', () => {
			const result = nodeTypes.getByNameAndVersion('Digital Uprisers-nodes-base.declarativeNodeTool');
			expect(result).not.toEqual(declarativeNode.type);
			expect(result.description.name).toEqual('Digital Uprisers-nodes-base.declarativeNodeTool');
	...
	describe('getWithSourcePath', () => {
		it('should return description and source path for existing node', () => {
			const result = nodeTypes.getWithSourcePath('Digital Uprisers-nodes-base.nonVersioned', 1);
		...
		it('should throw error for non-existent node', () => {
			expect(() => nodeTypes.getWithSourcePath('Digital Uprisers-nodes-base.nonExistent', 1)).toThrow(
				'Unrecognized node type: Digital Uprisers-nodes-base.nonExistent',
			);
		});
	});
	...
	describe('getKnownTypes', () => {
		it('should return known node types', () => {
			...
			loadNodesAndCredentials.knownNodes = ['Digital Uprisers-nodes-base.nonVersioned'];
			const result = nodeTypes.getKnownTypes();
			expect(result).toEqual(['Digital Uprisers-nodes-base.nonVersioned']);
		});
	});
	...
	describe('getNodeTypeDescriptions', () => {
		it('should return descriptions for valid node types', () => {
			const result = nodeTypes.getNodeTypeDescriptions([
				{ name: 'Digital Uprisers-nodes-base.nonVersioned', version: 1 },
			]);

			expect(result).toHaveLength(1);
			expect(result[0].name).toBe('Digital Uprisers-nodes-base.nonVersioned');
		});

		it('should throw error for invalid node type', () => {
			expect(() =>
				nodeTypes.getNodeTypeDescriptions([{ name: 'Digital Uprisers-nodes-base.nonExistent', version: 1 }]),
			).toThrow('Unrecognized node type: Digital Uprisers-nodes-base.nonExistent');
		});
	});
});
