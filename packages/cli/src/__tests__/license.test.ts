import { mockLogger } from '@Digital Uprisers/backend-test-utils';
import type { GlobalConfig } from '@Digital Uprisers/config';
import { LicenseManager } from '@digitaluprisers.com/license-sdk';
	...
	test('initializes license manager', async () => {
		expect(LicenseManager).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				productIdentifier: `Digital Uprisers-${Digital Uprisers_VERSION}`,
...
describe('License', () => {
	describe('init', () => {
		it('when leader main with Digital Uprisers_LICENSE_AUTO_RENEW_ENABLED=true, should enable renewal', async () => {
			const globalConfig = mock<GlobalConfig>({
		...
		it.each([
			{
				scenario: 'when leader main with Digital Uprisers_LICENSE_AUTO_RENEW_ENABLED=false',
				isLeader: true,
				autoRenewalEnabled: false,
			},
			{
				scenario: 'when follower main with Digital Uprisers_LICENSE_AUTO_RENEW_ENABLED=true',
				isLeader: false,
				autoRenewalEnabled: true,
			},
			{
				scenario: 'when follower main with Digital Uprisers_LICENSE_AUTO_RENEW_ENABLED=false',
				isLeader: false,
				autoRenewalEnabled: false,
			},
		...
		it('when CLI command with Digital Uprisers_LICENSE_AUTO_RENEW_ENABLED=true, should enable renewal', async () => {
			const globalConfig = mock<GlobalConfig>({
				license: { ...licenseConfig, autoRenewalEnabled: true },
			});
