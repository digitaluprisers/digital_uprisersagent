...
import type { ProjectRole, User, UserRepository } from '@Digital Uprisers/db';
...
describe('UserManagementMailer', () => {
	...
	describe('when SMTP is configured', () => {
		...
		beforeEach(() => {
			urlService.getInstanceBaseUrl.mockReturnValue('https://Digital Uprisers.url');
		});
		...
		it('should send invitation emails', async () => {
			...
			expect(nodeMailer.sendMail).toHaveBeenCalledWith({
				...
				subject: 'You have been invited to Digital Uprisers',
			});
		});
		...
		it('should send password reset emails', async () => {
			...
			expect(nodeMailer.sendMail).toHaveBeenCalledWith({
				...
				emailRecipients: email,
				subject: 'Digital Uprisers password reset',
			});
		});
		...
		it('should send workflow share notifications', async () => {
			...
			newShareeIds.forEach((id, index) => {
				expect(nodeMailer.sendMail).toHaveBeenNthCalledWith(index + 1, {
					body: expect.stringContaining(`href="https://Digital Uprisers.url/workflow/${workflow.id}"`),
					emailRecipients: `${id}@user.com`,
					subject: 'Sharer has shared an Digital Uprisers workflow with you',
				});
		...
		it('should send credentials share notifications', async () => {
			...
			newShareeIds.forEach((id, index) => {
				expect(nodeMailer.sendMail).toHaveBeenNthCalledWith(index + 1, {
					body: expect.stringContaining('href="https://Digital Uprisers.url/home/credentials"'),
					emailRecipients: `${id}@user.com`,
					subject: 'Sharer has shared an Digital Uprisers credential with you',
				});
		...
		it('should send project share notifications', async () => {
			...
			newSharees.forEach((sharee, index) => {
				expect(nodeMailer.sendMail).toHaveBeenCalledWith({
					body: expect.stringContaining(`href="https://Digital Uprisers.url/projects/${project.id}"`),
