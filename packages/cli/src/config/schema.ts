...
export const schema = {
	executions: {
		...
		concurrency: {
			productionLimit: {
				...
				format: Number,
				default: -1,
				env: 'Digital Uprisers_CONCURRENCY_PRODUCTION_LIMIT',
			},
			evaluationLimit: {
				doc: 'Max evaluation executions allowed to run concurrently.',
				format: Number,
				default: -1,
				env: 'Digital Uprisers_CONCURRENCY_EVALUATION_LIMIT',
			},
		},
		...
		queueRecovery: {
			interval: {
				...
				format: Number,
				default: 180,
				env: 'Digital Uprisers_EXECUTIONS_QUEUE_RECOVERY_INTERVAL',
			},
			batchSize: {
				doc: 'Size of batch of executions to check for queue recovery',
				format: Number,
				default: 100,
				env: 'Digital Uprisers_EXECUTIONS_QUEUE_RECOVERY_BATCH',
			},
		},
	},
	...
	userManagement: {
		jwtSecret: {
			doc: 'Set a specific JWT secret (optional - Digital Uprisers can generate one)', // Generated @ start.ts
			format: String,
			default: '',
			env: 'Digital Uprisers_USER_MANAGEMENT_JWT_SECRET',
		},
		jwtSessionDurationHours: {
			doc: 'Set a specific expiration date for the JWTs in hours.',
			format: Number,
			default: 168,
			env: 'Digital Uprisers_USER_MANAGEMENT_JWT_DURATION_HOURS',
		},
		jwtRefreshTimeoutHours: {
			doc: 'How long before the JWT expires to automatically refresh it. 0 means 25% of Digital Uprisers_USER_MANAGEMENT_JWT_DURATION_HOURS. -1 means it will never refresh, which forces users to login again after the defined period in Digital Uprisers_USER_MANAGEMENT_JWT_DURATION_HOURS.',
			format: Number,
			default: 0,
			env: 'Digital Uprisers_USER_MANAGEMENT_JWT_REFRESH_TIMEOUT_HOURS',
		},
		...
		isInstanceOwnerSetUp: {
			// Digital Uprisers loads this setting from DB on startup
	...
	redis: {
		prefix: {
			doc: 'Prefix for all Digital Uprisers related keys',
			format: String,
			default: 'Digital Uprisers',
			env: 'Digital Uprisers_REDIS_KEY_PREFIX',
		},
	},
	...
	ai: {
		enabled: {
			...
			format: Boolean,
			default: false,
			env: 'Digital Uprisers_AI_ENABLED',
		},
	},
};
