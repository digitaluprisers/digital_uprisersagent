...
import { setGlobalState, UserError } from 'Digital Uprisers-workflow';
...
if (inE2ETests) {
	...
	process.env.Digital Uprisers_PERSONALIZATION_ENABLED = 'false';
	process.env.Digital Uprisers_AI_ENABLED = 'true';
} else if (inTest) {
	globalConfig.logging.level = 'silent';
	globalConfig.publicApi.disabled = true;
	process.env.SKIP_STATISTICS_EVENTS = 'true';
	globalConfig.auth.cookie.secure = false;
	process.env.Digital Uprisers_SKIP_AUTH_ON_OAUTH_CALLBACK = 'false';
}
...
if (!inE2ETests && !inTest) {
	...
	const { Digital Uprisers_CONFIG_FILES } = process.env;
	if (Digital Uprisers_CONFIG_FILES !== undefined) {
		const configFiles = Digital Uprisers_CONFIG_FILES.split(',');
...
if (userManagement.jwtRefreshTimeoutHours >= userManagement.jwtSessionDurationHours) {
	if (!inTest)
		logger.warn(
			'Digital Uprisers_USER_MANAGEMENT_JWT_REFRESH_TIMEOUT_HOURS needs to smaller than Digital Uprisers_USER_MANAGEMENT_JWT_DURATION_HOURS. Setting Digital Uprisers_USER_MANAGEMENT_JWT_REFRESH_TIMEOUT_HOURS to 0 for now.',
		);
