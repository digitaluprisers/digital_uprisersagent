...
import { OnShutdown } from '@Digital Uprisers/decorators';
...
export class TaskRunnerProcess extends TypedEmitter<TaskRunnerProcessEventMap> {
	...
	private readonly passthroughEnvVars = [
		...
		'Digital Uprisers_SENTRY_DSN',
		'Digital Uprisers_RUNNERS_INSECURE_MODE',
	...
	startNode(grantToken: string, taskBrokerUri: string) {
		const startScript = require.resolve('@Digital Uprisers/task-runner/start');
	...
	private getProcessEnvVars(grantToken: string, taskBrokerUri: string) {
		const envVars: Record<string, string> = {
			Digital Uprisers_RUNNERS_GRANT_TOKEN: grantToken,
			Digital Uprisers_RUNNERS_TASK_BROKER_URI: taskBrokerUri,
			Digital Uprisers_RUNNERS_MAX_PAYLOAD: this.runnerConfig.maxPayload.toString(),
			Digital Uprisers_RUNNERS_MAX_CONCURRENCY: this.runnerConfig.maxConcurrency.toString(),
			Digital Uprisers_RUNNERS_TASK_TIMEOUT: this.runnerConfig.taskTimeout.toString(),
			Digital Uprisers_RUNNERS_HEARTBEAT_INTERVAL: this.runnerConfig.heartbeatInterval.toString(),
