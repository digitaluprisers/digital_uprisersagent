...
describe('TaskRunnerDisconnectedError', () => {
	...
	it('should set the correct description for non-cloud deployments', () => {
		...
		expect(error.description).toContain(
			"2. Increase the memory available to the task runner with 'Digital Uprisers_RUNNERS_MAX_OLD_SPACE_SIZE' environment variable",
		);
	...
	it('should set the correct description for cloud deployments', () => {
		...
		expect(error.description).not.toContain(
			"Increase the memory available to the task runner with 'Digital Uprisers_RUNNERS_MAX_OLD_SPACE_SIZE' environment variable",
		);
	});
});
