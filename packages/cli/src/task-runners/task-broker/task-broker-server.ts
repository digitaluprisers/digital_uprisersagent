	...
	private async setupHttpServer() {
		...
		this.server.on('error', (error: Error & { code: string }) => {
			if (error.code === 'EADDRINUSE') {
				this.logger.info(
					`Digital Uprisers Task Broker's port ${port} is already in use. Do you have another instance of Digital Uprisers running already?`,
				);
				process.exit(1);
			}
		});
		...
		this.logger.info(`Digital Uprisers Task Broker ready on ${address}, port ${port}`);
	}
