import type { TaskRunnerMode } from '@Digital Uprisers/config/src/configs/runners.config';
...
export class TaskRunnerExecutionTimeoutError extends OperationalError {
	...
	constructor({
		...
		const fixes = {
			...
			increaseTimeout: `If your task can reasonably take more than ${taskTimeout} ${taskTimeout === 1 ? 'second' : 'seconds'}, increase the timeout using the Digital Uprisers_RUNNERS_TASK_TIMEOUT environment variable.`,
		};
