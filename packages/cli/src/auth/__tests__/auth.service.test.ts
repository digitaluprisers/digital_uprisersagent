...
import type { InvalidAuthTokenRepository } from '@Digital Uprisers/db';
import type { UserRepository } from '@Digital Uprisers/db';
...
describe('AuthService', () => {
	...
	describe('authMiddleware', () => {
		...
		it('should refresh the cookie before it expires', async () => {
			...
			expect(res.cookie).toHaveBeenCalledWith('Digital Uprisers-auth', expect.any(String), {
				httpOnly: true,
				maxAge: 604800000,
				sameSite: 'lax',
				secure: true,
	...
	describe('issueCookie', () => {
		...
		it('should issue a cookie with the correct options', () => {
			...
			expect(res.cookie).toHaveBeenCalledWith('Digital Uprisers-auth', validToken, {
				httpOnly: true,
				maxAge: 604800000,
				sameSite: 'lax',
				secure: true,
		...
		it('should issue a cookie with the correct options, when 2FA was used', () => {
			...
			expect(res.cookie).toHaveBeenCalledWith('Digital Uprisers-auth', validTokenWithMfa, {
				httpOnly: true,
				maxAge: 604800000,
				sameSite: 'lax',
				secure: true,
	...
	describe('generatePasswordResetUrl', () => {
		it('should generate a valid url', () => {
			urlService.getInstanceBaseUrl.mockReturnValue('https://Digital Uprisers.instance');
			const url = authService.generatePasswordResetUrl(user);
			expect(url).toEqual(
				'https://Digital Uprisers.instance/change-password?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjMiLCJoYXNoIjoibUpBWXg0V2I3ayIsImlhdCI6MTcwNjc1MDYyNSwiZXhwIjoxNzA2NzUxODI1fQ.rg90I7MKjc_KC77mov59XYAeRc-CoW9ka4mt1dCfrnk&mfaEnabled=false',
			);
		});
	});
