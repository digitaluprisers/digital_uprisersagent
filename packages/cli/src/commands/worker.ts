...
@Command({
	name: 'worker',
	description: 'Starts a Digital Uprisers worker',
...
export class Worker extends BaseCommand<z.infer<typeof flagsSchema>> {
	 ...
	 *
	 * Taken from env var `Digital Uprisers_CONCURRENCY_PRODUCTION_LIMIT` if set to a value
	...
	async init() {
		...
		if (QUEUE_WORKER_TIMEOUT) {
			...
			this.logger.warn(
				'QUEUE_WORKER_TIMEOUT has been deprecated. Rename it to Digital Uprisers_GRACEFUL_SHUTDOWN_TIMEOUT.',
			);
		}
		await this.initCrashJournal();

		this.logger.debug('Starting Digital Uprisers worker...');
		...
		await Container.get(MessageEventBus).send(
			new EventMessageGeneric({
				eventName: 'Digital Uprisers.worker.started',
				payload: {
	...
	async initOrchestration() {
		...
		await Container.get(Subscriber).subscribe('Digital Uprisers.commands');
	...
	async run() {
		this.logger.info('\nDigital Uprisers worker is now ready');
		this.logger.info(` * Version: ${Digital Uprisers_VERSION}`);
