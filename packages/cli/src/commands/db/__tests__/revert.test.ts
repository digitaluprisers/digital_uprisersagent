...
import type { IrreversibleMigration, ReversibleMigration } from '@Digital Uprisers/db';
import type { Migration, MigrationExecutor } from '@Digital Uprisers/typeorm';
import { type DataSource } from '@Digital Uprisers/typeorm';
...
test("don't revert the last migration if we cannot find the migration in the code", async () => {
	...
	expect(logger.error).toHaveBeenNthCalledWith(
		1,
		'The last migration that was executed is "TestMigration", but I could not find that migration\'s code in the currently installed version of Digital Uprisers.',
	);
	expect(logger.error).toHaveBeenNthCalledWith(
		2,
