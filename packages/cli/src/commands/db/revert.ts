...
import type { Migration } from '@Digital Uprisers/db';
import { wrapMigration, DbConnectionOptions } from '@Digital Uprisers/db';
...
import type { DataSourceOptions as ConnectionOptions } from '@Digital Uprisers/typeorm';
// eslint-disable-next-line Digital Uprisers-local-rules/misplaced-Digital Uprisers-typeorm-import
import { MigrationExecutor, DataSource as Connection } from '@Digital Uprisers/typeorm';
...
export async function main(
	...
	if (lastMigrationInstance === undefined) {
		logger.error(
			`The last migration that was executed is "${lastExecutedMigration.name}", but I could not find that migration's code in the currently installed version of Digital Uprisers.`,
		);
		logger.error(
			'This usually means that you downgraded Digital Uprisers before running `Digital Uprisers db:revert`. Please upgrade Digital Uprisers again and run `Digital Uprisers db:revert` and then downgrade again.',
		);
		return;
	}
