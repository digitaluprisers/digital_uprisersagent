...
import { LICENSE_FEATURES } from '@Digital Uprisers/constants';
import { ExecutionRepository, SettingsRepository } from '@Digital Uprisers/db';
import { Command } from '@Digital Uprisers/decorators';
import { Container } from '@Digital Uprisers/di';
import glob from 'fast-glob';
import { createReadStream, createWriteStream, existsSync } from 'fs';
import { mkdir } from 'fs/promises';
import { jsonParse, randomString, type IWorkflowExecutionDataProcess } from 'Digital Uprisers-workflow';
...
const flagsSchema = z.object({
	...
	tunnel: z
		.boolean()
		.describe(
			'runs the webhooks via a hooks.Digital Uprisers.cloud tunnel server. Use only for testing and development!',
		)
		.optional(),
	reinstallMissingPackages: z
		.boolean()
		.describe(
			'Attempts to self heal Digital Uprisers if packages with nodes are missing. Might drastically increase startup times.',
		)
		.optional(),
});
...
@Command({
	name: 'start',
	description: 'Starts Digital Uprisers. Makes Web-UI available and starts active workflows',
...
export class Start extends BaseCommand<z.infer<typeof flagsSchema>> {
	...
	/**
	 * Stop Digital Uprisers in a graceful way.
	 * Make for example sure that all the webhooks from third party services
	 * get removed.
	 */
	async stopProcess() {
		this.logger.info('\nStopping Digital Uprisers...');
		...
		try {
			...
			await this.externalHooks?.run('Digital Uprisers.stop');
			...
			await this.exitWithCrash('There was an error shutting down Digital Uprisers.', error);
		}
	...
	private async generateStaticAssets() {
		...
		const Digital UprisersPath = this.globalConfig.path;
		...
		const compileFile = async (fileName: string) => {
			...
			if (/(index\.html)|.*\.(js|css)/.test(filePath) && existsSync(filePath)) {
				...
				const streams = [
					...
					replaceStream('/{{BASE_PATH}}/', Digital UprisersPath, { ignoreCase: false }),
					replaceStream('/%7B%7BBASE_PATH%7D%7D/', Digital UprisersPath, { ignoreCase: false }),
					replaceStream('/%257B%257BBASE_PATH%257D%257D/', Digital UprisersPath, { ignoreCase: false }),
					replaceStream('/static/', Digital UprisersPath + 'static/', { ignoreCase: false }),
				];
	...
	async init() {
		...
		this.logger.info('Initializing Digital Uprisers process');
		if (config.getEnv('executions.mode') === 'queue') {
		...
		if (flags.reinstallMissingPackages) {
			if (communityPackages.enabled) {
				this.logger.warn(
					'`--reinstallMissingPackages` is deprecated: Please use the env variable `Digital Uprisers_REINSTALL_MISSING_PACKAGES` instead',
				);
	...
	async initOrchestration() {
		...
		await subscriber.subscribe('Digital Uprisers.commands');
		await subscriber.subscribe('Digital Uprisers.worker-response');
	...
	async run() {
		...
		if (flags.tunnel) {
			...
			let tunnelSubdomain =
				process.env.Digital Uprisers_TUNNEL_SUBDOMAIN ?? this.instanceSettings.tunnelSubdomain ?? '';
			...
			const { default: localtunnel } = await import('@Digital Uprisers/localtunnel');
			const { port } = this.globalConfig;

			const webhookTunnel = await localtunnel(port, {
				host: 'https://hooks.Digital Uprisers.cloud',
			...
			this.log(
				'IMPORTANT! Do not share with anybody as it would give people access to your Digital Uprisers instance!',
			);
		}
		...
		// Allow to open Digital Uprisers editor by pressing "o"
