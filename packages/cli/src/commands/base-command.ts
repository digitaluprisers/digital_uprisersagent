...
import {
	...
	ModuleRegistry,
	ModulesConfig,
} from '@Digital Uprisers/backend-common';
...
import { ensureError, sleep, UnexpectedError, UserError } from 'Digital Uprisers-workflow';

import type { AbstractServer } from '@/abstract-server';
import config from '@/config';
import { Digital Uprisers_VERSION, Digital Uprisers_RELEASE_DATE } from '@/constants';
...
export abstract class BaseCommand<F = never> {
	...
	async init(): Promise<void> {
		...
		await this.errorReporter.init({
			...
			releaseDate: Digital Uprisers_RELEASE_DATE,
		});
	...
	async initObjectStoreService() {
		...
		if (isSelected && !isAvailable) {
			throw new UserError(
				'External storage selected but unavailable. Please make external storage available by adding "s3" to `Digital Uprisers_AVAILABLE_BINARY_DATA_MODES`.',
			);
		}
		...
		if (!isLicensed) {
			this.logger.error(
				'No license found for S3 storage. \n Either set `Digital Uprisers_DEFAULT_BINARY_DATA_MODE` to something else, or upgrade to a license that supports this feature.',
			);
