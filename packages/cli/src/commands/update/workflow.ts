...
export class UpdateWorkflowCommand extends BaseCommand<z.infer<typeof flagsSchema>> {
	async run() {
		...
		this.logger.info('Activation or deactivation will not take effect if Digital Uprisers is running.');
		this.logger.info('Please restart Digital Uprisers for changes to take effect if Digital Uprisers is currently running.');
	}
