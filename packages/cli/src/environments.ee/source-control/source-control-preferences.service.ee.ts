...
import { Cipher, InstanceSettings } from 'Digital Uprisers-core';
...
export class SourceControlPreferencesService {
	...
	constructor(
		...
		this.sshFolder = path.join(instanceSettings.Digital UprisersFolder, SOURCE_CONTROL_SSH_FOLDER);
	...
	async getPrivateKeyPath() {
		...
		const tempFilePath = path.join(this.instanceSettings.Digital UprisersFolder, 'ssh_private_key_temp');
