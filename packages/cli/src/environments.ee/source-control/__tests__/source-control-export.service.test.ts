...
import type { SharedCredentials } from '@Digital Uprisers/db';
import type { SharedWorkflow } from '@Digital Uprisers/db';
import type { FolderRepository } from '@Digital Uprisers/db';
import type { TagRepository } from '@Digital Uprisers/db';
import type { WorkflowTagMappingRepository } from '@Digital Uprisers/db';
import type { SharedCredentialsRepository } from '@Digital Uprisers/db';
...
import { Cipher, type InstanceSettings } from 'Digital Uprisers-core';
...
describe('SourceControlExportService', () => {
	...
	const service = new SourceControlExportService(
		...
		mock<InstanceSettings>({ Digital UprisersFolder: '/mock/Digital Uprisers' }),
	);
	...
	describe('exportCredentialsToWorkFolder', () => {
		...
		it('should export credentials to work folder', async () => {
			...
			expect(fsWriteFile).toHaveBeenCalledWith(
				'/mock/Digital Uprisers/git/credential_stubs/cred1.json',
				dataCaptor,
			);
