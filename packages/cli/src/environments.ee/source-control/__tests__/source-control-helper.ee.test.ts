...
beforeAll(async () => {
	...
	sourceControlPreferencesService.getPreferences.mockReturnValue({
		...
		branchColor: '#5296D6',
		publicKey:
			'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDBSz2nMZAiUBWe6n89aWd5x9QMcIOaznVW3fpuCYC4L Digital Uprisers deploy key',
	});
});
...
describe('Source Control', () => {
	...
	it('should check for git and ssh folders and create them if required', async () => {
		...
		const sshFolder = path.join(Digital UprisersFolder, SOURCE_CONTROL_SSH_FOLDER);
		const gitFolder = path.join(Digital UprisersFolder, SOURCE_CONTROL_GIT_FOLDER);
		let hasThrown = false;
		try {
	...
	it('should get repo type from url', async () => {
		expect(getRepoType('git@github.com:Digital Upriserstest/Digital Uprisers_testrepo.git')).toBe('github');
		expect(getRepoType('git@gitlab.com:Digital Upriserstest/Digital Uprisers_testrepo.git')).toBe('gitlab');
		expect(getRepoType('git@mygitea.io:Digital Upriserstest/Digital Uprisers_testrepo.git')).toBe('other');
	});
