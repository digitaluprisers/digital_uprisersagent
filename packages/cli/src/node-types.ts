...
import type { NeededNodeType } from '@Digital Uprisers/task-runner';
import type { Dirent } from 'fs';
import { readdir } from 'fs/promises';
import { RoutingNode } from 'Digital Uprisers-core';
import type { ExecuteContext } from 'Digital Uprisers-core';
import type { INodeType, INodeTypeDescription, INodeTypes, IVersionedNodeType } from 'Digital Uprisers-workflow';
import { NodeHelpers, UnexpectedError, UserError } from 'Digital Uprisers-workflow';
...
export class NodeTypes implements INodeTypes {
	...
	async getNodeTranslationPath({
		...
		const nodeType = longNodeType.replace('Digital Uprisers-nodes-base.', '');
	...
	getNodeTypeDescriptions(nodeTypes: NeededNodeType[]): INodeTypeDescription[] {
		return nodeTypes.map(({ name: nodeTypeName, version: nodeTypeVersion }) => {
			...
			descriptionCopy.name = descriptionCopy.name.startsWith('Digital Uprisers-nodes')
				? descriptionCopy.name
				: `Digital Uprisers-nodes-base.${descriptionCopy.name}`; // nodes-base nodes are unprefixed
