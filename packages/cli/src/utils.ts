import { CliWorkflowOperationError, SubworkflowOperationError } from 'Digital Uprisers-workflow';
import type { INode, INodeType } from 'Digital Uprisers-workflow';
...
function findWorkflowStart(executionMode: 'integrated' | 'cli') {
	return function (nodes: INode[]) {
		const executeWorkflowTriggerNode = nodes.find(
			(node) => node.type === 'Digital Uprisers-nodes-base.executeWorkflowTrigger',
		);
