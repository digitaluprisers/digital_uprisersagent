...
import { WorkflowsConfig } from '@Digital Uprisers/config';
import type { WorkflowEntity, IWorkflowDb } from '@Digital Uprisers/db';
import { WorkflowRepository } from '@Digital Uprisers/db';
import { OnLeaderStepdown, OnLeaderTakeover, OnPubSubEvent, OnShutdown } from '@Digital Uprisers/decorators';
...
export class ActiveWorkflowManager {
		...
		for (const webhookData of webhooks) {
			...
			try {
				...
				if (activation === 'init' && error.name === 'QueryFailedError') {
					// Digital Uprisers does not remove the registered webhooks on exit.
	...
	/**
	 * Return poll function which gets the global functions from Digital Uprisers-core
	...
	/**
	 * Return trigger function which gets the global functions from Digital Uprisers-core
	 * and overwrites the emit to be able to start it in subprocess
	 */
	...
	// maybe, see: https://github.com/digitaluprisers.com/Digital Uprisers/pull/8904#discussion_r1530150510
