...
import { EventMessageTypeNames, jsonParse } from 'Digital Uprisers-workflow';
		...
		if (!MessageEventBusLogWriter.instance) {
			...
			MessageEventBusLogWriter.options = {
				logFullBasePath: path.join(
					options?.logBasePath ?? Container.get(InstanceSettings).Digital UprisersFolder,
						...
						if (isEventMessageOptions(json) && json.__type !== undefined) {
							...
							if (msg?.eventName && msg.payload?.executionId) {
								...
								switch (msg.eventName) {
									case 'Digital Uprisers.workflow.started':
										...
										break;
									case 'Digital Uprisers.workflow.success':
									case 'Digital Uprisers.workflow.failed':
									case 'Digital Uprisers.execution.throttled':
									case 'Digital Uprisers.execution.started-during-bootup':
										delete results.unfinishedExecutions[executionId];
										break;
									case 'Digital Uprisers.node.started':
									case 'Digital Uprisers.node.finished':
										if (!Object.keys(results.unfinishedExecutions).includes(executionId)) {
											results.unfinishedExecutions[executionId] = [];
										}
