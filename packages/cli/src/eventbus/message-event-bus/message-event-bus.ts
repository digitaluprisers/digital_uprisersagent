...
import { EventDestinationsRepository, ExecutionRepository, WorkflowRepository } from '@Digital Uprisers/db';
...
import type { DeleteResult } from '@Digital Uprisers/typeorm';
// eslint-disable-next-line Digital Uprisers-local-rules/misplaced-Digital Uprisers-typeorm-import
...
// eslint-disable-next-line Digital Uprisers-local-rules/no-type-unsafe-event-emitter
			...
			if (unfinishedExecutionIds.length > 0) {
				...
				if (recoveryAlreadyAttempted || this.globalConfig.eventBus.crashRecoveryMode === 'simple') {
					...
					if (recoveredIds.length > 0) {
						...
						this.logger.info(
							'This could be due to a crash of an active workflow or a restart of Digital Uprisers',
						);
					}
				}
