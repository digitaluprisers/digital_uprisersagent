...
import type { Project, User, CreateExecutionPayload } from '@Digital Uprisers/db';
...
import { SubworkflowOperationError, Workflow } from 'Digital Uprisers-workflow';
...
export class WorkflowExecutionService {
	 ...
	 * to the workflow, prioritizing `Digital Uprisers-nodes-base.webhook` over other activators.
	 *
	 ...
	 * prioritizing `Digital Uprisers-nodes-base.webhook` over other activators. If the executed node
		...
		return workflow.nodes
			.filter(
				(node) =>
					...
					node.type !== 'Digital Uprisers-nodes-base.respondToWebhook',
			)
