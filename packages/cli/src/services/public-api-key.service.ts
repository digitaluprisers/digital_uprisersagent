import type { UnixTimestamp, UpdateApiKeyRequestDto } from '@Digital Uprisers/api-types';
import type { CreateApiKeyRequestDto } from '@Digital Uprisers/api-types/src/dto/api-keys/create-api-key-request.dto';
import type { AuthenticatedRequest, User } from '@Digital Uprisers/db';
import { ApiKey, ApiKeyRepository, UserRepository } from '@Digital Uprisers/db';
import { Service } from '@Digital Uprisers/di';
import type { GlobalRole, ApiKeyScope } from '@Digital Uprisers/permissions';
import { getApiKeyScopesForRole, getOwnerOnlyApiKeyScopes } from '@Digital Uprisers/permissions';
...
const API_KEY_ISSUER = 'Digital Uprisers';
const REDACT_API_KEY_REVEAL_COUNT = 4;
const REDACT_API_KEY_MAX_LENGTH = 10;
const PREFIX_LEGACY_API_KEY = 'Digital Uprisers_api_';
...
export class PublicApiKeyService {
	...
	getApiKeyScopeMiddleware(endpointScope: ApiKeyScope) {
		return async (req: Request, res: Response, next: NextFunction) => {
			const apiKey = req.headers['x-Digital Uprisers-api-key'];
