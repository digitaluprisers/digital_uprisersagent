import type { FrontendSettings, ITelemetrySettings } from '@Digital Uprisers/api-types';
import { LicenseState, Logger, ModuleRegistry } from '@Digital Uprisers/backend-common';
import { GlobalConfig, SecurityConfig } from '@Digital Uprisers/config';
import { LICENSE_FEATURES } from '@Digital Uprisers/constants';
import { Container, Service } from '@Digital Uprisers/di';
import { createWriteStream } from 'fs';
import { mkdir } from 'fs/promises';
import uniq from 'lodash/uniq';
import { BinaryDataConfig, InstanceSettings } from 'Digital Uprisers-core';
import type { ICredentialType, INodeTypeBaseDescription } from 'Digital Uprisers-workflow';
import path from 'path';

import config from '@/config';
import { inE2ETests, Digital Uprisers_VERSION } from '@/constants';
...
export class FrontendService {
	...
	private initSettings() {
		...
		this.settings = {
			...
			previewMode: process.env.Digital Uprisers_PREVIEW_MODE === 'true',
			...
			versionCli: Digital Uprisers_VERSION,
			...
			security: {
				blockFileAccessToDigital UprisersFiles: this.securityConfig.blockFileAccessToDigital UprisersFiles,
			},
