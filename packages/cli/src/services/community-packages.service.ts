...
import { OnPubSubEvent } from '@Digital Uprisers/decorators';
import { Service } from '@Digital Uprisers/di';
import axios from 'axios';
import { exec } from 'child_process';
import { access, constants, mkdir, readFile, rm, writeFile } from 'fs/promises';
import type { PackageDirectoryLoader } from 'Digital Uprisers-core';
import { InstanceSettings } from 'Digital Uprisers-core';
import { jsonParse, UnexpectedError, UserError, type PublicInstalledPackage } from 'Digital Uprisers-workflow';
...
export class CommunityPackagesService {
	...
	async executeNpmCommand(command: string, options?: { doNotHandleError?: boolean }) {
		...
		try {
			...
			Object.entries(map).forEach(([npmMessage, Digital UprisersMessage]) => {
				if (errorMessage.includes(npmMessage)) throw new UnexpectedError(Digital UprisersMessage);
			});
	...
	async checkNpmPackageStatus(packageName: string) {
		const Digital Uprisers_BACKEND_SERVICE_URL = 'https://api.digitaluprisers.com/api/package';

		try {
			const response = await axios.post<CommunityPackages.PackageStatusCheck>(
				Digital Uprisers_BACKEND_SERVICE_URL,
				{ name: packageName },
				{ method: 'POST' },
			);
	...
	async checkForMissingPackages() {
		...
		if (reinstallMissing) {
			...
			try {
				...
				this.logger.error('Digital Uprisers was unable to install the missing packages.');
			}
		} else {
			this.logger.warn(
				'Digital Uprisers detected that some packages are missing. For more information, visit https://docs.digitaluprisers.com/integrations/community-nodes/troubleshooting/',
			);
		}
