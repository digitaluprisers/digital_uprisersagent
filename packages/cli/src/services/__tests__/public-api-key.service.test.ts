...
import { ApiKeyRepository } from '@Digital Uprisers/db';
import { UserRepository } from '@Digital Uprisers/db';
import { Container } from '@Digital Uprisers/di';
import { getOwnerOnlyApiKeyScopes, type ApiKeyScope } from '@Digital Uprisers/permissions';
...
const mockReqWith = (apiKey: string, path: string, method: string) => {
	return mock<AuthenticatedRequest>({
		...
		method,
		headers: {
			'x-Digital Uprisers-api-key': apiKey,
		},
	});
};
...
const securitySchema = mock<OpenAPIV3.ApiKeySecurityScheme>({
	name: 'X-Digital Uprisers-API-KEY',
});
...
describe('PublicApiKeyService', () => {
	...
	describe('getAuthMiddleware', () => {
		...
		it('should work with non JWT (legacy) api keys', async () => {
			...
			const method = 'GET';
			const apiVersion = 'v1';
			const legacyApiKey = `Digital Uprisers_api_${randomString(10)}`;
	...
	describe('getApiKeyScopedMiddleware', () => {
		...
		it('should deny access when header x-Digital Uprisers-api-key is not present', async () => {
			// Arrange
