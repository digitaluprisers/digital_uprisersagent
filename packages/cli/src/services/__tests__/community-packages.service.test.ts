...
import { InstalledNodes } from '@Digital Uprisers/db';
...
import type { InstanceSettings, PackageDirectoryLoader } from 'Digital Uprisers-core';
import type { PublicInstalledPackage } from 'Digital Uprisers-workflow';
...
describe('CommunityPackagesService', () => {
	...
	const nodesDownloadDir = '/tmp/Digital Uprisers-jest-global-downloads';
	...
	describe('updatePackage', () => {
		const PACKAGE_NAME = 'Digital Uprisers-nodes-test';
		...
		test('should call `exec` with the correct sequence of commands, handle file ops, and interact with services', async () => {
			...
			expect(rm).toHaveBeenNthCalledWith(2, `${nodesDownloadDir}/Digital Uprisers-nodes-test-latest.tgz`);
	...
	describe('checkForMissingPackages', () => {
		...
		test('should handle failed reinstallations and record missing packages', async () => {
			...
			expect(communityPackagesService.installPackage).toHaveBeenCalledWith('package-1', '1.0.0');
			expect(logger.error).toHaveBeenCalledWith('Digital Uprisers was unable to install the missing packages.');
			expect(communityPackagesService.missingPackages).toEqual(['package-1@1.0.0']);
		});
