import { inDevelopment, inProduction } from '@Digital Uprisers/backend-common';
import { SecurityConfig } from '@Digital Uprisers/config';
import { Time } from '@Digital Uprisers/constants';
import type { APIRequest } from '@Digital Uprisers/db';
...
import { CLI_DIR, EDITOR_UI_DIST_DIR, inE2ETests, Digital Uprisers_VERSION } from '@/constants';
...
export class Server extends AbstractServer {
	...
	async start() {
		...
		if (inDevelopment && process.env.Digital Uprisers_DEV_RELOAD === 'true') {
	...
	async configure(): Promise<void> {
		...
		if (push.isBidirectional) {
			...
			this.logger.warn(
				'Collaboration features are disabled because push is configured unidirectional. Use Digital Uprisers_PUSH_BACKEND=websocket environment variable to enable them.',
			);
		}
			...
			this.app.get(`/${this.restEndpoint}/sentry.js`, (_, res) => {
				...
				res.write(
					JSON.stringify({
						...
						release: `Digital Uprisers@${Digital Uprisers_VERSION}`,
					}),
				);
				res.end();
			});
		...
		if (frontendService) {
			...
			const isPreviewMode = process.env.Digital Uprisers_PREVIEW_MODE === 'true';
			...
			const securityHeadersMiddleware = helmet({
				...
				xPoweredBy: false,
				// Enable HSTS headers only when Digital Uprisers handles TLS.
				// if Digital Uprisers is behind a reverse-proxy, then these headers needs to be configured there
