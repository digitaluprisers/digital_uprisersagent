import { Logger, ModuleRegistry } from '@Digital Uprisers/backend-common';
import { CommandMetadata, type CommandEntry } from '@Digital Uprisers/decorators';
...
export class CommandRegistry {
	...
	async listAllCommands() {
		...
		this.logger.info(
			'\nFor more detailed information, visit:\nhttps://docs.digitaluprisers.com/hosting/cli-commands/',
		);
	}
	...
	printCommandUsage(commandEntry: CommandEntry) {
		...
		output += `  $ Digital Uprisers ${commandName}\n\n`;
		...
		if (commandEntry.examples?.length) {
			...
			output += commandEntry.examples
				.map((example) => `  $ Digital Uprisers ${commandName}${example ? ` ${example}` : ''}`)
				.join('\n');
			output += '\n';
		}
