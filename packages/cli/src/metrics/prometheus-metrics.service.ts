...
export class PrometheusMetricsService {
	...
	async init(app: express.Application) {
		...
		this.initDigital UprisersVersionMetric();
	...
	/**
	 * Set up metric for Digital Uprisers version: `Digital Uprisers_version_info`
	 */
	private initDigital UprisersVersionMetric() {
		const Digital UprisersVersion = semverParse(Digital Uprisers_VERSION ?? '0.0.0');

		if (!Digital UprisersVersion) return;
	...
	/**
	 * Set up cache metrics: `Digital Uprisers_cache_hits_total`, `Digital Uprisers_cache_misses_total`, and
	 * `Digital Uprisers_cache_updates_total`
	 */
	...
	private toCounter(event: EventMessageTypes) {
		...
		if (!this.counters[eventName]) {
			const metricName = this.prefix + eventName.replace('Digital Uprisers.', '').replace(/\./g, '_') + '_total';
	...
	private toLabels(event: EventMessageTypes): Record<string, string> {
		...
		switch (__type) {
			case EventMessageTypeNames.audit:
				if (eventName.startsWith('Digital Uprisers.audit.user.credentials')) {
					return this.includes.labels.credentialsType
						? { credential_type: (event.payload.credentialType ?? 'unknown').replace(/\./g, '_') }
						: {};
				}

				if (eventName.startsWith('Digital Uprisers.audit.workflow')) {
				...
				return this.includes.labels.nodeType
					? {
							node_type: (payload.nodeType ?? 'unknown')
								.replace('Digital Uprisers-nodes-', '')
								.replace(/\./g, '_'),
						}
					: {};
