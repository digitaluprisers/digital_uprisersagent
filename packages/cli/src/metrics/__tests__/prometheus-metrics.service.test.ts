...
describe('PrometheusMetricsService', () => {
	...
	describe('constructor', () => {
		it('should enable metrics based on global config', async () => {
			...
			expect(promClient.Counter).toHaveBeenCalledWith({
				name: 'Digital Uprisers_cache_hits_total',
	...
	describe('init', () => {
		it('should set up `Digital Uprisers_version_info`', async () => {
			await prometheusMetricsService.init(app);

			expect(promClient.Gauge).toHaveBeenNthCalledWith(1, {
				name: 'Digital Uprisers_version_info',
				help: 'Digital Uprisers version info.',
		...
		it('should set up `Digital Uprisers_cache_hits_total`', async () => {
		...
		it('should set up `Digital Uprisers_cache_misses_total`', async () => {
			prometheusMetricsService.enableMetric('cache');
			await prometheusMetricsService.init(app);

			expect(promClient.Counter).toHaveBeenCalledWith({
				name: 'Digital Uprisers_cache_misses_total',
				help: 'Total number of cache misses.',
				labelNames: ['cache'],
			});
		});

		it('should set up `Digital Uprisers_cache_updates_total`', async () => {
			prometheusMetricsService.enableMetric('cache');
			await prometheusMetricsService.init(app);

			expect(promClient.Counter).toHaveBeenCalledWith({
				name: 'Digital Uprisers_cache_updates_total',
		...
		it('should set up route metrics with `express-prom-bundle`', async () => {
			...
			expect(promBundle).toHaveBeenCalledWith({
				httpDurationMetricName: 'Digital Uprisers_http_request_duration_seconds',
				autoregister: false,
				includeUp: false,
				includePath: false,
				includeMethod: false,
			...
			expect(promClient.Gauge).toHaveBeenNthCalledWith(2, {
				name: 'Digital Uprisers_last_activity',
		...
		it('should set up queue metrics if enabled', async () => {
			...
			// call 1 is for `Digital Uprisers_version_info` (always enabled)

			expect(promClient.Gauge).toHaveBeenNthCalledWith(2, {
				name: 'Digital Uprisers_scaling_mode_queue_jobs_waiting',
				help: 'Current number of enqueued jobs waiting for pickup in scaling mode.',
			});

			expect(promClient.Gauge).toHaveBeenNthCalledWith(3, {
				name: 'Digital Uprisers_scaling_mode_queue_jobs_active',
			...
			expect(promClient.Counter).toHaveBeenNthCalledWith(1, {
				name: 'Digital Uprisers_scaling_mode_queue_jobs_completed',
			...
			expect(promClient.Counter).toHaveBeenNthCalledWith(2, {
				name: 'Digital Uprisers_scaling_mode_queue_jobs_failed',
		...
		it('should setup active workflow count metric', async () => {
			...
			// First call is Digital Uprisers version metric
			expect(promClient.Gauge).toHaveBeenCalledTimes(2);

			expect(promClient.Gauge).toHaveBeenNthCalledWith(2, {
				name: 'Digital Uprisers_active_workflow_count',
