...
import type { ExecutionStatus, INodesGraphResult, ITelemetryTrackProperties } from 'Digital Uprisers-workflow';
import { TelemetryHelpers } from 'Digital Uprisers-workflow';
...
export class TelemetryEventRelay extends EventRelay {
	...
	private async serverStarted() {
		...
		const info = {
			...
			Digital Uprisers_version_notifications_enabled: this.globalConfig.versionNotifications.enabled,
			Digital Uprisers_disable_production_main_process:
			...
			Digital Uprisers_deployment_type: this.globalConfig.deployment.type,
			Digital Uprisers_binary_data_mode: this.binaryDataConfig.mode,
