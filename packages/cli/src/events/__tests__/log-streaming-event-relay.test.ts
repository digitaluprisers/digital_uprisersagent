import type { IWorkflowDb } from '@Digital Uprisers/db';
import { mock } from 'jest-mock-extended';
import type { InstanceSettings } from 'Digital Uprisers-core';
import type { INode, IRun, IWorkflowBase } from 'Digital Uprisers-workflow';
...
describe('LogStreamingEventRelay', () => {
	...
	describe('workflow events', () => {
		it('should log on `workflow-created` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.workflow.created',
				payload: {
					userId: '123',
					_email: 'john@digitaluprisers.com',
					_firstName: 'John',
					_lastName: 'Doe',
					globalRole: 'owner',
					workflowId: 'wf123',
		...
		it('should log on `workflow-archived` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.workflow.archived',
				payload: {
					userId: '456',
					_email: 'jane@digitaluprisers.com',
					_firstName: 'Jane',
					_lastName: 'Smith',
					globalRole: 'user',
					workflowId: 'wf789',
		...
		it('should log on `workflow-unarchived` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.workflow.unarchived',
				payload: {
					userId: '456',
					_email: 'jane@digitaluprisers.com',
					_firstName: 'Jane',
		...
		it('should log on `workflow-deleted` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.workflow.deleted',
				payload: {
					userId: '456',
					_email: 'jane@digitaluprisers.com',
					_firstName: 'Jane',
		...
		it('should log on `workflow-saved` event', () => {
			const event: RelayEventMap['workflow-saved'] = {
				user: {
					id: '789',
					email: 'alex@digitaluprisers.com',
					firstName: 'Alex',
					lastName: 'Johnson',
					role: 'editor',
				},
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.workflow.updated',
				payload: {
					userId: '789',
					_email: 'alex@digitaluprisers.com',
					_firstName: 'Alex',
					_lastName: 'Johnson',
					globalRole: 'editor',
					workflowId: 'wf101',
		...
		it('should log on `workflow-post-execute` for successful execution', () => {
			...
			expect(eventBus.sendWorkflowEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.workflow.success',
				payload: {
					...rest,
		...
		it('should log job completion on `workflow-post-execute` for successful job', () => {
			...
			expect(eventBus.sendQueueEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.queue.job.completed',
				payload: {
					executionId: 'exec-123',
					workflowId: 'wf-789',
					hostId: 'host-xyz',
		...
		it('should log on `workflow-post-execute` event for failed execution', () => {
			...
			expect(eventBus.sendWorkflowEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.workflow.failed',
				payload: {
					...rest,
		...
		it('should log job failure on `workflow-post-execute` for failed job', () => {
			...
			expect(eventBus.sendQueueEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.queue.job.failed',
				payload: {
					executionId: 'exec-456',
					workflowId: 'wf-101',
					hostId: 'host-xyz',
	...
	describe('user events', () => {
		it('should log on `user-updated` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.updated',
				payload: {
					userId: 'user456',
		...
		it('should log on `user-deleted` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.deleted',
				payload: {
					userId: '123',
					_email: 'john@digitaluprisers.com',
					_firstName: 'John',
		...
		it('should log on `user-invited` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.invited',
				payload: {
					userId: 'user101',
		...
		it('should log on `user-reinvited` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.reinvited',
				payload: {
					userId: 'user202',
		...
		it('should log on `user-signed-up` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.signedup',
				payload: {
					userId: 'user303',
		...
		it('should log on `user-logged-in` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.login.success',
				payload: {
					userId: 'user404',
	...
	describe('click events', () => {
		it('should log on `user-password-reset-request-click` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.reset.requested',
				payload: {
					userId: 'user101',
		...
		it('should log on `user-invite-email-click` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.invitation.accepted',
				payload: {
					inviter: {
						userId: '123',
						_email: 'john@digitaluprisers.com',
		...
		it('should log on `user-password-reset-email-click` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.reset',
				payload: {
					userId: 'user505',
	...
	describe('node events', () => {
		it('should log on `node-pre-execute` event', () => {
			...
			const event: RelayEventMap['node-pre-execute'] = {
				...
				workflow,
				nodeId: 'node2',
				nodeType: 'Digital Uprisers-nodes-base.httpRequest',
			};
		...
		it('should log on `node-post-execute` event', () => {
			const workflow = mock<IWorkflowBase>({
				...
				nodes: [
					...
					},
					{
						id: 'node2',
						name: 'HTTP Response',
						type: 'Digital Uprisers-nodes-base.httpResponse',
						typeVersion: 1,
						position: [300, 200],
					},
				],
			...
			const event: RelayEventMap['node-post-execute'] = {
				...
				workflow,
				nodeId: 'node2',
				nodeType: 'Digital Uprisers-nodes-base.httpResponse',
			};
	...
	describe('credentials events', () => {
		it('should log on `credentials-shared` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.credentials.shared',
				payload: {
					userId: 'user123',
		...
		it('should log on `credentials-created` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.credentials.created',
				payload: {
					userId: 'user123',
		...
		it('should log on `credentials-deleted` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.credentials.deleted',
				payload: {
					userId: 'user707',
		...
		it('should log on `credentials-updated` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.credentials.updated',
				payload: {
					userId: 'user808',
	...
	describe('auth events', () => {
		it('should log on `user-login-failed` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.login.failed',
				payload: {
					userEmail: 'user@example.com',
					authenticationMethod: 'email',
					reason: 'Invalid password',
	...
	describe('community package events', () => {
		it('should log on `community-package-updated` event', () => {
			const event: RelayEventMap['community-package-updated'] = {
				...
				packageName: 'Digital Uprisers-nodes-awesome-package',
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.package.updated',
				payload: {
					userId: 'user202',
		...
		it('should log on `community-package-installed` event', () => {
			const event: RelayEventMap['community-package-installed'] = {
				...
				inputString: 'Digital Uprisers-nodes-custom-package',
				packageName: 'Digital Uprisers-nodes-custom-package',
				success: true,
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.package.installed',
				payload: {
					userId: 'user789',
		...
		it('should log on `community-package-deleted` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.package.deleted',
				payload: {
					userId: 'user909',
	...
	describe('email events', () => {
		it('should log on `email-failed` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.email.failed',
				payload: {
					userId: 'user789',
	...
	describe('public API events', () => {
		it('should log on `public-api-key-created` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.api.created',
				payload: {
					userId: 'user101',
		...
		it('should log on `public-api-key-deleted` event', () => {
			...
			expect(eventBus.sendAuditEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.audit.user.api.deleted',
				payload: {
					userId: 'user606',
					_email: 'apiuser@example.com',
					_firstName: 'API',
	...
	describe('execution events', () => {
		it('should log on `execution-started-during-bootup` event', () => {
			...
			expect(eventBus.sendExecutionEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.execution.started-during-bootup',
				payload: {
					executionId: 'exec101010',
				},
			});
		...
		it('should log on `execution-throttled` event', () => {
			...
			expect(eventBus.sendExecutionEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.execution.throttled',
				payload: {
					executionId: 'exec123456',
					type: 'production',
				},
	...
	describe('AI events', () => {
		...
		it('should log on `ai-message-added-to-memory` event', () => {
			...
			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.memory.added.message',
				payload,
			});
		});
		...
		it('should log on `ai-output-parsed` event', () => {
			const payload: RelayEventMap['ai-output-parsed'] = {
				...
				workflowId: 'wf456',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.outputParser',
			};

			eventService.emit('ai-output-parsed', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.output.parser.parsed',
				payload,
			});
		});
		...
		it('should log on `ai-documents-retrieved` event', () => {
			const payload: RelayEventMap['ai-documents-retrieved'] = {
				...
				workflowId: 'wf123',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.retriever',
			};

			eventService.emit('ai-documents-retrieved', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.retriever.get.relevant.documents',
				payload,
			});
		});
		...
		it('should log on `ai-document-embedded` event', () => {
			const payload: RelayEventMap['ai-document-embedded'] = {
				...
				workflowId: 'wf789',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.embeddings',
			};

			eventService.emit('ai-document-embedded', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.embeddings.embedded.document',
				payload,
			});
		});
		...
		it('should log on `ai-query-embedded` event', () => {
			...
			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.embeddings.embedded.query',
				payload,
			});
		});
		...
		it('should log on `ai-document-processed` event', () => {
			...
			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.document.processed',
				payload,
			});
		});
		...
		it('should log on `ai-text-split` event', () => {
			const payload: RelayEventMap['ai-text-split'] = {
				...
				workflowId: 'wf789',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.textSplitter',
			};

			eventService.emit('ai-text-split', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.text.splitter.split',
				payload,
			});
		});
		...
		it('should log on `ai-tool-called` event', () => {
			const payload: RelayEventMap['ai-tool-called'] = {
				...
				workflowId: 'wf456',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.tool',
			};

			eventService.emit('ai-tool-called', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.tool.called',
				payload,
			});
		});
		...
		it('should log on `ai-vector-store-searched` event', () => {
			const payload: RelayEventMap['ai-vector-store-searched'] = {
				...
				workflowId: 'wf123',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.vectorStore',
			};

			eventService.emit('ai-vector-store-searched', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.vector.store.searched',
				payload,
			});
		});
		...
		it('should log on `ai-llm-generated-output` event', () => {
			const payload: RelayEventMap['ai-llm-generated-output'] = {
				...
				workflowId: 'wf789',
				workflowName: 'My Workflow',
				nodeType: 'Digital Uprisers-nodes-base.openai',
			};

			eventService.emit('ai-llm-generated-output', payload);

			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.llm.generated',
				payload,
			});
		});
		...
		it('should log on `ai-llm-errored` event', () => {
			...
			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.llm.error',
				payload,
			});
		});
		...
		it('should log on `ai-vector-store-populated` event', () => {
			...
			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.vector.store.populated',
				payload,
			});
		});
		...
		it('should log on `ai-vector-store-updated` event', () => {
			...
			expect(eventBus.sendAiNodeEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.ai.vector.store.updated',
				payload,
			});
		});
	});
	...
	describe('runner events', () => {
		it('should log on `runner-task-requested` event', () => {
			...
			expect(eventBus.sendRunnerEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.runner.task.requested',
				payload: {
					taskId: 't-1',
					nodeId: 'n-2',
					executionId: 'e-3',
		...
		it('should log on `runner-response-received` event', () => {
			...
			expect(eventBus.sendRunnerEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.runner.response.received',
				payload: {
					taskId: 't-1',
					nodeId: 'n-2',
					executionId: 'e-3',
	...
	describe('job events', () => {
		it('should log on `job-enqueued` event', () => {
			...
			expect(eventBus.sendQueueEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.queue.job.enqueued',
				payload: {
					executionId: 'exec-1',
					workflowId: 'wf-2',
					hostId,
		...
		it('should log on `job-dequeued` event', () => {
			...
			expect(eventBus.sendQueueEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.queue.job.dequeued',
				payload: {
					executionId: 'exec-1',
					workflowId: 'wf-2',
					hostId,
		...
		it('should log on `job-stalled` event', () => {
			...
			expect(eventBus.sendQueueEvent).toHaveBeenCalledWith({
				eventName: 'Digital Uprisers.queue.job.stalled',
				payload: {
					executionId: 'exec-1',
					workflowId: 'wf-2',
					hostId,
