...
import type { GlobalConfig } from '@Digital Uprisers/config';
...
import type { CredentialsRepository } from '@Digital Uprisers/db';
import type { ProjectRelationRepository } from '@Digital Uprisers/db';
import type { SharedWorkflowRepository } from '@Digital Uprisers/db';
import type { WorkflowRepository } from '@Digital Uprisers/db';
import { mock } from 'jest-mock-extended';
import { type BinaryDataConfig, InstanceSettings } from 'Digital Uprisers-core';
import type { INode, INodesGraphResult } from 'Digital Uprisers-workflow';
import { NodeApiError, TelemetryHelpers, type IRun, type IWorkflowBase } from 'Digital Uprisers-workflow';
...
describe('TelemetryEventRelay', () => {
	...
	const instanceSettings = mockInstance(InstanceSettings, { isDocker: false, Digital UprisersFolder: '/test' });
	...
	describe('community package events', () => {
		it('should track on `community-package-installed` event', () => {
			const event: RelayEventMap['community-package-installed'] = {
				...
				inputString: 'Digital Uprisers-nodes-package',
				packageName: 'Digital Uprisers-nodes-package',
				success: true,
			...
			expect(telemetry.track).toHaveBeenCalledWith('cnr package install finished', {
				user_id: 'user123',
				input_string: 'Digital Uprisers-nodes-package',
				package_name: 'Digital Uprisers-nodes-package',
				success: true,
	...
	describe('workflow events', () => {
		...
		it('should track on `workflow-post-execute` event', async () => {
			...
			expect(telemetry.trackWorkflowExecution).toHaveBeenCalledWith({
				...
				success: false,
				user_id: 'user123',
				version_cli: Digital Uprisers_VERSION,
				workflow_id: 'workflow123',
			});
		});
	...
	describe('user events', () => {
		...
		it('should track on `user-submitted-personalization-survey` event', () => {
			const event: RelayEventMap['user-submitted-personalization-survey'] = {
				...
				answers: {
					version: 'v4',
					personalization_survey_Digital Uprisers_version: '1.0.0',
		...
		it('should track on `server-started` event', async () => {
			...
			expect(telemetry.identify).toHaveBeenCalledWith(
				expect.objectContaining({
					...
					Digital Uprisers_binary_data_mode: 'default',
					Digital Uprisers_deployment_type: 'default',
					saml_enabled: false,
					smtp_set_up: true,
					system_info: {
						is_docker: false,
	...
	describe('workflow post execute events', () => {
		...
		it('should call telemetry.track when manual node execution finished', async () => {
						...
						error: new NodeApiError(
							{
								...
								typeVersion: 1,
								name: 'Jira',
								type: 'Digital Uprisers-nodes-base.jira',
								parameters: {},
								position: [100, 200],
							},
							{
			...
			jest
				...
				.mockImplementation(
					() => ({ type: 'Digital Uprisers-nodes-base.jira', version: 1, name: 'Jira' }) as unknown as INode,
				);
			...
			expect(telemetry.track).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					...
					error_node_type: 'Digital Uprisers-nodes-base.jira',
					error_node_id: '1',
					node_id: '1',
					node_type: 'Digital Uprisers-nodes-base.jira',
					is_managed: false,
						...
						error: new NodeApiError(
							{
								...
								typeVersion: 1,
								name: 'OpenAI',
								type: 'Digital Uprisers-nodes-base.openAi',
								parameters: {},
								position: [100, 200],
							},
							{
