import type { FrontendSettings, UserUpdateRequestDto } from '@Digital Uprisers/api-types';
import { Logger } from '@Digital Uprisers/backend-common';
import type { ClientOAuth2Options } from '@Digital Uprisers/client-oauth2';
import { GlobalConfig } from '@Digital Uprisers/config';
import type { TagEntity, User, ICredentialsDb, PublicUser } from '@Digital Uprisers/db';
import {
...
import type { IRun, IWorkflowBase, Workflow, WorkflowExecuteMode } from 'Digital Uprisers-workflow';
...
type ExternalHooksMap = {
	'Digital Uprisers.ready': [server: AbstractServer, config: Config];
	'Digital Uprisers.stop': never;
	'worker.ready': never;
