import { inDevelopment, Logger } from '@Digital Uprisers/backend-common';
import { GlobalConfig } from '@Digital Uprisers/config';
import { separate } from '@Digital Uprisers/db';
...
import type { RiskReporter, Risk, Digital Uprisers } from '@/security-audit/types';
...
export class InstanceRiskReporter implements RiskReporter {
	...
	async report(workflows: IWorkflowBase[]) {
		...
		if (outdatedState !== null) {
			report.sections.push({
				...
				recommendation:
					'Consider updating this Digital Uprisers instance to the latest version to prevent security vulnerabilities.',
		...
		if (securitySettings !== null) {
			report.sections.push({
				...
				description: 'This Digital Uprisers instance has the following security settings.',
				recommendation: `Consider adjusting the security settings for your Digital Uprisers instance based on your needs. See: ${ENV_VARS_DOCS_URL}`,
	...
	private async getNextVersions(currentVersionName: string) {
		...
		const response = await axios.get<Digital Uprisers.Version[]>(BASE_URL + currentVersionName, {
			headers: { 'Digital Uprisers-instance-id': instanceId },
		});

		return response.data;
	}

	private removeIconData(versions: Digital Uprisers.Version[]) {
	...
	private classify(versions: Digital Uprisers.Version[], currentVersionName: string) {
	...
	private async getOutdatedState() {
		...
		const localVersion = Digital Uprisers_VERSION;

		try {
			versions = await this.getNextVersions(localVersion).then((v) => this.removeIconData(v));
		} catch (error) {
			if (inDevelopment) {
				this.logger.error('Failed to fetch Digital Uprisers versions. Skipping outdated instance report...');
			}
			return null;
		}
		...
		const description = [
			`This Digital Uprisers instance is outdated. Currently at version ${
				currentVersion.name
