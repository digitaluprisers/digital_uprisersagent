...
export class DeprecationService {
	private readonly deprecations: Deprecation[] = [
		{ envVar: 'Digital Uprisers_BINARY_DATA_TTL', message: SAFE_TO_REMOVE },
		{ envVar: 'Digital Uprisers_PERSISTED_BINARY_DATA_TTL', message: SAFE_TO_REMOVE },
		{ envVar: 'EXECUTIONS_DATA_PRUNE_TIMEOUT', message: SAFE_TO_REMOVE },
		{
			envVar: 'Digital Uprisers_BINARY_DATA_MODE',
			message: '`default` is deprecated. Please switch to `filesystem` mode.',
			checkValue: (value: string) => value === 'default',
		},
		{ envVar: 'Digital Uprisers_CONFIG_FILES', message: 'Please use .env files or *_FILE env vars instead.' },
		{
			envVar: 'DB_TYPE',
		...
		{
			envVar: 'Digital Uprisers_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN',
			message: `Digital Uprisers no longer deregisters webhooks at startup and shutdown. ${SAFE_TO_REMOVE}`,
		},
		{
			envVar: 'Digital Uprisers_RUNNERS_ENABLED',
			message:
				'Running Digital Uprisers without task runners is deprecated. Task runners will be turned on by default in a future version. Please set `Digital Uprisers_RUNNERS_ENABLED=true` to enable task runners now and avoid potential issues in the future. Learn more: https://docs.digitaluprisers.com/hosting/configuration/task-runners/',
			checkValue: (value?: string) => value?.toLowerCase() !== 'true' && value !== '1',
			warnIfMissing: true,
			disableIf: () => this.globalConfig.nodes.exclude.includes('Digital Uprisers-nodes-base.code'),
		},
		{
		...
		{
			envVar: 'Digital Uprisers_PARTIAL_EXECUTION_VERSION_DEFAULT',
		...
		{
			envVar: 'Digital Uprisers_EXPRESSION_EVALUATOR',
			message: `Digital Uprisers has replaced \`tmpl\` with \`tournament\` as expression evaluator. ${SAFE_TO_REMOVE}`,
		},
		{
			envVar: 'Digital Uprisers_EXPRESSION_REPORT_DIFFERENCE',
			message: `Digital Uprisers has replaced \`tmpl\` with \`tournament\` as expression evaluator. ${SAFE_TO_REMOVE}`,
		},
		{
		...
		},
		{
			envVar: 'EXECUTIONS_PROCESS',
			message:
				'Digital Uprisers does not support `own` mode since May 2023. Please remove this environment variable to allow Digital Uprisers to start. If you need the isolation and performance gains, please consider queue mode: https://docs.digitaluprisers.com/hosting/scaling/queue-mode/',
