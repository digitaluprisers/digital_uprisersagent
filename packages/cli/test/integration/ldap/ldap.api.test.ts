import { randomEmail, randomName, uniqueId } from '@Digital Uprisers/backend-test-utils';
import { getPersonalProject } from '@Digital Uprisers/backend-test-utils';
import { testDb } from '@Digital Uprisers/backend-test-utils';
import { LDAP_DEFAULT_CONFIGURATION } from '@Digital Uprisers/constants';
import type { User } from '@Digital Uprisers/db';
import { AuthProviderSyncHistoryRepository } from '@Digital Uprisers/db';
import { UserRepository } from '@Digital Uprisers/db';
import { Container } from '@Digital Uprisers/di';
import { Not } from '@Digital Uprisers/typeorm';
import type { Entry as LdapUser } from 'ldapts';
import { Cipher } from 'Digital Uprisers-core';
...
describe('POST /login', () => {
	const runTest = async (ldapUser: LdapUser) => {
		...
		expect(response.headers['set-cookie'][0]).toContain('Digital Uprisers-auth=');
