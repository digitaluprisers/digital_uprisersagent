...
import type { InstalledNodes } from '@Digital Uprisers/db';
import type { InstalledPackages } from '@Digital Uprisers/db';
...
export const MOCK_09990_Digital Uprisers_VERSION = {
	name: '0.999.0',
	nodes: [
		{
	...
	documentationUrl: 'https://docs.digitaluprisers.com/reference/release-notes/#Digital Uprisers09990',
...
export const MOCK_01110_Digital Uprisers_VERSION = {
	name: '0.111.0',
	nodes: [],
	createdAt: '2022-01-01T00:00:00.000Z',
	description:
		'Includes <strong>new nodes</strong>, <strong>node enhancements</strong>, <strong>core functionality</strong> and <strong>bug fixes</strong>',
	documentationUrl: 'https://docs.digitaluprisers.com/reference/release-notes/#Digital Uprisers01110',
	hasBreakingChange: false,
	hasSecurityFix: false,
	hasSecurityIssue: false,
	securityIssueFixVersion: null,
...
export const MOCK_PACKAGE: InstalledPackages[] = [
	{
		createdAt: new Date(),
		updatedAt: new Date(),
		packageName: 'Digital Uprisers-nodes-test',
...
export function simulateOutdatedInstanceOnce(versionName = MOCK_01110_Digital Uprisers_VERSION.name) {
	const baseUrl = Container.get(GlobalConfig).versionNotifications.endpoint + '/';

	// @ts-expect-error readonly export
	constants.Digital Uprisers_VERSION = versionName;

	nock(baseUrl).get(versionName).reply(200, [MOCK_01110_Digital Uprisers_VERSION, MOCK_09990_Digital Uprisers_VERSION]);
}

export function simulateUpToDateInstance(versionName = MOCK_09990_Digital Uprisers_VERSION.name) {
	const baseUrl = Container.get(GlobalConfig).versionNotifications.endpoint + '/';

	// @ts-expect-error readonly export
	constants.Digital Uprisers_VERSION = versionName;

	nock(baseUrl).persist().get(versionName).reply(200, [MOCK_09990_Digital Uprisers_VERSION]);
}
