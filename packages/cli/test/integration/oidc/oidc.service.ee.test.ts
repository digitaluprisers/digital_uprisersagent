...
import type { OidcConfigDto } from '@Digital Uprisers/api-types';
import { testDb } from '@Digital Uprisers/backend-test-utils';
import { type User, UserRepository } from '@Digital Uprisers/db';
...
describe('OIDC service', () => {
	...
	describe('loadConfig', () => {
		it('should initialize with default config', () => {
			expect(oidcService.getRedactedConfig()).toEqual({
				...
				discoveryEndpoint: 'http://digitaluprisers.com/not-set',
				loginEnabled: false,
			});
		});
		...
		it('should fallback to default configuration', async () => {
			...
			expect(config).toEqual({
				clientId: '',
				clientSecret: '',
				discoveryEndpoint: new URL('http://digitaluprisers.com/not-set'),
				loginEnabled: false,
			});
		});
	...
	it('should generate a valid authentication URL', async () => {
		const mockConfiguration = new real_odic_client.Configuration(
			{
				issuer: 'https://example.com/auth/realms/Digital Uprisers',
				client_id: 'test-client-id',
				redirect_uris: ['http://digitaluprisers.com/sso/oidc/callback'],
				response_types: ['code'],
		...
		it('should handle new user login with valid callback URL', async () => {
			...
			const mockTokens: mocked_oidc_client.TokenEndpointResponse &
				...
				claims: () => {
					return {
						sub: 'mock-subject',
						iss: 'https://example.com/auth/realms/Digital Uprisers',
						aud: 'test-client-id',
