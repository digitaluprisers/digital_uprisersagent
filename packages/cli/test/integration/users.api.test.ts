import { createTeamProject, getPersonalProject, linkUserToProject } from '@Digital Uprisers/backend-test-utils';
import { createWorkflow, getWorkflowById, shareWorkflowWithUsers } from '@Digital Uprisers/backend-test-utils';
...
import { ProjectRelationRepository } from '@Digital Uprisers/db';
...
describe('GET /users', () => {
	...
	beforeAll(async () => {
		...
		owner = await createUser({
			role: 'global:owner',
			email: 'owner@digitaluprisers.com',
			firstName: 'OwnerFirstName',
			lastName: 'OwnerLastName',
		});
		member1 = await createUser({
			role: 'global:member',
			email: 'member1@digitaluprisers.com',
		...
		member2 = await createUser({
			role: 'global:member',
			email: 'member2@digitaluprisers.com',
		...
		describe('inviteAcceptUrl', () => {
			test('should include inviteAcceptUrl for pending users', async () => {
				...
				const pendingUser = await createUser({
					role: 'global:member',
					email: 'pending@digitaluprisers.com',
		...
		describe('sortBy', () => {
			...
			test('should sort by firstName and lastName combined', async () => {
				const user1 = await createUser({
					role: 'global:member',
					email: 'memberz1@digitaluprisers.com',
				...
				const user2 = await createUser({
					role: 'global:member',
					email: 'memberz2@digitaluprisers.com',
					firstName: 'ZZZFirstName',
