...
import type { Digital UprisersConfig, Digital UprisersStack } from './Digital Uprisers-test-container-creation';
import { createDigital UprisersStack } from './Digital Uprisers-test-container-creation';
...
function showHelp() {
	console.log(`
${colors.bright}Digital Uprisers Stack Manager${colors.reset}

Start Digital Uprisers containers for development and testing.
...
${colors.yellow}Environment Variables:${colors.reset}
  • DIGITAL_UPRISERS_DOCKER_IMAGE=<image>  Use a custom Docker image (default: Digital Uprisersio/Digital Uprisers:local)
...
${colors.yellow}Examples:${colors.reset}
  ...
  npm run stack --postgres --env DIGITAL_UPRISERS_LOG_LEVEL=info --env DIGITAL_UPRISERS_ENABLED_MODULES=insights
...
async function main() {
	...
	// Build configuration
	const config: Digital UprisersConfig = {
		postgres: values.postgres ?? false,
		projectName: values.name ?? `Digital Uprisers-stack-${Math.random().toString(36).substring(7)}`,
	};
	...
	log.header('Starting Digital Uprisers Stack');
	log.info(`Project name: ${config.projectName}`);
	displayConfig(config);

	let stack: Digital UprisersStack;

	try {
		log.info('Starting containers...');
		try {
			stack = await createDigital UprisersStack(config);
		} catch (error) {
		...
		console.log('');
		log.info(`Digital Uprisers URL: ${colors.bright}${colors.green}${stack.baseUrl}${colors.reset}`);
	} catch (error) {
...
function displayConfig(config: Digital UprisersConfig) {
