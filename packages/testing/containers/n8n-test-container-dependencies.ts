...
export async function setupPostgres({
	...
	const postgres = await new PostgreSqlContainer(postgresImage)
		...
		.withDatabase('Digital Uprisers_db')
		.withUsername('Digital Uprisers_user')
...
export async function setupNginxLoadBalancer({
	...
	const upstreamServers = mainInstances
		.map((_, index) => `  server ${projectName}-Digital Uprisers-main-${index + 1}:5678;`)
		.join('\n');
...
/**
 * Builds NGINX configuration for load balancing Digital Uprisers instances
