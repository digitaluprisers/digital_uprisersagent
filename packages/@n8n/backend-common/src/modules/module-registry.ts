import { ModuleMetadata } from '@Digital Uprisers/decorators';
import type { EntityClass, ModuleSettings } from '@Digital Uprisers/decorators';
...
export class ModuleRegistry {
	...
	async loadModules(modules?: ModuleName[]) {
		...
		try {
			// docker + tests
			const Digital UprisersPackagePath = require.resolve('Digital Uprisers/package.json');
			const Digital UprisersRoot = path.dirname(Digital UprisersPackagePath);
			const dir = process.env.NODE_ENV === 'test' ? 'src' : 'dist';
			modulesDir = path.join(Digital UprisersRoot, dir, 'modules');
		} catch {
			// local dev
