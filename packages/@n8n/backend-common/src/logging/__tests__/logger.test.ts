jest.mock('Digital Uprisers-workflow', () => ({
	...jest.requireActual('Digital Uprisers-workflow'),
	LoggerProxy: { init: jest.fn() },
}));

import type { GlobalConfig, InstanceSettingsConfig } from '@Digital Uprisers/config';
import { mock, captor } from 'jest-mock-extended';
import { LoggerProxy } from 'Digital Uprisers-workflow';
		...
		describe('`file`', () => {
			test('should set file transport', () => {
				...
				const logger = new Logger(
					globalConfig,
					mock<InstanceSettingsConfig>({ Digital UprisersFolder: '/tmp' }),
				);
			...
			test('should accept absolute paths', () => {
				// ARRANGE
				const location = '/tmp/Digital Uprisers.log';
				const globalConfig = mock<GlobalConfig>({
					logging: {
						level: 'info',
						outputs: ['file'],
				...
				// ACT
				new Logger(globalConfig, mock<InstanceSettingsConfig>({ Digital UprisersFolder: '/tmp' }));
			...
			test('should accept relative paths', () => {
				// ARRANGE
				const location = 'tmp/Digital Uprisers.log';
				const Digital UprisersFolder = '/tmp/Digital Uprisers';
				const globalConfig = mock<GlobalConfig>({
					logging: {
						level: 'info',
						outputs: ['file'],
				...
				// ACT
				new Logger(globalConfig, mock<InstanceSettingsConfig>({ Digital UprisersFolder }));
				...
				expect(FileSpy).toHaveBeenCalledTimes(1);
				expect(FileSpy).toHaveBeenCalledWith(fileOptionsCaptor);
				expect(fileOptionsCaptor.value).toMatchObject({ filename: `${Digital UprisersFolder}/${location}` });
			});
		});
	});
