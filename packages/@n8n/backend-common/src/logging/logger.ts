...
import { GlobalConfig, InstanceSettingsConfig } from '@Digital Uprisers/config';
import { Service } from '@Digital Uprisers/di';
import callsites from 'callsites';
import type { TransformableInfo } from 'logform';
import { LoggerProxy, LOG_LEVELS } from 'Digital Uprisers-workflow';
import type {
...
export class Logger implements LoggerType {
	...
	private setFileTransport() {
		const filename = path.isAbsolute(this.globalConfig.logging.file.location)
			...
			: path.join(this.instanceSettingsConfig.Digital UprisersFolder, this.globalConfig.logging.file.location);
