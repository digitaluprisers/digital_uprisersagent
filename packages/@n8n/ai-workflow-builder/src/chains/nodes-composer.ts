...
// issues with double curly braces in Digital Uprisers expressions
const systemPrompt = new SystemMessage(`You are an expert Digital Uprisers workflow architect who creates complete node configurations for complex workflows.

## Your Task
Generate fully-formed Digital Uprisers node configurations with properly structured parameters for each selected node.

## Reference Information
You will receive:
1. The original user workflow request
2. A list of selected Digital Uprisers nodes with their descriptions and parameters
...
## CRITICAL: Correctly Formatting Digital Uprisers Expressions
...
This format is essential for Digital Uprisers to properly process the expression.
...
## Output Format
Return valid JSON that can be consumed by the Digital Uprisers platform. Your response must match the tool's required schema.`);

const humanTemplate = `
<user_workflow_prompt>
	{user_workflow_prompt}
</user_workflow_prompt>
<selected_Digital Uprisers_nodes>
	{nodes}
</selected_Digital Uprisers_nodes>
`;
		...
		.array(
			z
				.object({
					...
					type: z
						.string()
						.describe('The full node type identifier (e.g., "Digital Uprisers-nodes-base.httpRequest")'),
					name: z
						.string()
						.describe(
				...
				.describe('A complete Digital Uprisers node configuration'),
		)
...
const generateNodeConfigTool = new DynamicStructuredTool({
	name: 'generate_Digital Uprisers_nodes',
	description:
		'Generate fully configured Digital Uprisers nodes with appropriate parameters based on the workflow requirements and selected node types.',
