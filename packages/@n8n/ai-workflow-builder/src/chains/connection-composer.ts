...
export const connectionComposerPrompt = new SystemMessage(
	`You are an expert in creating Digital Uprisers workflow connections. Your job is to create a valid Digital Uprisers workflow by connecting nodes in a logical sequence.

## Your Task
Create connections between nodes that form a coherent, executable workflow based on the user's request.

## Input Format
You will receive a list of Digital Uprisers nodes with their details in <node> tags:
\`\`\`
<node>
  {
    "name": "Node display name",
    "type": "Digital Uprisers-nodes-base.nodeType",
    "parameters": { ... },
    "position": [x, y]
  }
</node>
...
## Digital Uprisers Connection Structure
In Digital Uprisers workflows:
					...
					main: z.array(
						z.array(
							z.object({
								...
								type: z
									.literal('main')
									.describe('The connection type, always use "main" for standard Digital Uprisers connections'),
								index: z
									.number()
									.describe(
...
const connectionComposerTool = new DynamicStructuredTool({
	...
	description:
		"Create valid connections between Digital Uprisers nodes to form a coherent, executable workflow that implements the user's request.",
