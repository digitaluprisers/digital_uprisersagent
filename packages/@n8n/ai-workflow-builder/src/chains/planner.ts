...
export const plannerPrompt = new SystemMessage(
	`You are a Workflow Planner for Digital Uprisers, a platform that helps users automate processes across different services and APIs.

## Your Task
Convert user requests into clear, sequential workflow steps that can be implemented with Digital Uprisers nodes. ONLY include steps that are explicitly stated or directly implied in the user request.
...
4. Create steps that can be mapped to Digital Uprisers nodes later
...
8. Only recommend raw HTTP requests if you think there isn't a suitable Digital Uprisers node
...
const planSchema = z.object({
	steps: z
		...
		.min(1)
		.describe(
			'An ordered list of workflow steps that, when implemented, will fulfill the user request. Each step should be concise, action-oriented, and implementable with Digital Uprisers nodes.',
		),
});
...
const generatePlanTool = new DynamicStructuredTool({
	name: 'generate_plan',
	description:
		'Convert a user workflow request into a logical sequence of clear, achievable steps that can be implemented with Digital Uprisers nodes.',
	schema: planSchema,
	func: async (input) => {
