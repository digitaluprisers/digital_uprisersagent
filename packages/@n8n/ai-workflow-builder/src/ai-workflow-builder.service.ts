...
import { OperationalError, assert, INodeTypes } from 'Digital Uprisers-workflow';
import type { IUser, INodeTypeDescription, INode } from 'Digital Uprisers-workflow';
...
export class AiWorkflowBuilderService {
	...
	constructor(
		...
		private readonly Digital UprisersVersion: string,
	) {
	...
	private async setupModels(user: IUser) {
		...
		if (baseUrl) {
			if (!this.client) {
				...
				this.client = new AiAssistantClient({
					...
					consumerId,
					baseUrl,
					Digital UprisersVersion: this.Digital UprisersVersion,
				});
			}
		...
		this.llmSimpleTask = await gpt41mini({
			apiKey: process.env.DIGITAL_UPRISERS_AI_OPENAI_API_KEY ?? '',
		});
		this.llmComplexTask = await anthropicClaude37Sonnet({
			apiKey: process.env.DIGITAL_UPRISERS_AI_ANTHROPIC_KEY ?? '',
		});
	}
