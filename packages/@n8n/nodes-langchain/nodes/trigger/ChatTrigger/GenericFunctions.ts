...
import type { ICredentialDataDecryptedObject, IWebhookFunctions } from 'Digital Uprisers-workflow';
...
export async function validateAuth(context: IWebhookFunctions) {
	...
	if (authentication === 'none') {
	...
	} else if (authentication === 'Digital UprisersUserAuth') {
		...
		const authCookie = getCookie('Digital Uprisers-auth');
