...
```bash
cd packages/frontend/@Digital Uprisers/chat && pnpm run build 
```
...
```bash
http-server packages/frontend/@Digital Uprisers/chat/dist -g -S -C cert.pem -K key.pem --port 8443 --cors
```
