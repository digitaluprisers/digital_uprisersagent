...
import { NodeOperationError, BINARY_ENCODING, NodeConnectionTypes } from 'Digital Uprisers-workflow';
import type { ZodType } from 'zod';
import { z } from 'zod';

import type { Digital UprisersOutputParser } from '@utils/output_parsers/Digital UprisersOutputParser';
...
function getFakeOutputParser(returnSchema?: ZodType): Digital UprisersOutputParser {
	const fakeOutputParser = mock<Digital UprisersOutputParser>();
...
describe('prepareMessages', () => {
	...
	it('should include system_message with formatting_instructions in prompt templates if provided before version 1.9', async () => {
		...
		const messages = await prepareMessages(mockContext, 0, {
			systemMessage: 'Hello',
			outputParser: mock<Digital UprisersOutputParser>(),
		});
