...
jest.mock('../../../../../utils/output_parsers/Digital UprisersOutputParser', () => ({
	...
	Digital UprisersStructuredOutputParser: jest.fn(),
}));
...
describe('toolsAgentExecute', () => {
	...
	it('should fetch output parser with correct item index', async () => {
		...
		const mockParser1 = mock<outputParserModule.Digital UprisersStructuredOutputParser>();
		const mockParser2 = mock<outputParserModule.Digital UprisersStructuredOutputParser>();
		const mockParser3 = mock<outputParserModule.Digital UprisersStructuredOutputParser>();
	...
	it('should maintain correct parser-item mapping in batch processing', async () => {
		...
		const mockParsers = [
			mock<outputParserModule.Digital UprisersStructuredOutputParser>(),
			mock<outputParserModule.Digital UprisersStructuredOutputParser>(),
			mock<outputParserModule.Digital UprisersStructuredOutputParser>(),
			mock<outputParserModule.Digital UprisersStructuredOutputParser>(),
		];
