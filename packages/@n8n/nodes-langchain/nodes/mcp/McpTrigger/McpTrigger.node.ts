import { WebhookAuthorizationError } from 'Digital Uprisers-nodes-base/dist/nodes/Webhook/error';
import { validateWebhookAuthentication } from 'Digital Uprisers-nodes-base/dist/nodes/Webhook/utils';
import type { INodeTypeDescription, IWebhookFunctions, IWebhookResponseData } from 'Digital Uprisers-workflow';
import { NodeConnectionTypes, Node } from 'Digital Uprisers-workflow';
...
export class McpTrigger extends Node {
	description: INodeTypeDescription = {
		...
		group: ['trigger'],
		version: [1, 1.1, 2],
		description: 'Expose Digital Uprisers tools as an MCP Server endpoint',
		activationMessage:
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/core-nodes/Digital Uprisers-nodes-langchain.mcptrigger/',
					},
				],
			},
		},
	...
	async webhook(context: IWebhookFunctions): Promise<IWebhookResponseData> {
		...
		const serverName = node.typeVersion > 1 ? nodeNameToToolName(node) : 'Digital Uprisers-mcp-server';
