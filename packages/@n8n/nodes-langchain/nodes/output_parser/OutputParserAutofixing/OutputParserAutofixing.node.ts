...
import {
	...
	type Digital UprisersStructuredOutputParser,
...
export class OutputParserAutofixing implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/sub-nodes/Digital Uprisers-nodes-langchain.outputparserautofixing/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		const outputParser = (await this.getInputConnectionData(
			...
			itemIndex,
		)) as Digital UprisersStructuredOutputParser;
		...
		const parser = new Digital UprisersOutputFixingParser(
			this,
			model,
			outputParser,
