...
import {
	...
	type Digital UprisersOutputFixingParser,
} from '@utils/output_parsers/Digital UprisersOutputParser';
		...
		describe('Version 1.1 and below', () => {
          ...
          "required": ["user", "timestamp"]
				...
				const { response } = (await outputParser.supplyData.call(thisArg, 0)) as {
					response: Digital UprisersStructuredOutputParser;
				};
				const outputObject = {
					output: {
						user: {
	...
	describe('Auto-Fix', () => {
		...
		describe('Configuration', () => {
			it('should use default prompt when none specified', async () => {
				...
				const { response } = (await outputParser.supplyData.call(thisArg, 0)) as {
					response: Digital UprisersOutputFixingParser;
				};
		...
		describe('Parsing', () => {
			let mockStructuredOutputParser: MockProxy<Digital UprisersStructuredOutputParser>;

			beforeEach(() => {
				mockStructuredOutputParser = mock<Digital UprisersStructuredOutputParser>();

				jest
					.spyOn(Digital UprisersStructuredOutputParser, 'fromZodJsonSchema')
