...
import { Digital UprisersItemListOutputParser } from '@utils/output_parsers/Digital UprisersItemListOutputParser';
...
describe('OutputParserItemList', () => {
	...
	describe('supplyData', () => {
		it('should create a parser with default options', async () => {
			...
			expect(response).toBeInstanceOf(Digital UprisersItemListOutputParser);
	...
	describe('parse', () => {
		it('should parse a list with default separator', async () => {
			...
			const { response } = await outputParser.supplyData.call(thisArg, 0);
			const result = await (response as Digital UprisersItemListOutputParser).parse('item1\nitem2\nitem3');
		...
		it('should parse a list with custom separator', async () => {
			...
			const { response } = await outputParser.supplyData.call(thisArg, 0);
			const result = await (response as Digital UprisersItemListOutputParser).parse('item1,item2,item3');
			expect(result).toEqual(['item1', 'item2', 'item3']);
		});
		...
		it('should limit the number of items returned', async () => {
			...
			const { response } = await outputParser.supplyData.call(thisArg, 0);
			const result = await (response as Digital UprisersItemListOutputParser).parse(
				'item1\nitem2\nitem3\nitem4',
			);
		...
		it('should throw an error if not enough items are returned', async () => {
			...
			const { response } = await outputParser.supplyData.call(thisArg, 0);
			await expect(
				(response as Digital UprisersItemListOutputParser).parse('item1\nitem2\nitem3'),
