...
import type { IExecuteFunctions, INode } from 'Digital Uprisers-workflow';
import { NodeApiError, NodeConnectionTypes } from 'Digital Uprisers-workflow';

import * as helperModule from '@utils/helpers';
import * as outputParserModule from '@utils/output_parsers/Digital UprisersOutputParser';
...
jest.mock('@utils/output_parsers/Digital UprisersOutputParser', () => ({
	...
	describe('execute', () => {
		...
		it('should unwrap object responses when output parser is provided regardless of version', async () => {
			...
			(outputParserModule.getOptionalOutputParser as jest.Mock).mockResolvedValue(
				mock<outputParserModule.Digital UprisersOutputParser>(),
			);
		...
		it('should pass correct itemIndex to getOptionalOutputParser', async () => {
			...
			const mockParser1 = mock<outputParserModule.Digital UprisersOutputParser>();
			const mockParser2 = mock<outputParserModule.Digital UprisersOutputParser>();
			const mockParser3 = mock<outputParserModule.Digital UprisersOutputParser>();
		...
		it('should handle different output parsers for each item', async () => {
			...
			(outputParserModule.getOptionalOutputParser as jest.Mock)
				...
				.mockResolvedValueOnce(mock<outputParserModule.Digital UprisersOutputParser>());
