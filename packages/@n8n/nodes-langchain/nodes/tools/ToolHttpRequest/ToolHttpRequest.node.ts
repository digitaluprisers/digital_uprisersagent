...
import { Digital UprisersTool } from '@utils/Digital UprisersTool';
...
export class ToolHttpRequest implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/sub-nodes/Digital Uprisers-nodes-langchain.toolhttprequest/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		const requestOptions: IHttpRequestOptions = {
			...
			qs: {},
			headers: {
				// FIXME: This is a workaround to prevent the node from sending a default User-Agent (`Digital Uprisers`) when the header is not set.
		...
		let tool: DynamicTool | Digital UprisersTool;

		// If the node version is 1.1 or higher, we use the Digital UprisersTool wrapper:
		...
		if (this.getNode().typeVersion >= 1.1) {
			...
			tool = new Digital UprisersTool(this, {
				name,
