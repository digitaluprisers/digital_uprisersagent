...
function errorDescriptionMapper(error: NodeError) {
	if (error.description?.includes('properties: should be non-empty for OBJECT type')) {
		return 'Google Gemini requires at least one <a href="https://docs.digitaluprisers.com/advanced-ai/examples/using-the-fromai-function/" target="_blank">dynamic parameter</a> when using tools';
	}
...
export class LmChatGoogleGemini implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/sub-nodes/Digital Uprisers-nodes-langchain.lmchatgooglegemini/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		const model = new ChatGoogleGenerativeAI({
			...
			safetySettings,
			callbacks: [new Digital UprisersLlmTracing(this, { errorDescriptionMapper })],
