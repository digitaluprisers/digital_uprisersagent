...
import { makeResolverFromLegacyOptions } from '@Digital Uprisers/vm2';
import { JavaScriptSandbox } from 'Digital Uprisers-nodes-base/dist/nodes/Code/JavaScriptSandbox';
import { getSandboxContext } from 'Digital Uprisers-nodes-base/dist/nodes/Code/Sandbox';
import { standardizeOutput } from 'Digital Uprisers-nodes-base/dist/nodes/Code/utils';
import { NodeOperationError, NodeConnectionTypes } from 'Digital Uprisers-workflow';
import type {
	IExecuteFunctions,
	INodeExecutionData,
	INodeType,
...
export const vmResolver = makeResolverFromLegacyOptions({
	...
	resolve(moduleName, parentDirname) {
		if (moduleName.match(/^langchain\//) ?? moduleName.match(/^@langchain\//)) {
			return require.resolve(`@Digital Uprisers/Digital Uprisers-nodes-langchain/node_modules/${moduleName}.cjs`, {
				paths: [parentDirname],
			});
		}
		return;
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/root-nodes/Digital Uprisers-nodes-langchain.code/',
					},
				],
			},
		},
