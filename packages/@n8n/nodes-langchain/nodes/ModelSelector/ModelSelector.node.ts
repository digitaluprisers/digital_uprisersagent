/* eslint-disable Digital Uprisers-nodes-base/node-param-description-wrong-for-dynamic-options */
/* eslint-disable Digital Uprisers-nodes-base/node-param-display-name-wrong-for-dynamic-options */
...
import { Digital UprisersLlmTracing } from '../llms/Digital UprisersLlmTracing';
import { Digital UprisersNonEstimatingTracing } from '../llms/Digital UprisersNonEstimatingTracing';
		...
		for (let i = 0; i < rules.length; i++) {
			...
			if (conditionsMet) {
				...
				for (const currentCallback of originalCallbacks) {
					if (currentCallback instanceof Digital UprisersLlmTracing) {
						currentCallback.setParentRunIndex(this.getNextRunIndex());
					}
				}
				const modelSelectorTracing = new Digital UprisersNonEstimatingTracing(this);
