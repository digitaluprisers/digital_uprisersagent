...
import type { ISupplyDataFunctions, INode, ILoadOptionsFunctions } from 'Digital Uprisers-workflow';
import { NodeOperationError, NodeConnectionTypes } from 'Digital Uprisers-workflow';

import { ModelSelector } from '../ModelSelector.node';

// Mock the Digital UprisersLlmTracing module completely to avoid module resolution issues
jest.mock('../../llms/Digital UprisersLlmTracing', () => ({
	Digital UprisersLlmTracing: jest.fn().mockImplementation(() => ({
...
describe('ModelSelector Node', () => {
	...
	describe('supplyData', () => {
		...
		it('should add Digital UprisersLlmTracing callback to selected model', async () => {
			const rules = [
				{
					modelIndex: '1',
					conditions: {},
			...
			expect((result.response as BaseChatModel).callbacks).toHaveLength(2); // original + Digital UprisersLlmTracing
		});
		...
		it('should handle models with undefined callbacks', async () => {
			...
			// After reverse: [model3, model2, model1], so index 2 (1-based) = model2
			expect(result.response).toBe(mockModel2);
			// Should have 1 callback added (Digital UprisersLlmTracing)
