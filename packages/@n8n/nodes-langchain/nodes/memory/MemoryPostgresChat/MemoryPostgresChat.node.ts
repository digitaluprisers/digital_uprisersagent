...
import { configurePostgres } from 'Digital Uprisers-nodes-base/dist/nodes/Postgres/transport/index';
import type { PostgresNodeCredentials } from 'Digital Uprisers-nodes-base/dist/nodes/Postgres/v2/helpers/interfaces';
import { postgresConnectionTest } from 'Digital Uprisers-nodes-base/dist/nodes/Postgres/v2/methods/credentialTest';
import type {
...
export class MemoryPostgresChat implements INodeType {
	description: INodeTypeDescription = {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/sub-nodes/Digital Uprisers-nodes-langchain.memorypostgreschat/',
					},
				],
			},
		},
		...
		properties: [
			...
			sessionKeyProperty,
			{
				...
				name: 'tableName',
				type: 'string',
				default: 'Digital Uprisers_chat_histories',
				description:
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		const tableName = this.getNodeParameter('tableName', itemIndex, 'Digital Uprisers_chat_histories') as string;
