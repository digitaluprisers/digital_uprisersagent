...
export class MemoryMongoDbChat implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/sub-nodes/Digital Uprisers-nodes-langchain.memorymongochat/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		const collectionName = this.getNodeParameter(
			'collectionName',
			itemIndex,
			'Digital Uprisers_chat_histories',
		) as string;
		...
		if (credentials.configurationType === 'connectionString') {
			...
			connectionString = `mongodb://${authString}${host}:${port}/?appname=Digital Uprisers`;
			if (tls) {
