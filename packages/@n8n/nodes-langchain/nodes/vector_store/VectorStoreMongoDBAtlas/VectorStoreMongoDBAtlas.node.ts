...
import { type ILoadOptionsFunctions, NodeOperationError, type INodeProperties } from 'Digital Uprisers-workflow';
...
export async function getMongoClient(context: any) {
	...
	if (!mongoConfig.client || mongoConfig.connectionString !== connectionString) {
		...
		mongoConfig.client = new MongoClient(connectionString, {
			appName: 'devrel.integration.Digital Uprisers_vector_integ',
		});
...
export class VectorStoreMongoDBAtlas extends createVectorStoreNode({
	meta: {
		...
		docsUrl:
			'https://docs.digitaluprisers.com/integrations/builtin/cluster-nodes/root-nodes/Digital Uprisers-nodes-langchain.vectorstoremongodbatlas/',
		credentials: [
			{
				name: 'mongoDb',
				required: true,
