...
export function getConfig(): MemoryVectorStoreConfig {
	...
	if (process.env.DIGITAL_UPRISERS_VECTOR_STORE_MAX_MEMORY) {
		const parsed = parseInt(process.env.DIGITAL_UPRISERS_VECTOR_STORE_MAX_MEMORY, 10);
		if (!isNaN(parsed)) {
			maxMemoryMB = parsed;
		}
	}
	...
	if (process.env.DIGITAL_UPRISERS_VECTOR_STORE_TTL_HOURS) {
		const parsed = parseInt(process.env.DIGITAL_UPRISERS_VECTOR_STORE_TTL_HOURS, 10);
		if (!isNaN(parsed)) {
			ttlHours = parsed;
		}
	}
