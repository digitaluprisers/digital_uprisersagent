...
import type { INode, ISupplyDataFunctions } from 'Digital Uprisers-workflow';
import { z } from 'zod';
...
describe('Test Digital UprisersTool wrapper as DynamicStructuredTool', () => {
	it('should wrap a tool', () => {
		const func = jest.fn();

		const ctx = createMockExecuteFunction<ISupplyDataFunctions>({}, mockNode);

		const tool = new Digital UprisersTool(ctx, {
			name: 'Dummy Tool',
...
describe('Test Digital UprisersTool wrapper - DynamicTool fallback', () => {
