...
import { createMockExecuteFunction } from 'Digital Uprisers-nodes-base/test/nodes/Helpers';
import { NodeOperationError } from 'Digital Uprisers-workflow';
import type { ISupplyDataFunctions, IExecuteFunctions, INode } from 'Digital Uprisers-workflow';
import { z } from 'zod';
...
import { Digital UprisersTool } from '../Digital UprisersTool';
...
describe('getConnectedTools', () => {
	...
	let mockNode: INode;
	let mockDigital UprisersTool: Digital UprisersTool;
	...
	beforeEach(() => {
		...
		mockDigital UprisersTool = new Digital UprisersTool(mockExecuteFunctions as unknown as ISupplyDataFunctions, {
			name: 'Dummy Tool',
	...
	it('should convert Digital UprisersTool to dynamic tool when convertStructuredTool is true', async () => {
		...
		mockDigital UprisersTool.asDynamicTool = asDynamicToolSpy;

		mockExecuteFunctions.getInputConnectionData = jest.fn().mockResolvedValue([mockDigital UprisersTool]);
	...
	it('should not convert Digital UprisersTool when convertStructuredTool is false', async () => {
		mockExecuteFunctions.getInputConnectionData = jest.fn().mockResolvedValue([mockDigital UprisersTool]);

		const tools = await getConnectedTools(mockExecuteFunctions, true, false);
		expect(tools[0]).toBe(mockDigital UprisersTool);
	});
