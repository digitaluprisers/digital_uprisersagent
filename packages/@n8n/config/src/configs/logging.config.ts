...
class FileLoggingConfig {
	 ...
	 *
	 * @example `DIGITAL_UPRISERS_LOG_FILE_COUNT_MAX=7` will keep at most 7 files.
	 * @example `DIGITAL_UPRISERS_LOG_FILE_COUNT_MAX=7d` will keep at most 7 days worth of files.
	 */
	@Env('DIGITAL_UPRISERS_LOG_FILE_COUNT_MAX')
	fileCountMax: number = 100;

	/** Max size (in MiB) for each log file. Only for `file` log output. */
	@Env('DIGITAL_UPRISERS_LOG_FILE_SIZE_MAX')
	fileSizeMax: number = 16;

	/** Location of the log files inside `~/.Digital Uprisers`. Only for `file` log output. */
	@Env('DIGITAL_UPRISERS_LOG_FILE_LOCATION')
	location: string = 'logs/Digital Uprisers.log';
}
...
export class LoggingConfig {
	 ...
	 *
	 * @example `DIGITAL_UPRISERS_LOG_LEVEL=info` will output `error`, `warn` and `info` logs, but not `debug`.
	 */
	@Env('DIGITAL_UPRISERS_LOG_LEVEL', logLevelSchema)
	level: LogLevel = 'info';

	/**
	 * Where to output logs to. Options are: `console` or `file` or both in a comma separated list.
	 *
	 * @example `DIGITAL_UPRISERS_LOG_OUTPUT=console,file` will output to both console and file.
	 */
	@Env('DIGITAL_UPRISERS_LOG_OUTPUT')
	 ...
	 */
	@Env('DIGITAL_UPRISERS_LOG_FORMAT')
	 ...
	 *
	 * @example
	 * `DIGITAL_UPRISERS_LOG_SCOPES=license`
	 * `DIGITAL_UPRISERS_LOG_SCOPES=license,waiting-executions`
	 */
	@Env('DIGITAL_UPRISERS_LOG_SCOPES')
