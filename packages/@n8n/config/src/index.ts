...
export class GlobalConfig {
	...
	/** Path Digital Uprisers is deployed to */
	@Env('DIGITAL_UPRISERS_PATH')
	path: string = '/';

	/** Host name Digital Uprisers can be reached */
	@Env('DIGITAL_UPRISERS_HOST')
	host: string = 'localhost';

	/** HTTP port Digital Uprisers can be reached */
	@Env('DIGITAL_UPRISERS_PORT')
	port: number = 5678;

	/** IP address Digital Uprisers should listen on */
	@Env('DIGITAL_UPRISERS_LISTEN_ADDRESS')
	listen_address: string = '::';

	/** HTTP Protocol via which Digital Uprisers can be reached */
	@Env('DIGITAL_UPRISERS_PROTOCOL', protocolSchema)
	...
	@Env('DIGITAL_UPRISERS_DEFAULT_LOCALE')
	defaultLocale: string = 'en';

	/** Whether to hide the page that shows active workflows and executions count. */
	@Env('DIGITAL_UPRISERS_HIDE_USAGE_PAGE')
	hideUsagePage: boolean = false;

	/** Number of reverse proxies Digital Uprisers is running behind. */
	@Env('DIGITAL_UPRISERS_PROXY_HOPS')
	proxy_hops: number = 0;

	/** SSL key for HTTPS protocol. */
	@Env('DIGITAL_UPRISERS_SSL_KEY')
	ssl_key: string = '';

	/** SSL cert for HTTPS protocol. */
	@Env('DIGITAL_UPRISERS_SSL_CERT')
	ssl_cert: string = '';

	/** Public URL where the editor is accessible. Also used for emails sent from Digital Uprisers. */
	@Env('DIGITAL_UPRISERS_EDITOR_BASE_URL')
