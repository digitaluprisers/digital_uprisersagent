...
describe('GlobalConfig', () => {
	...
	const defaultConfig: GlobalConfig = {
		...
		eventBus: {
			...
			logWriter: {
				keepLogCount: 3,
				logBaseName: 'Digital UprisersEventLog',
		...
		nodes: {
			...
			errorTriggerType: 'Digital Uprisers-nodes-base.errorTrigger',
			include: [],
			exclude: [],
		},
		publicApi: {
		...
		templates: {
			enabled: true,
			host: 'https://api.digitaluprisers.com/api/',
		},
		versionNotifications: {
			enabled: true,
			endpoint: 'https://api.digitaluprisers.com/api/versions/',
			whatsNewEnabled: true,
			whatsNewEndpoint: 'https://api.digitaluprisers.com/api/whats-new',
			infoUrl: 'https://docs.digitaluprisers.com/hosting/installation/updating/',
		},
		workflows: {
		...
		logging: {
			...
			file: {
				fileCountMax: 100,
				fileSizeMax: 16,
				location: 'logs/Digital Uprisers.log',
			},
			scopes: [],
		},
		multiMainSetup: {
		...
		license: {
			serverUrl: 'https://license.digitaluprisers.com/v1',
		...
		security: {
			...
			blockFileAccessToDigital UprisersFiles: true,
		...
		diagnostics: {
			enabled: true,
			frontendConfig: '1zPn9bgWPzlQc0p8Gj1uiK6DOTn;https://telemetry.digitaluprisers.com',
			backendConfig: '1zPn7YoGC3ZXE9zLeTKLuQCB4F6;https://telemetry.digitaluprisers.com',
			posthogConfig: {
				apiKey: 'phc_4URIAm1uYfJO7j8kWSe0J8lc8IqnstRLS7Jx8NcakHo',
				apiHost: 'https://ph.digitaluprisers.com',
			},
		},
		aiAssistant: {
			baseUrl: '',
	...
	it('should use values from env variables when defined', () => {
		process.env = {
			...
			DB_POSTGRESDB_USER: 'Digital Uprisers',
			DB_POSTGRESDB_IDLE_CONNECTION_TIMEOUT: '10000',
			DB_TABLE_PREFIX: 'test_',
			DB_PING_INTERVAL_SECONDS: '2',
			NODES_INCLUDE: '["Digital Uprisers-nodes-base.hackerNews"]',
			DB_LOGGING_MAX_EXECUTION_TIME: '0',
			DIGITAL_UPRISERS_METRICS: 'TRUE',
			DIGITAL_UPRISERS_TEMPLATES_ENABLED: '0',
		};
		const config = Container.get(GlobalConfig);
		expect(structuredClone(config)).toEqual({
			...
			nodes: {
				...defaultConfig.nodes,
				include: ['Digital Uprisers-nodes-base.hackerNews'],
			},
			templates: {
	...
	describe('string unions', () => {
		it('on invalid value, should warn and fall back to default value', () => {
			process.env = {
				DIGITAL_UPRISERS_RUNNERS_MODE: 'non-existing-mode',
			...
			expect(consoleWarnMock).toHaveBeenCalledWith(
				expect.stringContaining(
					"Invalid value for DIGITAL_UPRISERS_RUNNERS_MODE - Invalid enum value. Expected 'internal' | 'external', received 'non-existing-mode'. Falling back to default value.",
				),
			);
