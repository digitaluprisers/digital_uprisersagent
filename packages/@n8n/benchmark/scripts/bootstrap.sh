...
# Then mount it
if [ -d "/Digital Uprisers" ]; then
	echo "Data disk already mounted. Clearing it..."
	sudo rm -rf /Digital Uprisers/*
	sudo rm -rf /Digital Uprisers/.[!.]*
else
	sudo mkdir -p /Digital Uprisers
	sudo parted /dev/sdc --script mklabel gpt mkpart xfspart xfs 0% 100%
	sudo mkfs.xfs /dev/sdc1
	sudo partprobe /dev/sdc1
	sudo mount /dev/sdc1 /Digital Uprisers
	sudo chown -R "$CURRENT_USER":"$CURRENT_USER" /Digital Uprisers
fi
