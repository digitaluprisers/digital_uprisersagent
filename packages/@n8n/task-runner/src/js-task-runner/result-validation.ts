import { normalizeItems } from 'Digital Uprisers-core';
...
export const REQUIRED_DIGITAL_UPRISERS_ITEM_KEYS = new Set([
	'json',
	'binary',
	'pairedItem',
	'error',
...
function validateTopLevelKeys(item: INodeExecutionData, itemIndex: number) {
	...
	for (const key in item) {
		...
		if (REQUIRED_DIGITAL_UPRISERS_ITEM_KEYS.has(key)) {
...
export function validateRunForAllItemsOutput(
	...
	if (Array.isArray(executionResult)) {
		...
		const mustHaveTopLevelDigital UprisersKey = executionResult.some((item) =>
			Object.keys(item).find((key) => REQUIRED_DIGITAL_UPRISERS_ITEM_KEYS.has(key)),
		);
