import { isSerializedBuffer, toBuffer } from 'Digital Uprisers-core';
import { ApplicationError, ensureError, randomInt } from 'Digital Uprisers-workflow';
...
export abstract class TaskRunner extends EventEmitter {
	...
	constructor(opts: TaskRunnerOpts) {
		...
		this.ws.addEventListener('error', (event) => {
			...
			if (
				...
				console.error(
					`Error: Failed to connect to Digital Uprisers task broker. Please ensure Digital Uprisers task broker is reachable at: ${taskBrokerHost}`,
				);
				process.exit(1);
			} else {
				console.error(`Error: Failed to connect to Digital Uprisers task broker at ${taskBrokerHost}`);
	...
	private receiveMessage = (message: MessageEvent) => {
		// eslint-disable-next-line Digital Uprisers-local-rules/no-uncaught-json-parse
