...
describe('generateNodesGraph', () => {
	test('should return node graph when node type is unknown', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['Digital Uprisers-nodes-base.manualTrigger', 'test.unknown'],
	...
	test('should return node graph when node has multiple operation fields with different display options', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['Digital Uprisers-nodes-base.manualTrigger', 'test.googleSheets'],
				node_connections: [{ start: '0', end: '1' }],
				nodes: {
					'0': {
						id: 'fa7d5628-5a47-4c8f-98ef-fb3532e5a9f5',
	...
	test('should return node graph indicating pinned data', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			...
			nodeGraph: {
				is_pinned: true,
				node_connections: [],
				node_types: ['Digital Uprisers-nodes-base.manualTrigger'],
				nodes: {
					'0': {
						id: 'e59d3ad9-3448-4899-9f47-d2922c8727ce',
						position: [460, 460],
	...
	test('should return graph with webhook node', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			evaluationTriggerNodeNames: [],
			nodeGraph: {
				node_types: ['Digital Uprisers-nodes-base.webhook'],
				node_connections: [],
				nodes: {
					'0': {
						id: '5e49e129-2c59-4650-95ea-14d4b94db1f3',
	...
	test('should return graph with http v4 node with generic auth', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			evaluationTriggerNodeNames: [],
			nodeGraph: {
				node_types: ['Digital Uprisers-nodes-base.httpRequest'],
				node_connections: [],
				nodes: {
					'0': {
						id: '04d6e44f-09c1-454d-9225-60aeed7f022c',
	...
	it.each([
		{
			...
			expected: {
				nodeGraph: {
					node_types: ['Digital Uprisers-nodes-base.merge'],
					node_connections: [],
					nodes: {
						'0': {
							id: 'b468b603-3e59-4515-b555-90cfebd64d47',
		...
		const workflow: Partial<IWorkflowBase> = {
			nodes: [
				...
				},
				{
					...
					name: 'Chain',
					type: '@Digital Uprisers/Digital Uprisers-nodes-langchain.chainLlm',
					typeVersion: 1,
					position: [760, 320],
				},
				{
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: [
					...
					'@Digital Uprisers/Digital Uprisers-nodes-langchain.chainLlm',
					'@Digital Uprisers/Digital Uprisers-nodes-langchain.lmChatOpenAi',
				],
				node_connections: [
					{
						start: '0',
	...
	test('should add run and items count', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes, { runData })).toEqual({
			...
			nodeGraph: {
				...
				node_types: [
					'Digital Uprisers-nodes-base.executeWorkflowTrigger',
					'Digital Uprisers-nodes-base.set',
					'Digital Uprisers-nodes-base.set',
					'Digital Uprisers-nodes-base.set',
					'Digital Uprisers-nodes-base.debugHelper',
...
describe('makeAIMetrics', () => {
	...
	it('should count applicable nodes and parameters', async () => {
		const nodes = [
			makeNode(
				{
					sendTo: "={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('To', ``, 'string') }}",
					sendTwo: "={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('To', ``, 'string') }}",
					subject: "={{ $fromAI('Subject', ``, 'string') }}",
				},
				'Digital Uprisers-nodes-base.gmailTool',
			),
			makeNode(
				{
					subject: "={{ $fromAI('Subject', ``, 'string') }}",
	...
	it('should not count non-applicable nodes and parameters', async () => {
		const nodes = [
			makeNode(
				...
				'Digital Uprisers-nodes-base.gmail',
			),
		];
	...
	it('should return empty object if no vector store nodes are present', () => {
		const nodes = [
			makeNode(
				...
				'Digital Uprisers-nodes-base.nonVectorStoreNode',
			),
		];
	...
	it('should detect vector store nodes that inserted data', () => {
		const nodes = [
			makeNode(
				...
				'Digital Uprisers-nodes-base.vectorStoreNode',
			),
		];
...
describe('extractLastExecutedNodeStructuredOutputErrorInfo', () => {
	...
	const mockLanguageModelNode = (name = 'Model', model = 'gpt-4'): INode => ({
		id: 'model-node-id',
		name,
		type: 'Digital Uprisers-nodes-langchain.lmChatOpenAi',
		typeVersion: 1,
		position: [200, 200],
		parameters: {
			model,
	...
	const mockToolNode = (name: string): INode => ({
		id: `tool-${name}`,
		name,
		type: 'Digital Uprisers-nodes-base.httpRequestTool',
		typeVersion: 1,
		position: [300, 300],
		parameters: {},
	});
	...
	it('should extract model name from modelName parameter when model parameter is not present', () => {
		...
		const modelNode: INode = {
			...
			type: 'Digital Uprisers-nodes-langchain.lmChatGoogleGemini',
			typeVersion: 1,
			position: [200, 200],
			parameters: {
