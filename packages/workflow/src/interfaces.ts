...
import type { LogScope } from '@Digital Uprisers/config';
...
export namespace DeclarativeRestApiSettings {
	...
	export type ResultOptions = {
		...
		requestOperations?: IDigital UprisersRequestOperations;
	};
}
...
export type IExecuteResponsePromiseData = IDataObject | IDigital UprisersHttpFullResponse;
...
export type IDigital UprisersHttpResponse = IDataObject | Buffer | GenericValue | GenericValue[] | null;

export interface IDigital UprisersHttpFullResponse {
	body: IDigital UprisersHttpResponse | Readable;
...
export interface IDigital UprisersRequestOperations {
	pagination?:
		| IDigital UprisersRequestOperationPaginationGeneric
		| IDigital UprisersRequestOperationPaginationOffset
		| ((
...
export interface IDigital UprisersRequestOperationPaginationBase {
	type: string;
	properties: {
		[key: string]: unknown;
	};
...
export interface IDigital UprisersRequestOperationPaginationGeneric extends IDigital UprisersRequestOperationPaginationBase {
	type: 'generic';
	properties: {
		continue: boolean | string;
		request: IRequestOptionsSimplifiedAuth;
...
export interface IDigital UprisersRequestOperationPaginationOffset extends IDigital UprisersRequestOperationPaginationBase {
	type: 'offset';
	properties: {
...
namespace ExecuteFunctions {
	...
	export type GetNodeParameterFn = {
		// @TECH_DEBT: Refactor to remove this barely used overload - Digital Uprisers-5632
...
export interface ILoadOptions {
	routing?: {
		operations?: IDigital UprisersRequestOperations;
...
export interface INodePropertyRouting {
	operations?: IDigital UprisersRequestOperations; // Should be changed, does not sound right
	output?: INodeRequestOutput;
	request?: DeclarativeRestApiSettings.HttpRequestOptions;
...
export type NodeDefaults = Partial<{
	/**
	 * @deprecated Use {@link INodeTypeBaseDescription.iconColor|iconColor} instead. `iconColor` supports dark mode and uses preset colors from Digital Uprisers's design system.
	 */
	color: string;
	name: string;
}>;
...
export interface INodeGraphItem {
	...
	agent?: string; //@Digital Uprisers/Digital Uprisers-nodes-langchain.agent
	...
	workflow_id?: string; //@Digital Uprisers/Digital Uprisers-nodes-langchain.toolWorkflow and Digital Uprisers-nodes-base.executeWorkflow
	runs?: number;
	items_total?: number;
	metric_names?: string[];
}
...
export type Digital UprisersAIProviderType = 'openai' | 'unknown';
...
export type IPersonalizationSurveyAnswersV4 = {
	...
	personalization_survey_Digital Uprisers_version: string;
