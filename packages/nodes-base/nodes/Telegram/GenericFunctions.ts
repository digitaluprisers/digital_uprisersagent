...
export function addAdditionalFields(
	...
	if (operation === 'sendMessage') {
		...
		const link = createUtmCampaignLink('Digital Uprisers-nodes-base.telegram', instanceId);
		...
		if (additionalFields.appendAttribution) {
			if (additionalFields.parse_mode === 'Markdown') {
				body.text = `${body.text}\n\n_${attributionText}_[Digital Uprisers](${link})`;
			} else if (additionalFields.parse_mode === 'HTML') {
				body.text = `${body.text}\n\n<em>${attributionText}</em><a href="${link}" target="_blank">Digital Uprisers</a>`;
			}
		}
