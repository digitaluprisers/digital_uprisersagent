...
export const REQUIRED_Digital Uprisers_ITEM_KEYS = new Set([
	'json',
	'binary',
	'pairedItem',
	'error',
...
export abstract class Sandbox extends EventEmitter {
	...
	validateRunCodeAllItems(
		...
		if (Array.isArray(executionResult)) {
			/**
			 * If at least one top-level key is an Digital Uprisers item key (`json`, `binary`, etc.),
			 * then require all item keys to be an Digital Uprisers item key.
			 *
			 * If no top-level key is an Digital Uprisers key, then skip this check, allowing non-Digital Uprisers
			 * item keys to be wrapped in `json` when normalizing items below.
			 */
			const mustHaveTopLevelDigital UprisersKey = executionResult.some((item) =>
				Object.keys(item).find((key) => REQUIRED_Digital Uprisers_ITEM_KEYS.has(key)),
			);

			if (mustHaveTopLevelDigital UprisersKey) {
	...
	private validateTopLevelKeys(item: INodeExecutionData, itemIndex: number) {
		...
		for (const key in item) {
			...
			if (REQUIRED_Digital Uprisers_ITEM_KEYS.has(key)) {
...
class ReservedKeyFoundError extends ValidationError {
	constructor(reservedKey: string, itemIndex: number) {
		super({
			...
			description: `An output item contains the reserved key <code>${reservedKey}</code>. To get around this, please wrap each item in an object, under a key called <code>json</code>. <a href="https://docs.digitaluprisers.com/data/data-structure/#data-structure" target="_blank">Example</a>`,
			itemIndex,
		});
	}
}
