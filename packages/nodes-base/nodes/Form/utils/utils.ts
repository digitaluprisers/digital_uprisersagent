...
export function prepareFormData({
	...
	const Digital UprisersWebsiteLink = `https://digitaluprisers.com/?utm_source=Digital Uprisers-internal&utm_medium=form-trigger${utm_campaign}`;
...
export const validateResponseModeConfiguration = (context: IWebhookFunctions) => {
	...
	const isRespondToWebhookConnected = connectedNodes.some(
		(node) => node.type === 'Digital Uprisers-nodes-base.respondToWebhook',
	);
	...
	if (isRespondToWebhookConnected && nodeVersion > 2.1) {
		throw new NodeOperationError(
			...
			{
				description:
					'To configure your response, add an "Digital Uprisers Form" node and set the "Page Type" to "Form Ending"',
			},
		);
	}
};
