...
export const formFieldsProperties: INodeProperties[] = [
	...
	},
	{
		...
		hint: '<a href="https://docs.digitaluprisers.com/integrations/builtin/core-nodes/Digital Uprisers-nodes-base.form/" target="_blank">See docs</a> for field syntax',
		displayOptions: {
			show: {
				defineForm: ['json'],
			},
	...
	[
		{
			...
			displayName: 'On Digital Uprisers Form Submission',
			name: 'respondWith',
			type: 'options',
			default: 'text',
			options: [
...
export class Form extends Node {
	...
	description: INodeTypeDescription = {
		displayName: 'Digital Uprisers Form',
		name: 'form',
		icon: 'file:form.svg',
		group: ['input'],
		version: 1,
		...
		properties: [
			{
				displayName: 'An Digital Uprisers Form Trigger node must be set up before this node',
				name: 'triggerNotice',
				type: 'notice',
				default: '',
			},
	...
	async webhook(context: IWebhookFunctions): Promise<IWebhookResponseData> {
		...
		const trigger = parentNodes.find(
			(node) => node.type === 'Digital Uprisers-nodes-base.formTrigger',
