...
export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	if (config.appendAttribution !== false) {
		...
		htmlBody = createEmailBodyWithDigital UprisersAttribution(config.message, buttons.join('\n'), instanceId);
	} else {
		htmlBody = createEmailBodyWithoutDigital UprisersAttribution(config.message, buttons.join('\n'));
	}
