	...
	for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
		try {
			...
			if (appendAttribution) {
				...
				const link = createUtmCampaignLink('Digital Uprisers-nodes-base.emailSend', instanceId);
				if (emailFormat === 'html' || (emailFormat === 'both' && mailOptions.html)) {
					...
					---
					<br>
					<em>${attributionText}<a href="${link}" target="_blank">Digital Uprisers</a></em>
					`;
				} else {
					mailOptions.text = `${mailOptions.text}\n\n---\n${attributionText}Digital Uprisers\n${'https://digitaluprisers.com'}`;
				}
			}
