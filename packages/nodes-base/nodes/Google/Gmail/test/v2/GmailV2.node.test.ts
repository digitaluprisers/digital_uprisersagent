/* eslint-disable Digital Uprisers-nodes-base/node-param-display-name-miscased */
import { NodeTestHarness } from '@nodes-testing/node-test-harness';
import { mock, mockDeep } from 'jest-mock-extended';
import { jsonParse, type ILoadOptionsFunctions, type INode } from 'Digital Uprisers-workflow';
...
describe('Test Gmail Node v2', () => {
	...
	describe('Messages', () => {
		...
		beforeAll(() => {
			...
			gmailNock.get('/v1/users/me/profile').times(2).reply(200, { emailAddress: 'test@digitaluprisers.com' });
			gmailNock
	...
	describe('Threads', () => {
		...
		beforeAll(() => {
			...
			gmailNock
				...
				.reply(200, messages[0]);
			gmailNock.get('/v1/users/me/profile').reply(200, { emailAddress: 'test@digitaluprisers.com' });
			gmailNock
				.post('/v1/users/me/messages/send')
		...
		describe('getGmailAliases', () => {
			it('should return a list of Gmail aliases', async () => {
				const loadOptionsFunctions = mockDeep<ILoadOptionsFunctions>({
					...
					helpers: mock<ILoadOptionsFunctions['helpers']>({
						requestWithAuthentication: jest.fn(async () => ({
							sendAs: [
								{ isDefault: false, sendAsEmail: 'alias1@digitaluprisers.com' },
								{ isDefault: true, sendAsEmail: 'alias2@digitaluprisers.com' },
							],
						})),
					}),
				});
				...
				expect(await getGmailAliases.call(loadOptionsFunctions)).toEqual([
					{
						name: 'alias1@digitaluprisers.com',
						value: 'alias1@digitaluprisers.com',
					},
					{
						name: 'alias2@digitaluprisers.com (Default)',
						value: 'alias2@digitaluprisers.com',
					},
				]);
			});
		});
