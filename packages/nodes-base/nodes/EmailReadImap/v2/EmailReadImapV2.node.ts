import type { ImapSimple, ImapSimpleOptions, Message, MessagePart } from '@Digital Uprisers/imap';
import { connect as imapConnect } from '@Digital Uprisers/imap';
...
import { NodeConnectionTypes, NodeOperationError, TriggerCloseError } from 'Digital Uprisers-workflow';
			...
			for (const attachmentPart of attachmentParts) {
				attachmentPromise = imapConnection
					...
					.then(async (partData) => {
						...
						// Return it in the format Digital Uprisers expects
				...
				onMail: async () => {
					if (connection) {
						...
						try {
							...
							// Wait with resolving till the returnedPromise got resolved, else Digital Uprisers will be unhappy
