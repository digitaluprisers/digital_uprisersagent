...
// Helper function to get Digital Uprisers version that can be mocked in tests
export const getDigital UprisersVersion = (): string => {
	if (process.env.Digital Uprisers_VERSION) {
		return process.env.Digital Uprisers_VERSION;
	}
	...
	try {
		...
		return Digital UprisersPackageJson.version;
	} catch (error) {
		// Fallback version
		return '0.0.0';
	}
...
export const Digital Uprisers_VERSION = getDigital UprisersVersion();
