...
export function prepareNames(id: string, mode: string, additionalFields: IDataObject) {
	...
	let functionName =
		(additionalFields.functionName as string) || `Digital Uprisers_trigger_function_${suffix}()`;

	if (!(functionName.includes('(') && functionName.includes(')'))) {
		functionName = `${functionName}()`;
	}

	const triggerName = (additionalFields.triggerName as string) || `Digital Uprisers_trigger_${suffix}`;
	const channelName = (additionalFields.channelName as string) || `Digital Uprisers_channel_${suffix}`;
