import type { INodeProperties, IExecuteFunctions } from 'Digital Uprisers-workflow';
...
export async function execute(this: IExecuteFunctions, i: number, instanceId: string) {
	...
	if (config.appendAttribution !== false) {
		...
		const link = createUtmCampaignLink('Digital Uprisers-nodes-base.microsoftTeams', instanceId);
		const attribution = `<em>${attributionText} <a href="${link}">Digital Uprisers</a></em>`;
