...
describe('MailerLite', () => {
	describe('Run v2 workflow', () => {
		beforeAll(() => {
			...
			mock.get('/subscribers/user@digitaluprisers.com').reply(200, getSubscriberResponseV2);
			mock
				.get('/subscribers')
				.query({ 'filter[status]': 'junk', limit: 2 })
				.reply(200, getAllSubscribersResponseV2);
			mock.put('/subscribers/user@digitaluprisers.com').reply(200, getUpdateSubscriberResponseV2);
		});
