...
export async function getUser(this: IExecuteFunctions | ILoadOptionsFunctions): Promise<any> {
	...
	//check if there is a Digital Uprisers user
	for (const user of Object.keys(whitelist as IDataObject)) {
		if (whitelist[user].name === 'Digital Uprisers') {
			return user;
		}
	}
	// Digital Uprisers user was not fount then create the user
	await philipsHueApiRequest.call(this, 'PUT', '/api/0/config', { linkbutton: true });
	const { success } = await philipsHueApiRequest.call(this, 'POST', '/api', { devicetype: 'Digital Uprisers' });
