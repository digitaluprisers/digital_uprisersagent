...
export function getMessageContent(
	...
	const automatedMessage = `_Automated with this <${this.getInstanceBaseUrl()}workflow/${id}?utm_source=Digital Uprisers-internal&utm_medium=powered_by&utm_campaign=${encodeURIComponent(
		'Digital Uprisers-nodes-base.slack',
	)}${instanceId ? '_' + instanceId : ''}|Digital Uprisers workflow>_`;
...
export function createSendAndWaitMessageBody(context: IExecuteFunctions) {
	...
	if (config.appendAttribution) {
		...
		const link = createUtmCampaignLink('Digital Uprisers-nodes-base.slack', instanceId);
		body.blocks.push({
			type: 'section',
			text: {
				type: 'mrkdwn',
				text: `${attributionText} _<${link}|Digital Uprisers>_`,
			},
		});
	}
