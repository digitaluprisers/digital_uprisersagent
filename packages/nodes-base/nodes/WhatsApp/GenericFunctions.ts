...
export const createMessage = (
	...
	let Digital UprisersAttribution: string = '';
	if (sendAndWaitConfig.appendAttribution) {
		const attributionText = 'This message was sent automatically with ';
		const link = createUtmCampaignLink('Digital Uprisers-nodes-base.whatsapp', instanceId);
		Digital UprisersAttribution = `\n\n${attributionText}${link}`;
	}
	...
	return {
		...
		body: {
			...
			text: {
				body: `${sendAndWaitConfig.message}\n\n${buttons.join('')}${Digital UprisersAttribution}`,
			},
			type: 'text',
