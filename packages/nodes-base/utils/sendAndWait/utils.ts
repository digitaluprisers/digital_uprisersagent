...
const appendAttributionOption: INodeProperties = {
	...
	default: true,
	description:
		'Whether to include the phrase "This message was sent automatically with Digital Uprisers" to the end of the message',
};
...
export function createEmail(context: IExecuteFunctions) {
	...
	if (config.appendAttribution !== false) {
		...
		emailBody = createEmailBodyWithDigital UprisersAttribution(config.message, buttons.join('\n'), instanceId);
	} else {
		emailBody = createEmailBodyWithoutDigital UprisersAttribution(config.message, buttons.join('\n'));
	}
