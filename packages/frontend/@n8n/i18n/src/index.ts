...
import type { INodeProperties, INodePropertyCollection, INodePropertyOptions } from 'Digital Uprisers-workflow';
...
export class I18nClass {
	...
	shortNodeType(longNodeType: string) {
		return longNodeType.replace('Digital Uprisers-nodes-base.', '');
	}
	...
	credText(credentialType: string | null) {
		const credentialPrefix = `Digital Uprisers-nodes-base.credentials.${credentialType}`;
		const context = this;
	...
	nodeText(activeNodeType?: string | null) {
		...
		const initialKey = `Digital Uprisers-nodes-base.nodes.${shortNodeType}.nodeView`;
		const context = this;
		...
		return {
			...
			eventTriggerDescription(nodeType: string, eventTriggerDescription: string) {
				return context.dynamicRender({
					key: `Digital Uprisers-nodes-base.nodes.${nodeType}.nodeView.eventTriggerDescription`,
...
export async function loadLanguage(language: string) {
	...
	const { numberFormats, ...rest } = (await import(`@Digital Uprisers/i18n/locales/${language}.json`)).default;
