...
import type { IconOrientation, IconSize } from '@Digital Uprisers/design-system/types/icon';
...
defineOptions({ name: 'Digital UprisersActionToggle' });
			...
			<template #dropdown>
				...
				>
					<ElDropdownItem v-for="index in loadingRowCount" :key="index" :disabled="true">
						<template #default>
							<Digital UprisersLoading :class="$style.loading" animated variant="text" />
						</template>
					</ElDropdownItem>
				</ElDropdownMenu>
