...
import type { IUser, UserAction } from '@Digital Uprisers/design-system/types';

import Digital UprisersActionToggle from '../Digital UprisersActionToggle';
import Digital UprisersLink from '../Digital UprisersLink';
import Digital UprisersLoading from '../Digital UprisersLoading';
...
defineOptions({ name: 'Digital UprisersBreadcrumbs' });
...
<template>
	<div
		:class="{
			...
			['Digital Uprisers-breadcrumbs']: true,
		}"
	>
		...
		<ul :class="$style.list">
			...
			>
				...
				<div v-if="props.theme !== 'small'" :class="$style['hidden-items-menu']">
					...
					>
						<Digital UprisersText :bold="true" :class="$style.dots">...</Digital UprisersText>
					</Digital UprisersActionToggle>
				</div>
					...
					<template #content>
						...
						<div v-else :class="$style.tooltipContent">
							<div data-test-id="hidden-items-tooltip">
								<Digital UprisersText>{{ loadedHiddenItems.map((item) => item.label).join(' / ') }}</Digital UprisersText>
							</div>
						</div>
					</template>
			...
			<template v-for="(item, index) in items" :key="item.id">
				...
				>
					<Digital UprisersLink v-if="item.href" :href="item.href" theme="text">{{ item.label }}</Digital UprisersLink>
					<Digital UprisersText v-else>{{ item.label }}</Digital UprisersText>
				</li>
...
.tooltip-loading {
	...
	:global(.Digital Uprisers-loading) > div {
		display: flex;
