...
// Simple mock for the Digital UprisersActionToggle component to test hidden items and events
const Digital UprisersActionToggleMock = defineComponent({
	name: 'Digital UprisersActionToggle',
	props: {
		actions: {
...
describe('Breadcrumbs', async () => {
	...
	it('renders hidden items correctly', async () => {
		...
		const { container, getByTestId, queryByTestId } = render(Breadcrumbs, {
			...
			global: {
				stubs: {
					'Digital Uprisers-action-toggle': Digital UprisersActionToggleMock,
				},
				plugins: [router],
			},
		});
