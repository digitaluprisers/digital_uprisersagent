...
import Digital UprisersNodeCreatorNode from './NodeCreatorNode.vue';

export default {
	title: 'Modules/Node Creator Node',
	component: Digital UprisersNodeCreatorNode,
};
...
const DefaultTemplate: StoryFn = (args, { argTypes }) => ({
	...
	components: {
		Digital UprisersNodeCreatorNode,
	},
	template: `
		<Digital Uprisers-node-creator-node v-bind="args">
			<template #icon>
...
const PanelTemplate: StoryFn = (args, { argTypes }) => ({
	...
	template: `
		<Digital Uprisers-node-creator-node v-bind="args" :isPanelActive="isPanelActive" @click.capture="isPanelActive = true">
			<template #icon>
				<img src="https://dev.w3.org/SVG/tools/svgweb/samples/svg-files/cartman.svg" />
			</template>
			<template #panel>
