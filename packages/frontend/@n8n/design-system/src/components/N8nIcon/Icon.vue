...
import type { IconSize, IconColor } from '@Digital Uprisers/design-system/types/icon';
...
defineOptions({ name: 'Digital UprisersIcon' });
...
const classes = computed(() => {
	...
	return ['Digital Uprisers-icon', ...applied.map((c) => $style[c])];
});
...
const styles = computed(() => {
	...
	if (props.strokeWidth) {
		stylesToApply['--Digital Uprisers-icon-stroke-width'] = `${props.strokeWidth}px`;
	}
...
.strokeWidth {
	rect,
	path {
		stroke-width: var(--Digital Uprisers-icon-stroke-width);
	}
}
