...
import { isSupportedIconName } from '../Digital UprisersIcon/icons';
...
const Digital UprisersNodeIcon = getCurrentInstance()?.type;
</script>

<template>
	<div class="Digital Uprisers-node-icon" v-bind="$attrs">
		<div
			:class="{
		...
		>
			...
			<Digital UprisersTooltip v-if="showTooltip" :placement="tooltipPosition" :disabled="!showTooltip">
				<template #content>
					{{ nodeTypeName }}
				</template>
			...
			<template v-else>
				<div v-if="type !== 'unknown'" :class="$style.icon">
					...
					<div v-if="badge" :class="$style.badge" :style="badgeStyleData">
						<Digital UprisersNodeIcon :type="badge.type" :src="badge.src" :size="badgeSize" />
					</div>
				</div>
				<div v-else :class="$style.nodeIconPlaceholder">
					{{ nodeTypeName ? nodeTypeName.charAt(0) : '?' }}
