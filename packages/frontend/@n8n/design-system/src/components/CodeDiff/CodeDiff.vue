...
import { useI18n } from '@Digital Uprisers/design-system/composables/useI18n';
...
<template>
	<div :class="$style.container" data-test-id="code-diff-suggestion">
		...
		<div :class="$style.actions">
			<div v-if="error">
				<Digital UprisersIcon icon="triangle-alert" color="danger" class="mr-5xs" />
			...
			<div v-else-if="replaced">
				...
				<Digital UprisersIcon icon="check" color="success" class="ml-xs" />
			...
			<Digital UprisersButton
				...
				:loading="replacing"
				@click="() => emit('replace')"
				>{{ replacing ? t('codeDiff.replacing') : t('codeDiff.replaceMyCode') }}</Digital UprisersButton
			>
		</div>
	</div>
</template>
