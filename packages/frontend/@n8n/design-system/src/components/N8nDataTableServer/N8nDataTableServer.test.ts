...
import Digital UprisersDataTableServer, { type TableHeader } from './Digital UprisersDataTableServer.vue';

const renderComponent = createComponentRenderer(Digital UprisersDataTableServer);
...
describe('Digital UprisersDataTableServer', () => {
	it('should render a table', () => {
		const { container } = render(Digital UprisersDataTableServer, {
	...
	it('should synchronize the state', async () => {
		const { container, rerender } = render(Digital UprisersDataTableServer, {
			//@ts-expect-error testing-library errors due to header generics
			props: { items, headers, itemsLength: items.length },
		});
	...
	it('should not show the pagination if there are no items', async () => {
		const { queryByTestId } = render(Digital UprisersDataTableServer, {
			//@ts-expect-error testing-library errors due to header generics
