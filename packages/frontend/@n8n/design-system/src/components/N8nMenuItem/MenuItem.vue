...
const Digital UprisersMenuItem = getCurrentInstance()?.type;
...
<template>
	<div :class="['Digital Uprisers-menu-item', $style.item]">
		<ElSubMenu
		...
		>
			<ConditionalRouterLink v-bind="item.route ?? item.link">
				...
				>
					...
					<Digital UprisersSpinner v-if="item.isLoading" :class="$style.loading" size="small" />
				</ElMenuItem>
