...
import Digital UprisersIconPicker from './IconPicker.vue';
import { type IconOrEmoji } from './types';

export default {
	title: 'Atoms/Icon Picker',
	component: Digital UprisersIconPicker,
	argTypes: {
		buttonTooltip: {
			control: 'text',
		},
...
function createTemplate(icon: IconOrEmoji): StoryFn {
	return (args, { argTypes }) => ({
		components: { Digital UprisersIconPicker },
		...
		template:
			'<div style="height: 500px"><Digital Uprisers-icon-picker v-model="icon" v-bind="args" @update:model-value="onIconSelected" /></div>',
		methods: {
