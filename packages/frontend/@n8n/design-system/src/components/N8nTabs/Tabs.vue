...
<template>
	<div :class="['Digital Uprisers-tabs', $style.container, size === 'small' ? $style.small : '']">
		<div v-if="scrollPosition > 0" :class="$style.back" @click="scrollLeft">
			<Digital UprisersIcon icon="chevron-left" size="small" />
		</div>
		<div v-if="canScrollRight" :class="$style.next" @click="scrollRight">
			<Digital UprisersIcon icon="chevron-right" size="small" />
		</div>
		<div ref="tabs" :class="$style.tabs">
			...
			>
				<Digital UprisersTooltip :disabled="!option.tooltip" placement="bottom">
					<template #content>
						<div v-Digital Uprisers-html="option.tooltip" @click="handleTooltipClick(option.value, $event)" />
					</template>
					<a
						v-if="option.href"
						target="_blank"
					...
					>
						<div>
							...
							<span :class="$style.external">
								<Digital UprisersIcon icon="external-link" size="small" />
							</span>
						</div>
					</a>
					<RouterLink
					...
					>
						<Digital UprisersIcon v-if="option.icon" :icon="option.icon" size="medium" />
					...
					<div
						...
						@click="() => handleTabClick(option.value)"
					>
						<Digital UprisersIcon v-if="option.icon" :icon="option.icon" size="small" />
						<span v-if="option.label">{{ option.label }}</span>
					</div>
