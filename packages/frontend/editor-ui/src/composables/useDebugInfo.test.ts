...
import type { RootStoreState } from '@Digital Uprisers/stores/useRootStore';
...
const MOCK_BASE_SETTINGS: RecursivePartial<ReturnType<typeof useSettingsStoreType>> = {
	...
	security: {
		blockFileAccessToDigital UprisersFiles: false,
		secureCookie: false,
	},
};
...
vi.mock('@Digital Uprisers/composables/useDeviceSupport', () => ({
	useDeviceSupport: () => ({
		isTouchDevice: false,
...
describe('useDebugInfo', () => {
	...
	it('should include security info if insecure settings are found', () => {
		...
		expect(debugInfo).toContain('blockFileAccessToDigital UprisersFiles: false');
	...
	it('should not include security info if all settings are secure', () => {
		...
		expect(debugInfo).not.toContain('blockFileAccessToDigital UprisersFiles');
