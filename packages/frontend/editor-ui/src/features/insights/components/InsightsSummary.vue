...
import type { InsightsDateRange, InsightsSummary } from '@Digital Uprisers/api-types';
...
<template>
	<div :class="$style.insights">
		<ul data-test-id="insights-summary-tabs">
			...
			>
				<Digital UprisersTooltip placement="top" :disabled="!(summaryHasNoData && id === 'total')">
					<template #content>
					...
					<router-link :to="to" :exact-active-class="$style.activeTab" @click="trackTabClick(id)">
						<strong>
							<Digital UprisersTooltip placement="bottom" :disabled="id !== 'timeSaved'">
								<template #content>
						...
						<span v-if="value === 0 && id === 'timeSaved'" :class="$style.empty">
							...
							<small>
								<Digital UprisersTooltip placement="bottom">
									...
									<Digital UprisersIcon :class="$style.icon" icon="info" size="medium" />
								</Digital UprisersTooltip>
							</small>
						</span>
						<span v-else>
							...
							<small v-if="deviation !== null" :class="getImpactStyle(id, deviation)">
								<Digital UprisersIcon
									...
									"
								/>
								<Digital UprisersTooltip placement="bottom" :disabled="id !== 'failureRate'">
									<template #content>
