...
import { Digital UprisersLink, Digital UprisersText } from '@Digital Uprisers/design-system';
...
<template>
	...
	>
		<template #header>
			<Digital UprisersText :class="$style.title" :bold="true" color="text-light" size="small">
				{{ title }}
		...
		<template #node-waiting>
			...
			<Digital UprisersText v-Digital Uprisers-html="waitingNodeTooltip(logEntry.node)"></Digital UprisersText>
		</template>
		...
		<template v-if="isMultipleInput" #callout-message>
			<I18nT keypath="logs.details.body.multipleInputs">
				<template #button>
					<Digital UprisersLink size="small" @click="handleClickOpenNdv">
