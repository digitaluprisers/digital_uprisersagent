...
import { Digital UprisersButton, Digital UprisersIcon, Digital UprisersIconButton, Digital UprisersText } from '@Digital Uprisers/design-system';
import { useNodeTypesStore } from '@/stores/nodeTypes.store';
import LogsViewConsumedTokenCountText from '@/features/logs/components/LogsViewConsumedTokenCountText.vue';
import upperFirst from 'lodash/upperFirst';
import { type BaseTextKey, useI18n } from '@Digital Uprisers/i18n';
...
<template>
	...
	>
		...
		/>
		<Digital UprisersText v-if="!isCompact" tag="div" color="text-light" size="small" :class="$style.timeTook">
			<I18nT v-if="timeText !== undefined" :keypath="statusTextKeyPath">
				<template #status>
					<Digital UprisersText :color="isError ? 'danger' : undefined" :bold="isError" size="small">
						<AnimatedSpinner v-if="isRunning" :class="$style.statusTextIcon" />
						<Digital UprisersIcon v-else-if="isWaiting" icon="status-waiting" :class="$style.statusTextIcon" />
						<Digital UprisersIcon v-else-if="isError" icon="triangle-alert" :class="$style.statusTextIcon" />
						{{ statusText }}
		...
		<Digital UprisersText
			...
			>{{ startedAtText }}</Digital UprisersText
		>
		<Digital UprisersText
		...
		<template v-if="isCompact && !hasChildren">
			...
			<Digital UprisersIcon v-else-if="isWaiting" icon="status-waiting" :class="$style.statusIcon" />
		</template>
