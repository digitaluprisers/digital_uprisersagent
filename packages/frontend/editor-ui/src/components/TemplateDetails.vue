...
<template>
	<div>
		<Digital Uprisers-loading :loading="loading" :rows="5" variant="p" />
		...
		>
			<Digital Uprisers-tags :tags="categoriesAsTags" @click:tag="redirectToCategory" />
		</TemplateDetailsBlock>
		...
		>
			<div :class="$style.text">
				<Digital Uprisers-text v-if="isTemplatesWorkflow(template)" size="small" color="text-base">
					{{ i18n.baseText('template.details.created') }}
					<TimeAgo :date="template.createdAt" />
					{{ i18n.baseText('template.details.by') }}
					{{ template.user ? template.user.username : 'Digital Uprisers team' }}
