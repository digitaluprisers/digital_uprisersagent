...
const onDownloadClick = () => {
	const filename = 'Digital Uprisers-recovery-codes.txt';
		...
		<template #content>
			<div v-if="!showRecoveryCodes" :class="[$style.container, $style.modalContent]">
				<div :class="$style.textContainer">
					<Digital Uprisers-text size="large" color="text-dark" :bold="true">{{
						i18n.baseText('mfa.setup.step1.instruction1.title')
					}}</Digital Uprisers-text>
				</div>
				<div>
					<Digital Uprisers-text size="medium" :bold="false">
			...
			<div v-else :class="$style.container">
				...
				<div :class="$style.recoveryCodesContainer">
					<div v-for="recoveryCode in recoveryCodes" :key="recoveryCode">
						<Digital Uprisers-text size="medium">{{ recoveryCode }}</Digital Uprisers-text>
					</div>
				</div>
				<Digital Uprisers-info-tip :bold="false" :class="$style['edit-mode-footer-infotip']">
					<i18n-t keypath="mfa.setup.step2.infobox.description" tag="span">
						<template #part1>
							{{ i18n.baseText('mfa.setup.step2.infobox.description.part1') }}
						</template>
						<template #part2>
							<Digital Uprisers-text size="small" :bold="true" :class="$style.loseAccessText">
