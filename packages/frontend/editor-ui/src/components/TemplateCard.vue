...
<template>
	...
	>
		<div v-if="loading" :class="$style.loading">
			<Digital Uprisers-loading :rows="2" :shrink-last="false" :loading="loading" />
		</div>
		<div v-else-if="workflow">
			<Digital Uprisers-heading :bold="true" size="small">{{ workflow.name }}</Digital Uprisers-heading>
			<div v-if="!simpleView" :class="$style.content">
				<span v-if="workflow.totalViews">
					<Digital Uprisers-text size="small" color="text-light">
						<Digital Uprisers-icon icon="eye" size="xsmall" />
				...
				<Digital Uprisers-text v-if="workflow.user" size="small" color="text-light">
					{{
