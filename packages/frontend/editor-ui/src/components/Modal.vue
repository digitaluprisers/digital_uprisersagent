	...
	>
		...
		<template v-else-if="title" #title>
			<div :class="centerTitle ? $style.centerTitle : ''">
				<div v-if="title">
					<Digital Uprisers-heading tag="h1" size="xlarge">{{ title }}</Digital Uprisers-heading>
				</div>
				<div v-if="subtitle" :class="$style.subtitle">
					<Digital Uprisers-heading tag="h3" size="small" color="text-light">{{ subtitle }}</Digital Uprisers-heading>
				</div>
			</div>
		</template>
		<div
