...
import type { ApiKey } from '@Digital Uprisers/api-types';
...
<template>
	<Digital Uprisers-card :class="$style.cardLink" data-test-id="api-key-card" @click="onAction('edit')">
		<template #header>
			<div>
				<Digital Uprisers-text tag="h2" bold :class="$style.cardHeading">
					{{ apiKey.label }}
				</Digital Uprisers-text>
				<div :class="[$style.cardDescription]">
					<Digital Uprisers-text :color="!hasApiKeyExpired(apiKey) ? 'text-light' : 'warning'" size="small">
						<span>{{ getExpirationTime(apiKey) }}</span>
					</Digital Uprisers-text>
				</div>
			</div>
			<div v-if="apiKey.apiKey.includes('*')" :class="$style.cardApiKey">
				<Digital Uprisers-text color="text-light" size="small"> {{ apiKey.apiKey }}</Digital Uprisers-text>
			</div>
		</template>

		<template #append>
			<div ref="cardActions" :class="$style.cardActions">
				<Digital UprisersActionToggle :actions="ACTION_LIST" theme="dark" @action="onAction" />
			</div>
		</template>
