...
import type { FilterOperatorType } from 'Digital Uprisers-workflow';
...
<template>
	...
	>
		<template v-if="selectedGroupIcon" #prefix>
			<Digital Uprisers-icon :class="$style.icon" :icon="selectedGroupIcon" color="text-light" size="small" />
		</template>
					...
					<template #reference>
						...
						>
							<div :class="$style.groupTitle">
								<Digital Uprisers-icon v-if="group.icon" :icon="group.icon" :class="$style.icon" size="small" />
								<span>{{ i18n.baseText(group.name) }}</span>
							</div>
							<Digital Uprisers-icon icon="chevron-right" color="text-light" size="xsmall" />
						</div>
					</template>
					<div>
