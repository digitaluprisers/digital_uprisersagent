...
import type { MessageEventBusDestinationOptions } from 'Digital Uprisers-workflow';
import { deepCopy, defaultMessageEventBusDestinationOptions } from 'Digital Uprisers-workflow';
...
import { assert } from '@Digital Uprisers/utils/assert';
...
<template>
	<Digital Uprisers-card :class="$style.cardLink" data-test-id="destination-card" @click="onClick">
		<template #header>
			<div>
				<Digital Uprisers-heading tag="h2" bold :class="$style.cardHeading">
					{{ destination.label }}
				</Digital Uprisers-heading>
				<div :class="$style.cardDescription">
					<Digital Uprisers-text color="text-light" size="small">
		...
		<template #append>
			<div ref="cardActions" :class="$style.cardActions">
				<div :class="$style.activeStatusText" data-test-id="destination-activator-status">
					<Digital Uprisers-text v-if="nodeParameters.enabled" :color="'success'" size="small" bold>
						{{ i18n.baseText('workflowActivator.active') }}
					</Digital Uprisers-text>
					<Digital Uprisers-text v-else color="text-base" size="small" bold>
				...
				<Digital Uprisers-action-toggle :actions="actions" theme="dark" @action="onAction" />
			</div>
		</template>
