...
import type { IUser, PublicInstalledPackage } from 'Digital Uprisers-workflow';
...
<template>
	<div :class="$style.cardContainer" data-test-id="community-package-card">
		<div v-if="loading" :class="$style.cardSkeleton">
			<Digital Uprisers-loading :class="$style.loader" variant="p" :rows="1" />
			<Digital Uprisers-loading :class="$style.loader" variant="p" :rows="1" />
		</div>
		<div v-else-if="communityPackage" :class="$style.packageCard">
			<div :class="$style.cardInfoContainer">
				<div :class="$style.cardTitle">
					<Digital Uprisers-text :bold="true" size="large">{{ communityPackage.packageName }}</Digital Uprisers-text>
				</div>
				<div :class="$style.cardSubtitle">
					<Digital Uprisers-text :bold="true" size="small" color="text-light">
						{{
					...
					<Digital Uprisers-text size="small" color="text-light">
			...
			<div :class="$style.cardControlsContainer">
				<Digital Uprisers-text :bold="true" size="large" color="text-light">
					v{{ communityPackage.installedVersion }}
				</Digital Uprisers-text>
				<Digital Uprisers-tooltip v-if="communityPackage.failedLoading === true" placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.failedToLoad.tooltip') }}
						</div>
					</template>
					<Digital Uprisers-icon icon="triangle-alert" color="danger" size="large" />
				...
				>
					...
					<Digital Uprisers-button outline label="Update" @click="onUpdateClick" />
				</Digital Uprisers-tooltip>
				<Digital Uprisers-tooltip v-else placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.upToDate.tooltip') }}
						</div>
					</template>
					<Digital Uprisers-icon icon="circle-check" color="text-light" size="large" />
				</Digital Uprisers-tooltip>
				<div :class="$style.cardActions">
					<Digital Uprisers-action-toggle :actions="packageActions" @action="onAction"></Digital Uprisers-action-toggle>
				</div>
			</div>
		</div>
	</div>
