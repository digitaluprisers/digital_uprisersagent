...
import type { ICredentialType, INodeTypeDescription } from 'Digital Uprisers-workflow';
...
describe('CredentialIcon', () => {
	const renderComponent = createComponentRenderer(CredentialIcon, {
		...
		global: {
			stubs: ['Digital Uprisers-tooltip'],
		},
	});
	...
	it('shows correct icon when iconUrl is set on credential', () => {
		const testIconUrl = 'icons/Digital Uprisers-nodes-base/dist/nodes/Test/test.svg';
	...
	it('shows correct icon when credential has an icon with node: prefix', () => {
		...
		useCredentialsStore().setCredentialTypes([
			mock<ICredentialType>({
				name: 'test',
				icon: 'node:Digital Uprisers-nodes-base.test',
				iconColor: 'azure',
			}),
		]);
		...
		useNodeTypesStore().setNodeTypes([
			mock<INodeTypeDescription>({
				version: 1,
				name: 'Digital Uprisers-nodes-base.test',
				iconUrl: testIconUrl,
			}),
		]);
