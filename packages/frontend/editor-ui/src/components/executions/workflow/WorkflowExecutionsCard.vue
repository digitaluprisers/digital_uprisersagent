	...
	>
		...
		>
			<div :class="$style.description">
				...
				<Digital UprisersText v-else color="text-dark" :bold="true" size="medium" data-test-id="execution-time">
				...
				<div :class="$style.executionStatus">
					...
					/>
					<Digital UprisersText :class="$style.statusLabel" size="small">{{ executionUIDetails.label }}</Digital UprisersText>
					{{ ' ' }}
					<Digital UprisersText
						v-if="executionUIDetails.name === 'running'"
				...
				<div v-if="execution.mode === 'retry'">
					<Digital UprisersText :color="isActive ? 'text-dark' : 'text-base'" size="small">
				...
				<div v-if="isAnnotationEnabled" :class="$style.annotation">
					<div v-if="execution.annotation?.vote" :class="$style.ratingIcon">
						<Digital UprisersIcon v-if="execution.annotation.vote == 'up'" :class="$style.up" icon="thumbs-up" />
						<Digital UprisersIcon v-else :class="$style.down" icon="thumbs-down" />
					</div>
			...
			<div :class="$style.icons">
				...
				/>
				<Digital UprisersTooltip v-if="execution.mode === 'manual'" placement="top">
					<template #content>
						<span>{{ locale.baseText('executionsList.test') }}</span>
					</template>
					<Digital UprisersIcon :class="[$style.icon, $style.manual]" icon="flask-conical" />
				</Digital UprisersTooltip>
				<Digital UprisersTooltip v-if="execution.mode === 'evaluation'" placement="top">
					<template #content>
						<span>{{ locale.baseText('executionsList.evaluation') }}</span>
					</template>
					<Digital UprisersIcon :class="[$style.icon, $style.evaluation]" icon="check-check" />
				</Digital UprisersTooltip>
			</div>
		</router-link>
	</div>
