...
import type { ExecutionStatus, ExecutionSummary } from 'Digital Uprisers-workflow';
import { WAIT_INDEFINITELY } from 'Digital Uprisers-workflow';
...
<template>
	<tr :class="classes">
		...
		<td>
			<Digital UprisersTooltip :content="execution.workflowName || workflowName" placement="top">
				<RouterLink
					:to="{
		...
		<td data-test-id="execution-status">
			...
			>
				<div>
					<Digital UprisersIcon :icon="statusRender.icon" :color="statusRender.color" class="mr-2xs" />
					{{ statusRender.label }}
				</div>
			</GlobalExecutionsListItemQueuedTooltip>
			<Digital UprisersTooltip
		...
		<td>
			<Digital UprisersIcon v-if="execution.mode === 'manual'" icon="flask-conical" />
		</td>
		<td>
			<Digital UprisersButton
		...
		<td>
			<ElDropdown v-if="!isRunning" trigger="click" @command="handleActionItemClick">
				<Digital UprisersIconButton text type="tertiary" icon="ellipsis-vertical" />
				<template #dropdown>
					<ElDropdownMenu
						:class="{
							[$style.actions]: true,
