...
import { ROLE, type UsersList } from '@Digital Uprisers/api-types';
...
const { emitters, addEmitter } = useEmitters<
	'settingsUsersRoleCell' | 'settingsUsersActionsCell' | 'Digital UprisersDataTableServer'
>();
...
// Mock Digital UprisersDataTableServer to emit events
vi.mock('@Digital Uprisers/design-system', async (importOriginal) => {
	const original = await importOriginal<object>();
	return {
		...original,
		Digital UprisersDataTableServer: defineComponent({
			props: {
			...
			setup(_, { emit }) {
				addEmitter('Digital UprisersDataTableServer', emit);
			},
			template: `
				<ul>
...
describe('SettingsUsersTable', () => {
	...
	it('should delegate update:options event from Digital UprisersDataTableServer', () => {
		const { emitted } = renderComponent();
		emitters.Digital UprisersDataTableServer.emit('update:options', { page: 1, itemsPerPage: 20 });
