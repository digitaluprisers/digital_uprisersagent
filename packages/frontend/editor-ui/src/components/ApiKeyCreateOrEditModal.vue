...
import type { ApiKey, ApiKeyWithRawValue, CreateApiKeyRequestDto } from '@Digital Uprisers/api-types';
	...
	>
		<template #content>
			<div @keyup.enter="handleEnterKey">
				<Digital Uprisers-card v-if="newApiKey" class="mb-4xs">
					<CopyInput
				...
				<div v-else :class="$style.form">
					...
					<div v-if="mode === 'new'" :class="$style.expirationSection">
						...
						<Digital UprisersText v-if="expirationDate" class="mb-xs">{{
		...
		<template #footer>
			<div :class="$style.footer">
				...
				<Digital UprisersButton
					...
					@click="onEdit"
				/>
				<Digital UprisersText v-if="mode === 'edit'" size="small" color="text-light">{{
					apiKeyCreationDate
				}}</Digital UprisersText>
			</div>
		</template>
