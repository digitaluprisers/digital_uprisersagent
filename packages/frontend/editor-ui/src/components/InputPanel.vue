...
import { Digital UprisersIcon, Digital UprisersRadioButtons, Digital UprisersText, Digital UprisersTooltip } from '@Digital Uprisers/design-system';
import {
...
<template>
	...
	>
		...
		<template #node-not-run>
			...
			>
				<template v-if="isNDVV2">
					...
					>
						<template #icon>
							<Digital UprisersIcon icon="arrow-right-to-line" size="xlarge" />
						</template>
				...
				<template v-else>
					<template v-if="isMappingEnabled || hasRootNodeRun">
						<Digital UprisersText tag="div" :bold="true" color="text-dark" size="large">{{
					...
					<template v-else>
						...
						<Digital UprisersText tag="div" color="text-dark" size="medium">
							<i18n-t tag="span" keypath="ndv.input.rootNodeHasNotRun.description">
								<template #link>
									<a
										href="#"
					...
					<Digital UprisersTooltip v-if="!readOnly" :visible="showDraggableHint && showDraggableHintWithDelay">
						<template #content>
							<div
					...
					<Digital UprisersText v-if="!readOnly" tag="div" size="small">
						<i18n-t keypath="ndv.input.noOutputData.hint">
							<template #info>
								<Digital UprisersTooltip placement="bottom">
									...
									<Digital UprisersIcon icon="circle-help" />
								</Digital UprisersTooltip>
							</template>
						</i18n-t>
					</Digital UprisersText>
				...
				<NDVEmptyState v-if="isNDVV2" :title="i18n.baseText('ndv.input.notConnected.v2.title')">
					...
					<template #description>
						<i18n-t tag="span" keypath="ndv.input.notConnected.v2.description">
							<template #link>
								<a
									href="https://docs.digitaluprisers.com/workflows/connections/"
									target="_blank"
		...
		<template #node-waiting>
			<Digital UprisersText :bold="true" color="text-dark" size="large">
				{{ i18n.baseText('ndv.output.waitNodeWaiting.title') }}
			</Digital UprisersText>
			<Digital UprisersText v-Digital Uprisers-html="waitingMessage"></Digital UprisersText>
		</template>
