...
<template>
	<div :class="[$style.tip, { [$style.drag]: tip === 'drag' }]">
		<Digital Uprisers-text size="small" :class="$style.tipText"
			>{{ i18n.baseText('parameterInput.tip') }}:
		</Digital Uprisers-text>

		<div v-if="tip === 'drag'" :class="$style.content">
			<Digital Uprisers-text size="small" :class="$style.text">
		...
		<div v-else-if="tip === 'dotPrimitive'" :class="$style.content">
			<span v-Digital Uprisers-html="i18n.baseText('expressionTip.typeDotPrimitive')" />
		</div>

		<div v-else-if="tip === 'dotObject'" :class="$style.content">
			<span v-Digital Uprisers-html="i18n.baseText('expressionTip.typeDotObject')" />
		</div>

		<div v-else :class="$style.content">
			<span v-Digital Uprisers-html="i18n.baseText('expressionTip.javascript')" />
		</div>
	</div>
</template>
