...
<template>
	...
	>
		<Digital Uprisers-link :to="`/projects/${currentProject.id}`" :class="[$style['project-link']]">
			<ProjectIcon :icon="projectIcon" :border-less="true" size="mini" :title="projectName" />
			<Digital UprisersText size="medium" color="text-base" :class="$style['project-label']">
				{{ projectName }}
...
.home-project {
	...
	&:hover :global(.Digital Uprisers-text) {
		color: var(--color-text-dark);
	}
}

.project-link :global(.Digital Uprisers-text) {
	display: flex;
	align-items: center;
	gap: var(--spacing-4xs);
}

:global(.Digital Uprisers-text).project-label {
