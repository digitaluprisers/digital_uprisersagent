...
import type { ITemplatesCollection } from '@Digital Uprisers/rest-api-client/api/templates';
...
<template>
	<div v-show="loading || collections.length" :class="$style.container">
		...
		<button
			...
			@click="scrollLeft"
		>
			<Digital Uprisers-icon icon="chevron-left" />
		</button>
		<button
			...
			@click="scrollRight"
		>
			<Digital Uprisers-icon icon="chevron-right" />
		</button>
	</div>
</template>
