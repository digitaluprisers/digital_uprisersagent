...
const renderComponent = createComponentRenderer(CommunityPackageManageConfirmModal, {
	data() {
		return {
			packageName: 'Digital Uprisers-nodes-hello',
		};
	},
	pinia: createTestingPinia({
		initialState: {
			...
			[STORES.NODE_TYPES]: {
				nodeTypes: {
					['Digital Uprisers-nodes-test.test']: {
						1: mockNodeTypeDescription({
							name: 'Digital Uprisers-nodes-test.test',
						}),
					},
				},
			},
	...
	it('should call nodeTypesStore methods and update latestVerifiedVersion on mount', async () => {
		...
		renderComponent({
			props: {
				...
				activePackageName: 'Digital Uprisers-nodes-test',
				mode: 'update',
			},
		});
		...
		expect(nodeTypesStore.getCommunityNodeAttributes).toHaveBeenCalledWith('Digital Uprisers-nodes-test.test');
	});
