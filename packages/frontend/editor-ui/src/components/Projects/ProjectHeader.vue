...
import { Digital UprisersButton, Digital UprisersTooltip } from '@Digital Uprisers/design-system';
...
import { type IconOrEmoji, isIconOrEmoji } from '@Digital Uprisers/design-system/components/Digital UprisersIconPicker/types';
...
const projectDescriptionTruncated = computed(() => {
	...
	// Fallback to Digital UprisersText component default font-size, small
		...
		<div ref="projectHeaderRef" :class="$style.projectHeader">
			<div :class="$style.projectDetails">
				...
				<div :class="$style.headerActions">
					<Digital UprisersHeading v-if="projectName" bold tag="h2" size="xlarge" data-test-id="project-name">{{
						projectName
					}}</Digital UprisersHeading>
					<Digital UprisersText v-if="sectionDescription" color="text-light" data-test-id="project-subtitle">
						{{ sectionDescription }}
					</Digital UprisersText>
					<template v-else-if="projectDescription">
						<div :class="$style.projectDescriptionWrapper">
							<Digital UprisersText color="text-light" data-test-id="project-subtitle">
								{{ projectDescriptionTruncated || projectDescription }}
							</Digital UprisersText>
							<div v-if="projectDescriptionTruncated" :class="$style.tooltip">
								<Digital UprisersText color="text-light">{{ projectDescription }}</Digital UprisersText>
							</div>
						</div>
					</template>
				</div>
