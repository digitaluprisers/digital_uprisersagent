...
<template>
	<div :class="{ [$style.wrapper]: true, [$style['no-border']]: showBadgeBorder }" v-bind="$attrs">
		<Digital UprisersTooltip :disabled="!badgeTooltip || numberOfMembersInHomeTeamProject !== 0" placement="top">
			...
			>
				...
				<router-link v-if="projectLocation" :to="projectLocation">
					<span v-Digital Uprisers-truncate:20="badgeText" />
				</router-link>
				<span v-else v-Digital Uprisers-truncate:20="badgeText" />
			</Digital UprisersBadge>
			<template #content>
				{{ badgeTooltip }}
			</template>
