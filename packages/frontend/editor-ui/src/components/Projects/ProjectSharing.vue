...
<template>
	<div>
		...
		>
			<template #prefix>
				<Digital UprisersIcon v-if="projectIcon.type === 'icon'" :icon="projectIcon.value" color="text-dark" />
				<Digital UprisersText v-else-if="projectIcon.type === 'emoji'" color="text-light" :class="$style.emoji">
					{{ projectIcon.value }}
		...
		<ul v-if="Array.isArray(model)" :class="$style.selectedProjects">
			<li v-if="props.homeProject" :class="$style.project" data-test-id="project-sharing-owner">
				<ProjectSharingInfo :project="props.homeProject">
					<Digital UprisersBadge theme="tertiary" bold>
						{{ locale.baseText('auth.roles.owner') }}
					</Digital UprisersBadge></ProjectSharingInfo
				>
			</li>
			<li
			...
			>
				...
				>
					<Digital UprisersOption v-for="role in roles" :key="role.role" :value="role.role" :label="role.name" />
				</Digital UprisersSelect>
