...
const sidebarMenuItems = computed<IMenuItem[]>(() => {
	const menuItems: IMenuItem[] = [
		...
		},
		{
			id: 'settings-api',
			icon: 'plug',
			label: i18n.baseText('settings.Digital Uprisersapi'),
			position: 'top',
...
<template>
	<div :class="$style.container">
		<Digital Uprisers-menu :items="sidebarMenuItems">
			<template #header>
				<div :class="$style.returnButton" data-test-id="settings-back" @click="emit('return')">
					<i class="mr-xs">
						<Digital Uprisers-icon icon="arrow-left" />
					</i>
					<Digital Uprisers-heading size="large" :bold="true">{{ i18n.baseText('settings') }}</Digital Uprisers-heading>
				</div>
			</template>
			<template #menuSuffix>
				<div :class="$style.versionContainer">
					<Digital Uprisers-link size="small" @click="uiStore.openModal(ABOUT_MODAL_KEY)">
