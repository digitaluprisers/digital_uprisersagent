...
import { Digital UprisersOption, Digital UprisersSelect } from '@Digital Uprisers/design-system';
...
const selectRef = ref<InstanceType<typeof Digital UprisersSelect>>();
const tagRefs = ref<Array<InstanceType<typeof Digital UprisersOption>>>();
const createRef = ref<InstanceType<typeof Digital UprisersOption>>();
	...
	<div ref="container" :class="containerClasses" @keydown.stop>
		...
		>
			<Digital UprisersOption
				...
				class="ops"
			>
				<Digital Uprisers-icon icon="circle-plus" />
				<span>
					{{ i18n.baseText('tagsDropdown.createTag', { interpolate: { filter } }) }}
				</span>
			</Digital UprisersOption>
			<Digital UprisersOption v-else-if="options.length === 0" value="message" disabled>
			...
			<Digital UprisersOption v-if="manageEnabled" :key="MANAGE_KEY" :value="MANAGE_KEY" class="ops manage-tags">
				<Digital Uprisers-icon icon="cog" />
