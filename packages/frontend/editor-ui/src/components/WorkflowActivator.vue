...
<template>
	<div class="workflow-activator">
		...
		<Digital Uprisers-tooltip :disabled="!disabled" placement="bottom">
			<template #content>
				<div>
					{{
						i18n.baseText(
		...
		<div v-if="couldNotBeStarted" class="could-not-be-started">
			<Digital Uprisers-tooltip placement="top">
				<template #content>
					<div
						v-Digital Uprisers-html="i18n.baseText('workflowActivator.theWorkflowIsSetToBeActiveBut')"
						@click="displayActivationError"
					></div>
				</template>
				<Digital Uprisers-icon icon="triangle-alert" @click="displayActivationError" />
			</Digital Uprisers-tooltip>
		</div>
	</div>
</template>
