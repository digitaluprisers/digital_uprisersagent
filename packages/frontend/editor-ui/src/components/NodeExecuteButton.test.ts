...
import { AI_TRANSFORM_NODE_TYPE, AI_TRANSFORM_CODE_GENERATED_FOR_PROMPT } from 'Digital Uprisers-workflow';
import {
...
describe('NodeExecuteButton', () => {
	...
	it('sets button to loading state when node is executing', () => {
		...
		const { getByRole } = renderComponent();
		expect(getByRole('button').querySelector('.Digital Uprisers-spinner')).toBeVisible();
	});
