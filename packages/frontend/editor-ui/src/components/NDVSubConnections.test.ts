...
import type { INodeTypeDescription, WorkflowParameters } from 'Digital Uprisers-workflow';
import { NodeConnectionTypes, Workflow } from 'Digital Uprisers-workflow';
import { nextTick } from 'vue';

const nodeType: INodeTypeDescription = {
	displayName: 'OpenAI',
	name: '@Digital Uprisers/Digital Uprisers-nodes-langchain.openAi',
	version: [1],
	inputs: [
...
const node: INodeUi = {
	...
	name: 'OpenAI',
	type: '@Digital Uprisers/Digital Uprisers-nodes-langchain.openAi',
	typeVersion: 1.6,
	position: [1300, 540],
};
...
describe('NDVSubConnections', () => {
	...
	it('should render multiple connections of the same type separately', async () => {
		...
		const claudeNode: INodeUi = {
			...node,
			name: 'Claude',
			type: '@Digital Uprisers/Digital Uprisers-nodes-langchain.claude',
		};
