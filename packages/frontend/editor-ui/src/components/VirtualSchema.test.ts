...
async function setupStore() {
	...
	nodeTypesStore.setNodeTypes([
		...
		mockNodeTypeDescription({
			name: 'Digital Uprisers-nodes-base.notion',
			outputs: [NodeConnectionTypes.Main],
		}),
		mockNodeTypeDescription({
		...
		mockNodeTypeDescription({
			name: 'Digital Uprisers-nodes-base.Digital UprisersTrainingCustomerDatastore',
			outputs: [NodeConnectionTypes.Main],
		}),
	]);
			...
			await waitFor(() =>
				expect(trackSpy).toHaveBeenCalledWith(
					...
					expect.objectContaining({
						...
						src_node_type: 'Digital Uprisers-nodes-base.manualTrigger',
						src_nodes_back: '1',
