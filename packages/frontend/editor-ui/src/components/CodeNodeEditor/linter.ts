...
import type { CodeExecutionMode, CodeNodeEditorLanguage } from 'Digital Uprisers-workflow';
...
export const useLinter = (
	...
	function lintSource(editorView: EditorView): Diagnostic[] {
		...
		if (toValue(mode) === 'runOnceForAllItems') {
			...
			const isForOfStatementOverDigital UprisersVar = (node: Node) =>
				...
				node.right.callee.object.name.startsWith('$'); // Digital Uprisers var, e.g $input

			const found = walk<TargetNode>(ast, isForOfStatementOverDigital UprisersVar);
