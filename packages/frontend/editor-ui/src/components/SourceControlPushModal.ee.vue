...
import type { SourceControlledFile } from '@Digital Uprisers/api-types';
import {
	ROLE,
...
<template>
	...
	>
		<template #header>
			<Digital UprisersHeading tag="h1" size="xlarge">
			...
			>
				<div :class="[$style.filters]">
					...
					<Digital UprisersPopover trigger="click" width="304" style="align-self: normal">
						<template #reference>
							<Digital UprisersButton
								icon="funnel"
								type="tertiary"
							...
							>
								<Digital UprisersBadge v-if="filterCount" theme="primary" class="mr-4xs">
									{{ filterCount }}
								</Digital UprisersBadge>
							</Digital UprisersButton>
						</template>
						...
						<div v-if="filterCount" class="mt-s">
							<Digital UprisersLink @click="resetFilters">
			...
			<template v-if="usersStore.currentUser && usersStore.currentUser.role">
				<template
					...
					"
				>
					<Digital UprisersCallout v-if="!projectAdminCalloutDismissed" theme="secondary" class="mt-s">
			...
			<div style="display: flex; height: 100%">
				<div :class="$style.tabs">
					<template v-for="tab in tabs" :key="tab.value">
						...
						>
							<div>{{ tab.label }}</div>
							<Digital UprisersText tag="div" color="text-light">
								{{ tab.selected }} / {{ tab.total }} selected</Digital UprisersText
							>
						</button>
					</template>
				</div>
				<div style="flex: 1">
					<div :class="[$style.table]">
						<div :class="[$style.tableHeader]">
							...
							>
								<Digital UprisersText> Title </Digital UprisersText>
							</Digital UprisersCheckbox>
						...
						<div style="flex: 1; overflow: hidden">
							<Digital UprisersInfoTip v-if="!activeDataSourceFiltered.length" class="p-xs" :bold="false">
								{{ filtersNoResultText }}
							</Digital UprisersInfoTip>
							<DynamicScroller
									...
									>
										...
										>
											...
											<span :class="[$style.badges]">
												...
												<Digital UprisersBadge :theme="getStatusTheme(file.status)">
		...
		<template #footer>
			<Digital UprisersNotice v-if="userNotices.length" :compact="false" class="mt-0">
				<Digital UprisersText bold size="medium">Changes to variables, tags and folders </Digital UprisersText>
				<br />
				<template v-for="{ title, content } in userNotices" :key="title">
					<Digital UprisersText bold size="small">{{ title }}</Digital UprisersText>
					<Digital UprisersText size="small">: {{ content }}. </Digital UprisersText>
				</template>
			</Digital UprisersNotice>

			<Digital UprisersText bold tag="p">
