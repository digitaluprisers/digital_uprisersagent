...
import { Digital UprisersLoading } from '@Digital Uprisers/design-system';
import type { EventBus } from '@Digital Uprisers/utils/event-bus';
import { createEventBus } from '@Digital Uprisers/utils/event-bus';
import type { INodeParameterResourceLocator, NodeParameterValue } from 'Digital Uprisers-workflow';
...
<template>
	...
	>
		...
		<div v-if="filterable && !errorView" :class="$style.searchInput" @keydown="onKeyDown">
			...
			>
				<template #prefix>
					<Digital Uprisers-icon :class="$style.searchIcon" icon="search" />
				</template>
			</Digital UprisersInput>
		</div>
		...
		>
			...
			>
				<div :class="$style.resourceNameContainer">
					...
					<Digital Uprisers-icon :class="$style.addResourceIcon" icon="plus" />
				</div>
			</div>
			<div
			...
			>
				<div :class="$style.resourceNameContainer">
					...
					<span v-if="result.isArchived">
						<Digital UprisersBadge class="ml-3xs" theme="tertiary" bold data-test-id="workflow-archived-tag">
			...
			<div v-if="loading && !errorView">
				<div v-for="i in 3" :key="i" :class="$style.loadingItem">
					<Digital UprisersLoading :class="$style.loader" variant="p" :rows="1" />
				</div>
			</div>
		</div>
		<template #reference>
