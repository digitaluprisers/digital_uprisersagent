...
import type { GenericValue, IDataObject, INodeExecutionData } from 'Digital Uprisers-workflow';
...
import { Digital UprisersIconButton, Digital UprisersInfoTip, Digital UprisersTooltip, Digital UprisersTree } from '@Digital Uprisers/design-system';
			...
			<tbody>
				...
				>
					...
					<td
						...
						@mouseleave="onMouseLeaveCell"
					>
						<Digital UprisersInfoTip>{{ i18n.baseText('runData.emptyItemHint') }}</Digital UprisersInfoTip>
					</td>
		...
		<table v-else ref="tableRef" :class="$style.table">
			...
			<thead>
				<tr>
					...
					>
						<Digital UprisersTooltip placement="bottom-start" :disabled="!mappingEnabled" :show-after="1000">
							<template #content>
								<div>
								...
								<template #default="{ isDragging }">
									...
									>
										...
										<div v-if="mappingEnabled" :class="$style.dragButton">
											<Digital Uprisers-icon icon="grip-vertical" />
										</div>
									</div>
								</template>
							</Draggable>
					...
					<th v-if="columnLimitExceeded" :class="$style.header">
						<Digital UprisersTooltip placement="bottom-end">
							<template #content>
								<div>
							...
							<span>
								<Digital Uprisers-icon :class="$style['warningTooltip']" icon="triangle-alert" />
			...
			>
				...
				>
					...
					>
						...
						/>
						<Digital UprisersTree v-else-if="isObject(data)" :node-class="$style.nodeClass" :value="data">
