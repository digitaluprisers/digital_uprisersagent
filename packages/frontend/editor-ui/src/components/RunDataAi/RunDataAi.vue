		...
		<template v-if="aiData.length > 0">
				...
				>
					<template #default="{ node: currentNode, data }">
						...
						>
							...
							>
								<Digital Uprisers-icon :icon="currentNode.expanded ? 'chevron-down' : 'chevron-right'" />
							</button>
							<Digital Uprisers-tooltip :disabled="!slim" placement="right">
								<template #content>
									{{ currentNode.label }}
								</template>
			...
			<div :class="$style.runData">
				<div v-if="selectedRun.length === 0" :class="$style.empty">
					<Digital Uprisers-text size="large">
						{{
