		...
		<template #content>
			<Digital Uprisers-notice v-if="!isAdvancedPermissionsEnabled">
				<i18n-t keypath="settings.users.advancedPermissions.warning">
					<template #link>
						<Digital Uprisers-link size="small" @click="goToUpgradeAdvancedPermissions">
			...
			<div v-if="showInviteUrls">
				<Digital Uprisers-users-list :users="invitedUsers">
					<template #actions="{ user }">
						<Digital Uprisers-tooltip>
							<template #content>
								{{ i18n.baseText('settings.users.inviteLink.copy') }}
							</template>
							<Digital Uprisers-icon-button
								icon="link"
								type="tertiary"
