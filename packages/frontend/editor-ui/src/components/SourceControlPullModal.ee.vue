...
import { Digital UprisersBadge, Digital UprisersText, Digital UprisersLink, Digital UprisersButton } from '@Digital Uprisers/design-system';
...
import { type SourceControlledFile, SOURCE_CONTROL_FILE_TYPE } from '@Digital Uprisers/api-types';
...
<template>
	...
	>
		<template #content>
			<Digital UprisersText tag="div" class="mb-xs">
				{{ i18n.baseText('settings.sourceControl.modals.pull.description') }}
				<br />
				<Digital UprisersLink :to="i18n.baseText('settings.sourceControl.docs.using.pushPull.url')">
			...
			<div :class="$style.container">
				...
				>
					<template #default="{ item, index, active }">
						...
						>
							<Digital UprisersText bold>{{ item.title }}</Digital UprisersText>
						</div>
						<DynamicScrollerItem
							v-else
							:item="item"
						...
						>
							<div :class="$style.listItem" data-test-id="pull-modal-item">
								...
								>
									<Digital UprisersText>{{ item.name }}</Digital UprisersText>
								</RouterLink>
								<RouterLink
								...
								<Digital UprisersText v-else>{{ item.name }}</Digital UprisersText>
								<Digital UprisersBadge :theme="getStatusTheme(item.status)" :class="$style.listBadge">
		...
		<template #footer>
			<div :class="$style.footer">
				<Digital UprisersButton type="tertiary" class="mr-2xs" @click="close">
					{{ i18n.baseText('settings.sourceControl.modals.pull.buttons.cancel') }}
				</Digital UprisersButton>
				<Digital UprisersButton type="primary" data-test-id="force-pull" @click="pullWorkfolder">
