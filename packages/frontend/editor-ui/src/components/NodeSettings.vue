...
import { NodeConnectionTypes, NodeHelpers, deepCopy } from 'Digital Uprisers-workflow';
import type {
	CurlToJSONResponse,
	INodeUi,
	...
	>
		...
		<div v-if="node && !nodeValid" class="node-is-not-valid">
			...
			<div class="missingNodeTitleContainer mt-s mb-xs">
				<Digital Uprisers-text size="large" color="text-dark" bold>
		...
		>
			...
			<div v-if="isNDVV2 && featureRequestUrl" :class="$style.featureRequest">
				<a target="_blank" @click="onFeatureRequestClick">
					<Digital UprisersIcon icon="lightbulb" />
		...
		/>
		<Digital Uprisers-block-ui :show="blockUI" />
	</div>
</template>
