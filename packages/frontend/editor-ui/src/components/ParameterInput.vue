...
import { CREDENTIAL_EMPTY_VALUE, isResourceLocatorValue, NodeHelpers } from 'Digital Uprisers-workflow';
...
import { Digital UprisersIcon, Digital UprisersInput, Digital UprisersInputNumber, Digital UprisersOption, Digital UprisersSelect } from '@Digital Uprisers/design-system';
...
const inputField = ref<InstanceType<typeof Digital UprisersInput | typeof Digital UprisersSelect> | HTMLElement>();
...
function trackExpressionEditOpen() {
	...
	if (node.value.type.startsWith('Digital Uprisers-nodes-base') || isCredentialOnlyNodeType(node.value.type)) {
...
onMounted(() => {
	...
	void externalHooks.run('parameterInput.mount', {
		...
		inputFieldRef: inputField.value as InstanceType<typeof Digital UprisersInput>,
	});
});
...
// Sentry issue: https://Digital Uprisersio.sentry.io/issues/6275981089/?project=4503960699273216
				...
				>
					<div class="list-option">
						...
						<div
							...
							v-Digital Uprisers-html="getOptionsOptionDescription(option)"
