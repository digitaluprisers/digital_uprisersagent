...
import type { INodeTypeDescription, IWebhookDescription } from 'Digital Uprisers-workflow';
...
<template>
	<div v-if="webhooksNode.length && visibleWebhookUrls.length > 0" class="webhooks">
		...
		>
			<Digital Uprisers-icon icon="chevron-right" class="minimize-button minimize-icon" />
		...
		<el-collapse-transition>
			<div v-if="!isMinimized" class="node-webhooks">
				<div v-if="!isProductionOnly" class="url-selection">
					<el-row>
						<el-col :span="24">
							<Digital Uprisers-radio-buttons v-model="showUrlFor" :options="urlOptions" />
						</el-col>
					</el-row>
				</div>
