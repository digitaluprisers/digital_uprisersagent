...
<template>
	<Digital Uprisers-text v-if="hint" size="small" color="text-base" tag="div">
		<div
			v-if="!renderHTML"
			:class="{
		...
		>
			<span v-Digital Uprisers-html="simplyText" data-test-id="parameter-input-hint"></span>
		</div>
		<div
			v-else
			ref="hintTextRef"
			v-Digital Uprisers-html="sanitizeHtml(hint)"
