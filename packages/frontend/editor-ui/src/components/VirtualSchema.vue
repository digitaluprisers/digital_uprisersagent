...
<template>
	<div :class="['run-data-schema', 'full-height', props.compact ? 'compact' : '']">
		<div v-if="noSearchResults" class="no-results">
			<Digital UprisersText tag="h3" size="large">{{ i18n.baseText('ndv.search.noNodeMatch.title') }}</Digital UprisersText>
			...
			>
				<template #default="{ item, index, active }">
					...
					>
						...
						<Digital UprisersTooltip v-else-if="item.type === 'icon'" :content="item.tooltip" placement="top">
							<Digital UprisersIcon size="small" :icon="item.icon" class="icon" />
						</Digital UprisersTooltip>

						<div
							v-else-if="item.type === 'notice'"
							v-Digital Uprisers-html="item.message"
							class="notice"
						...
						<div
							...
							:style="{ '--schema-level': item.level }"
						>
							<Digital UprisersText tag="div" size="small">
								<i18n-t
