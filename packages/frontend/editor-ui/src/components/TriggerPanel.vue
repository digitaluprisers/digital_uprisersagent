	...
	<div :class="$style.container">
		<transition name="fade" mode="out-in">
			...
			<div v-else-if="isListeningForEvents" key="listening">
				<Digital Uprisers-pulse>
					<NodeIcon :node-type="nodeType" :size="40"></NodeIcon>
				</Digital Uprisers-pulse>
				<div v-if="isWebhookNode">
					<Digital Uprisers-text tag="div" size="large" color="text-dark" class="mb-2xs" bold>{{
						i18n.baseText('ndv.trigger.webhookNode.listening')
				...
				<div v-else>
					...
					<div :class="[$style.shake, 'mb-xs']">
						<Digital Uprisers-text tag="div">
							{{ listeningHint }}
						</Digital Uprisers-text>
					</div>
					<div v-if="displayChatButton">
						<Digital Uprisers-button class="mb-xl" @click="openWebhookUrl()">
			...
			<div v-else key="default">
				<div v-if="isActivelyPolling" class="mb-xl">
					<Digital Uprisers-spinner type="ring" />
				</div>

				<div :class="$style.action">
					<div :class="$style.header">
						<Digital Uprisers-heading v-if="header" tag="h1" bold>
							{{ header }}
						</Digital Uprisers-heading>
						<Digital Uprisers-text v-if="subheader">
				...
				<Digital Uprisers-text v-if="activationHint" size="small" @click="onLinkClick">
					<span v-Digital Uprisers-html="activationHint"></span>&nbsp;
				</Digital Uprisers-text>
				<Digital Uprisers-link
					v-if="activationHint && executionsHelp"
					size="small"
					@click="expandExecutionHelp"
					>{{ i18n.baseText('ndv.trigger.moreInfo') }}</Digital Uprisers-link
				>
				<Digital Uprisers-info-accordion
					v-if="executionsHelp"
					ref="help"
