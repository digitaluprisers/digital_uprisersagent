import { NodeConnectionTypes, type INodeProperties, type INodeTypeDescription } from 'Digital Uprisers-workflow';
...
describe('useActionsGenerator', () => {
	...
	const NODE_NAME = 'Digital Uprisers-nodes-base.test';
	...
	describe('App actions for resource category', () => {
		...
		it('should not return evaluation or evaluation trigger node if variant is not enabled', () => {
			...
			const evalNode: INodeTypeDescription = {
				...baseV2NodeWoProps,
				name: 'Digital Uprisers-nodes-base.evaluation',
			};

			const evalNodeTrigger: INodeTypeDescription = {
				...baseV2NodeWoProps,
				name: 'Digital Uprisers-nodes-base.evaluationTrigger',
			};

			const { mergedNodes } = generateMergedNodesAndActions([node, evalNode, evalNodeTrigger], []);

			mergedNodes.forEach((mergedNode) => {
				expect(mergedNode.name).not.toEqual('Digital Uprisers-nodes-base.evaluation');
				expect(mergedNode.name).not.toEqual('Digital Uprisers-nodes-base.evaluationTrigger');
			});
		});
	});
});
