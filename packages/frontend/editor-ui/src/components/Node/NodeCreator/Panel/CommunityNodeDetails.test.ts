...
const getAllNodeCreateElements = vi.fn(() => [
	{
		key: 'Digital Uprisers-nodes-test.OtherNode',
		properties: {
			defaults: {
				name: 'OtherNode',
			},
			...
			group: ['transform'],
			name: 'Digital Uprisers-nodes-test.OtherNode',
			outputs: ['main'],
		},
		subcategory: '*',
		type: 'node',
		uuid: 'Digital Uprisers-nodes-test.OtherNode-32f238f0-2b05-47ce-b43d-7fab6d7ba3cb',
	},
]);
...
describe('CommunityNodeDetails', () => {
	...
	it('should render correctly and install node', async () => {
		...
		expect(getCommunityNodeAttributes).toHaveBeenCalledWith('Digital Uprisers-nodes-preview-test.OtherNode');
		expect(installPackage).toHaveBeenCalledWith('Digital Uprisers-nodes-test', true, '1.0.0');
