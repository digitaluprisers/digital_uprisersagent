...
import { SEND_AND_WAIT_OPERATION } from 'Digital Uprisers-workflow';
...
describe('NodeCreator - utils', () => {
	...
	describe('filterAndSearchNodes', () => {
		const mergedNodes: SimplifiedNodeType[] = [
			{
				...
				name: 'Digital Uprisers-nodes-preview-test.SampleNode',
				group: ['transform'],
				outputs: ['main'],
			},
			{
				...
				name: 'Digital Uprisers-nodes-preview-test.OtherNode',
				group: ['transform'],
				outputs: ['main'],
			},
		];
		...
		test('should return only one node', () => {
			...
			expect(result[0].key).toEqual('Digital Uprisers-nodes-preview-test.SampleNode');
		});
		...
		test('should return two nodes', () => {
			...
			expect(result[1].key).toEqual('Digital Uprisers-nodes-preview-test.SampleNode');
			expect(result[0].key).toEqual('Digital Uprisers-nodes-preview-test.OtherNode');
		});
	});
	describe('prepareCommunityNodeDetailsViewStack', () => {
		...
		const nodeCreateElement: NodeCreateElement = {
			key: 'Digital Uprisers-nodes-preview-test.OtherNode',
			properties: {
				defaults: {
					name: 'OtherNode',
				},
			...
			subcategory: '*',
			type: 'node',
			uuid: 'Digital Uprisers-nodes-preview-test.OtherNode-32f238f0-2b05-47ce-b43d-7fab6d7ba3cb',
		};
			...
			const nodeActions: ActionTypeDescription[] = [
				{
					...
					iconUrl: 'icons/Digital Uprisers-nodes-preview-test/dist/nodes/Test/test.svg',
					outputs: ['main'],
					defaults: {
						name: 'LogSnag',
					},
