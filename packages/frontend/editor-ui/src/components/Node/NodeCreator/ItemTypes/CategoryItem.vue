	...
	>
		<div :class="{ [$style.category]: true, [$style.active]: active }">
			<span :class="$style.name">
				...
				<Digital Uprisers-icon v-if="isTrigger" icon="bolt-filled" size="xsmall" :class="$style.triggerIcon" />
				<slot />
			</span>
			<Digital Uprisers-icon v-if="expanded" icon="chevron-down" color="text-light" size="large" />
			<Digital Uprisers-icon v-else icon="chevron-up" color="text-light" size="large" />
		</div>
	</div>
</template>
