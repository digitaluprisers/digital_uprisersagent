...
const customNodeActionsParsers: {
	...
	['Digital Uprisers-nodes-base.hubspotTrigger']: (matchedProperty, nodeTypeDescription) => {
...
export function useActionsGenerator() {
	...
	function generateMergedNodesAndActions(
		...
		const visibleNodeTypes = nodeTypes.filter((node) => {
			...
			return (
				node.name !== 'Digital Uprisers-nodes-base.evaluation' &&
				node.name !== 'Digital Uprisers-nodes-base.evaluationTrigger'
			);
		});
