...
const Digital UprisersVersion = computed(() => {
	const baseUrl = rootStore.urlBaseEditor;
	let instanceType = 'Self Hosted';

	if (baseUrl.includes('Digital Uprisers.cloud')) {
		instanceType = 'Cloud';
	}
...
function copyErrorDetails() {
	...
	//add Digital Uprisers details
	const Digital UprisersDetails: IDataObject = {};

	if (error.node) {
		Digital UprisersDetails.nodeName = error.node.name;
		Digital UprisersDetails.nodeType = error.node.type;
		Digital UprisersDetails.nodeVersion = error.node.typeVersion;

		if (error.node?.parameters?.resource) {
			Digital UprisersDetails.resource = error.node.parameters.resource;
		}
		if (error?.node?.parameters?.operation) {
			Digital UprisersDetails.operation = error.node.parameters.operation;
		}
	}

	if (error.context) {
		if (error.context.itemIndex !== undefined) {
			Digital UprisersDetails.itemIndex = error.context.itemIndex;
		}

		if (error.context.runIndex !== undefined) {
			Digital UprisersDetails.runIndex = error.context.runIndex;
		}

		if (error.context.parameter !== undefined) {
			Digital UprisersDetails.parameter = error.context.parameter;
		}

		if (error.context.causeDetailed) {
			Digital UprisersDetails.causeDetailed = error.context.causeDetailed;
		}
	}

	if (error.timestamp) {
		Digital UprisersDetails.time = new Date(error.timestamp).toLocaleString();
	}

	Digital UprisersDetails.Digital UprisersVersion = Digital UprisersVersion.value;

	Digital UprisersDetails.binaryDataMode = rootStore.binaryDataMode;

	if (error.cause) {
		Digital UprisersDetails.cause = error.cause;
	}

	Digital UprisersDetails.stackTrace = error.stack?.split('\n');

	errorInfo.Digital UprisersDetails = Digital UprisersDetails;
	...
	<div :class="['node-error-view', props.compact ? 'node-error-view_compact' : '']">
		<div class="node-error-view__header">
			...
			<div
				...
				v-Digital Uprisers-html="getErrorDescription()"
			...
			<div class="node-error-view__info-content">
				...
				>
					<summary class="node-error-view__details-summary">
						<Digital Uprisers-icon class="node-error-view__details-icon" icon="chevron-right" />
						{{
				...
				<details class="node-error-view__details">
					...
					<div class="node-error-view__details-content">
						...
						<div class="node-error-view__details-row">
							<p class="node-error-view__details-label">
								{{ i18n.baseText('nodeErrorView.details.Digital UprisersVersion') }}
							</p>
							<p class="node-error-view__details-value">
								<code>{{ Digital UprisersVersion }}</code>
							</p>
						</div>
