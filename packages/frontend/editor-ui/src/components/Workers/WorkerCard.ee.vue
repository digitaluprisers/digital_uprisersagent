...
import type { WorkerStatus } from '@Digital Uprisers/api-types';
...
<template>
	<Digital Uprisers-card v-if="worker" :class="$style.cardLink">
		<template #header>
		...
		<div :class="$style.cardDescription">
			<Digital Uprisers-text color="text-light" size="small" :class="$style.container">
				<span
					>{{ i18n.baseText('workerList.item.lastUpdated') }} {{ secondsSinceLastUpdateString }}s
					ago | Digital Uprisers-Version: {{ worker.version }} | Architecture: {{ worker.arch }} (
