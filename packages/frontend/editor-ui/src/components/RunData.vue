...
import { ViewableMimeTypes } from '@Digital Uprisers/api-types';
	...
	if (hasRunError.value && node.value) {
		...
		if (error && errorsToTrack.some((e) => error.message?.toLowerCase().includes(e))) {
			telemetry.track('User encountered an error', {
				...
				Digital UprisersVersion: rootStore.versionCli,
			});
		}
	}
});
	...
	>
		...
		>
			<div :class="$style.runSelectorInner">
				...
				<Digital UprisersTooltip v-if="canLinkRuns" placement="right">
					<template #content>
		...
		<div v-if="props.calloutMessage || $slots['callout-message']" :class="$style.hintCallout">
			<Digital UprisersCallout theme="info" data-test-id="run-data-callout">
				<slot name="callout-message">
					<Digital UprisersText v-Digital Uprisers-html="props.calloutMessage" size="small"></Digital UprisersText>
				</slot>
			</Digital UprisersCallout>
		</div>
		...
		>
			<Digital UprisersText v-Digital Uprisers-html="hint.message" size="small"></Digital UprisersText>
		</Digital UprisersCallout>
		...
		<div ref="dataContainerRef" :class="$style.dataContainer" data-test-id="ndv-data-container">
			...
			>
				<div v-if="!props.compact" :class="$style.spinner">
					<Digital UprisersSpinner type="ring" />
				</div>
				<Digital UprisersText>{{ executingMessage }}</Digital UprisersText>
			</div>
			...
			<div v-else-if="editMode.enabled" :class="$style.editMode">
				...
				<div :class="$style.editModeFooter">
					<Digital UprisersInfoTip :bold="false" :class="$style.editModeFooterInfotip">
						{{ i18n.baseText('runData.editor.copyDataInfo') }}
						<Digital UprisersLink :to="DATA_EDITING_DOCS_URL" size="small">
			...
			>
				<Digital UprisersText>
					...
					<Digital UprisersLink @click="enableNode">
			...
			<div
				...
				:class="$style.center"
			>
				<div :class="$style.spinner"><Digital UprisersSpinner type="ring" /></div>
				<Digital UprisersText color="text-dark" size="large">
					{{ i18n.baseText('runData.trimmedData.loading') }}
				</Digital UprisersText>
			</div>

			<div v-else-if="isTrimmedManualExecutionDataItem" :class="$style.center">
				<Digital UprisersText bold color="text-dark" size="large">
			...
			<div v-else-if="hasNodeRun && hasRunError" :class="$style.stretchVertically">
				<Digital UprisersText v-if="isPaneTypeInput" :class="$style.center" size="large" tag="p" bold>
					{{
			...
			>
				<div v-if="search">
					<Digital UprisersText tag="h3" size="large">{{ i18n.baseText('ndv.search.noMatch.title') }}</Digital UprisersText>
					<Digital UprisersText>
			...
			<div
				...
				:class="$style.center"
			>
				<Digital UprisersText :bold="true" color="text-dark" size="large">{{ tooMuchDataTitle }}</Digital UprisersText>
				<Digital UprisersText align="center" tag="div"
					><span
			...
			<div v-else-if="displayMode === 'binary' && binaryData.length === 0" :class="$style.center">
				<Digital UprisersText align="center" tag="div">{{ i18n.baseText('runData.noBinaryDataFound') }}</Digital UprisersText>
			</div>
						...
						>
							<div :data-test-id="'ndv-binary-data_' + index">
								...
								<div v-if="binaryData.fileExtension">
									<div>
										<Digital UprisersText size="small" :bold="true"
											>{{ i18n.baseText('runData.fileExtension') }}:</Digital UprisersText
										>
									</div>
		...
		/>
		<Digital UprisersBlockUi :show="blockUI" :class="$style.uiBlocker" />
	</div>
</template>
