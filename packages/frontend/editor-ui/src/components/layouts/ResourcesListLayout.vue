...
import type { DatatableColumn } from '@Digital Uprisers/design-system';
...
import { useDigital UprisersLocalStorage } from '@/composables/useDigital UprisersLocalStorage';
...
const Digital UprisersLocalStorage = useDigital UprisersLocalStorage();
...
const savePaginationPreferences = async () => {
	...
	Digital UprisersLocalStorage.saveProjectPreferencesToLocalStorage(
...
const loadPaginationPreferences = async () => {
	...
	const localStorageValues = Digital UprisersLocalStorage.loadProjectPreferencesFromLocalStorage(
		(route.params.projectId as string) ?? '',
		'workflows',
	);
...
<template>
	<PageViewLayout>
		...
		<div v-if="loading" class="resource-list-loading">
			<Digital Uprisers-loading :rows="25" :shrink-last="false" />
		</div>
		<template v-else>
			...
			<PageViewLayoutList v-else>
				<template #header>
					...
					>
						<Digital Uprisers-info-tip :bold="false">
							...
							<Digital Uprisers-link data-test-id="workflows-filter-reset" size="small" @click="resetFilters">
				...
				<div v-if="resourcesRefreshing" class="resource-list-loading resource-list-loading-instant">
					<Digital Uprisers-loading :rows="rowsPerPage" :shrink-last="false" />
				</div>
				<div
				...
				>
					...
					<Digital Uprisers-recycle-scroller
						v-if="type === 'list-full'"
