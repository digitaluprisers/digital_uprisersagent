...
import * as nodeTypesApi from '@Digital Uprisers/rest-api-client/api/nodeTypes';
...
import { addHeaders, addNodeTranslation } from '@Digital Uprisers/i18n';
	...
	const getNodesInformation = async (
		...
		nodesInformation.forEach((nodeInformation) => {
			if (nodeInformation.translation) {
				const nodeType = nodeInformation.name.replace('Digital Uprisers-nodes-base.', '');
