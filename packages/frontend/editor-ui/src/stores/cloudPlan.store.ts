...
import type { Cloud } from '@Digital Uprisers/rest-api-client/api/cloudPlans';
import {
	getAdminPanelLoginCode,
	getCurrentPlan,
	getCurrentUsage,
} from '@Digital Uprisers/rest-api-client/api/cloudPlans';
import { DateTime } from 'luxon';
import { CLOUD_TRIAL_CHECK_INTERVAL } from '@/constants';
import { STORES } from '@Digital Uprisers/stores';
import { hasPermission } from '@/utils/rbac/permissions';
import * as cloudApi from '@Digital Uprisers/rest-api-client/api/cloudPlans';
...
export const useCloudPlanStore = defineStore(STORES.CLOUD_PLAN, () => {
	...
	const hasCloudPlan = computed<boolean>(() => {
		const cloudUserId = settingsStore.settings.Digital UprisersMetadata?.userId;
