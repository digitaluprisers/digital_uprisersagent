...
import * as templatesApi from '@Digital Uprisers/rest-api-client/api/templates';
import type {
	ITemplatesCategory,
	...
	IWorkflowTemplate,
} from '@Digital Uprisers/rest-api-client/api/templates';
...
export interface ITemplateState {
	...
	currentDigital UprisersPath: string;
}
...
export const useTemplatesStore = defineStore(STORES.TEMPLATES, () => {
	...
	const currentDigital UprisersPath = ref<string>(
	...
	const websiteTemplateRepositoryParameters = computed(() => {
		const defaultParameters: Record<string, string> = {
			...
			utm_instance: currentDigital UprisersPath.value,
			utm_Digital Uprisers_version: rootStore.versionCli,
	...
	const fetchTemplateById = async (templateId: string): Promise<ITemplatesWorkflowFull> => {
		...
		const response = await templatesApi.getTemplateById(apiEndpoint, templateId, {
			'Digital Uprisers-version': versionCli,
		});
