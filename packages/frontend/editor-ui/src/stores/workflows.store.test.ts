...
import { deepCopy, SEND_AND_WAIT_OPERATION } from 'Digital Uprisers-workflow';
import type {
	IPinData,
	ExecutionSummary,
	IConnection,
...
import type { PushPayload, FrontendSettings } from '@Digital Uprisers/api-types';
...
describe('useWorkflowsStore', () => {
	...
	describe('updateNodeExecutionData', () => {
		...
		it('should add node error event and track errored executions', async () => {
			...
			expect(track).toHaveBeenCalledWith('Manual exec errored', {
				...
				node_type: 'Digital Uprisers-nodes-base.set',
				node_type_version: 3.4,
				node_id: '554c7ff4-7ee2-407c-8931-e34234c5056a',
				node_graph_string:
					'{"node_types":["Digital Uprisers-nodes-base.set"],"node_connections":[],"nodes":{"0":{"id":"554c7ff4-7ee2-407c-8931-e34234c5056a","type":"Digital Uprisers-nodes-base.set","version":3.4,"position":[680,180]}},"notes":{},"is_pinned":false}',
			});
		});
	...
	describe('setNodes()', () => {
		it('should transform credential-only nodes', () => {
			...
			expect(workflowsStore.workflow.nodes[1].type).toEqual('Digital Uprisers-creds-base.alienVaultApi');
