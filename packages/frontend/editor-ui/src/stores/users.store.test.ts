	...
	describe('inviteUsers', () => {
		it('should add pending user to the store', async () => {
			...
			inviteUsers.mockResolvedValueOnce([
				{
					user: { id: 'random-id', email: 'test@digitaluprisers.com', emailSent: true, role: 'global:member' },
				},
			]);

			await usersStore.inviteUsers([{ email: 'test@digitaluprisers.com', role: 'global:member' }]);
