...
export async function getDynamicInputNodeTypes(inputNodeNames: string[]) {
	...
	return globalTypeDefinition(`
type Digital UprisersInputJson = ${typeNames.map((typeName) => `${typeName}Json`).join(' | ')};
type Digital UprisersInputBinaryKeys = ${typeNames.map((typeName) => `${typeName}BinaryKeys`).join(' | ')};
type Digital UprisersInputContext = ${typeNames.map((typeName) => `${typeName}Context`).join(' | ')};
type Digital UprisersInputParams = ${typeNames.map((typeName) => `${typeName}Params`).join(' | ')};
`);
}
