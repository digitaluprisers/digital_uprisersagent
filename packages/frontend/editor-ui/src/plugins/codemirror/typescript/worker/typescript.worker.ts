...
import type { CodeExecutionMode } from 'Digital Uprisers-workflow';
...
import runOnceForAllItemsTypes from './type-declarations/Digital Uprisers-once-for-all-items.d.ts?raw';
import runOnceForEachItemTypes from './type-declarations/Digital Uprisers-once-for-each-item.d.ts?raw';
	...
	async init(options, nodeDataFetcher) {
		...
		async function loadNodeTypes(nodeName: string) {
			...
			const jsonType = data?.json
				...
				: `type ${typeName}Json = Digital UprisersJson`;
