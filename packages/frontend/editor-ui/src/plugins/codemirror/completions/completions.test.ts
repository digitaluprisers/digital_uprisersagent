...
describe('Resolution-based completions', () => {
	...
	describe('recommended completions', () => {
		...
		test('should recommend extractEmail() for: {{ "string with test@digitaluprisers.com in it".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce(
				'string with test@digitaluprisers.com in it',
			);
			const options = completions('{{ "string with test@digitaluprisers.com in it".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractEmail()', section: RECOMMENDED_SECTION }),
			);
		});

		test('should recommend extractDomain(), isEmail() for: {{ "test@digitaluprisers.com".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce('test@digitaluprisers.com');
			const options = completions('{{ "test@digitaluprisers.com".| }}');
			expect(options?.[0]).toEqual(
		...
		test('should recommend extractDomain(), extractUrlPath() for: {{ "https://digitaluprisers.com/pricing".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce('https://digitaluprisers.com/pricing');
			const options = completions('{{ "https://digitaluprisers.com/pricing".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractDomain()', section: RECOMMENDED_SECTION }),
			);
			expect(options?.[1]).toEqual(
