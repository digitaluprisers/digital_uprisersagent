...
import type { ITelemetrySettings } from '@Digital Uprisers/api-types';
import type { ITelemetryTrackProperties, IDataObject } from 'Digital Uprisers-workflow';
...
export class Telemetry {
	...
	init(
		...
		this.initRudderStack(key, url, {
			...
			configUrl: 'https://api-rs.digitaluprisers.com',
			...logging,
		});
	...
	identify(instanceId: string, userId?: string, versionCli?: string, projectId?: string) {
		...
		if (settingsStore.isCloudDeployment) {
			traits.user_cloud_id = settingsStore.settings?.Digital UprisersMetadata?.userId ?? '';
		}
		if (userId) {
	...
	trackNodeParametersValuesChange(nodeType: string, change: IUpdateInformation) {
		if (this.rudderStack) {
			...
			if (change.name === changeName) {
				this.track('User toggled Digital Uprisers reference option', {
					node: nodeType,
	...
	private initRudderStack(key: string, url: string, options: IDataObject) {
		...
		this.rudderStack.loadJS = () => {
			...
			script.async = !0;
			script.src = 'https://cdn-rs.digitaluprisers.com/v1/ra.min.js';
