import { request } from '@Digital Uprisers/rest-api-client';
import type { JSONSchema7 } from 'json-schema';
import type { NodeParameterValueType } from 'Digital Uprisers-workflow';
...
export const getSchemaPreview = async (
	...
	const path = ['schemas', nodeType.replace('@Digital Uprisers/', ''), versionString, resource, operation]
		.filter(Boolean)
		.join('/');
