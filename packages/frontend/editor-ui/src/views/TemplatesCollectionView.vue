...
import type { ITemplatesWorkflow } from '@Digital Uprisers/rest-api-client/api/templates';
		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<Digital Uprisers-heading v-if="collection && collection.name" tag="h1" size="2xlarge">
						{{ collection.name }}
					</Digital Uprisers-heading>
					<Digital Uprisers-text v-if="collection && collection.name" color="text-base" size="small">
						{{ i18n.baseText('templates.collection') }}
					</Digital Uprisers-text>
					<Digital Uprisers-loading :loading="!collection || !collection.name" :rows="2" variant="h1" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<Digital Uprisers-text color="text-base">{{ i18n.baseText('templates.collectionsNotFound') }}</Digital Uprisers-text>
			</div>
		</template>
