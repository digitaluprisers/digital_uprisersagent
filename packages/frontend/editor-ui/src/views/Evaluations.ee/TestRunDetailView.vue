	...
	<div :class="$style.container" data-test-id="test-definition-run-detail">
		<div :class="$style.header">
			<button :class="$style.backButton" @click="router.back()">
				<Digital UprisersIcon icon="arrow-left" />
				<Digital Uprisers-heading size="large" :bold="true">{{
		...
		<Digital Uprisers-callout v-if="run?.status === 'error'" theme="danger" icon="triangle-alert" class="mb-s">
			<Digital UprisersText size="small" :class="$style.capitalized">
				{{
					locale.baseText(
		...
		<el-scrollbar always :class="$style.scrollableSummary" class="mb-m">
			<div style="display: flex">
				<div :class="$style.summaryCard">
					<Digital UprisersText size="small" :class="$style.summaryCardTitle">
						{{ locale.baseText('evaluation.runDetail.totalCases') }}
					</Digital UprisersText>
					<Digital UprisersText size="xlarge" :class="$style.summaryCardContentLargeNumber" bold>{{
						testCases.length
					}}</Digital UprisersText>
				</div>
				...
				<div :class="$style.summaryCard">
					...
					<div>
						<Digital UprisersText size="medium"> {{ formattedTime.date }} {{ formattedTime.time }} </Digital UprisersText>
					</div>
				</div>
				...
				<div v-for="(value, key) in metrics" :key="key" :class="$style.summaryCard">
					<Digital UprisersTooltip :content="key" placement="top">
						<Digital UprisersText
							size="small"
		...
		<div v-if="isLoading" :class="$style.loading">
			<Digital Uprisers-loading :loading="true" :rows="5" />
		</div>
				...
				<div style="display: inline-flex; gap: 12px; align-items: center; max-width: 100%">
					...
					<template v-if="row.status === 'error'">
						<Digital UprisersTooltip placement="top" :show-after="300">
							...
							<Digital UprisersText color="danger" :class="$style.capitalized">
								{{
									locale.baseText(`${getErrorBaseKey(row.errorCode)}` as BaseTextKey) || row.status
								}}
							</Digital UprisersText>
					...
					<template v-else>
						<Digital UprisersText :class="$style.capitalized">
							{{ row.status }}
						</Digital UprisersText>
					</template>
				</div>
