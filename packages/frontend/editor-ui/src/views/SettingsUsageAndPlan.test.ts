...
describe('SettingsUsageAndPlan', () => {
	beforeEach(() => {
		...
		usageStore.viewPlansUrl = 'https://subscription.digitaluprisers.com';
		usageStore.managePlanUrl = 'https://subscription.digitaluprisers.com';
	});
	...
	it('should not show badge but unlock notice', async () => {
		...
		expect(container.querySelector('.Digital Uprisers-badge')).toBeNull();
	...
	it('should show community registered badge', async () => {
		...
		expect(getByRole('heading', { level: 3 })).toContain(container.querySelector('.Digital Uprisers-badge'));
		expect(container.querySelector('.Digital Uprisers-badge')).toHaveTextContent('Registered');
	});
});
