...
import { uid } from '@Digital Uprisers/design-system/utils';
...
<template>
	...
	>
		<template #header>
			<Digital Uprisers-heading size="2xlarge" class="mb-m">
				{{ i18n.baseText('variables.heading') }}
			</Digital Uprisers-heading>
		</template>
		<template #add-button>
			<Digital UprisersTooltip placement="top" :disabled="canCreateVariables">
				<div>
					<Digital UprisersButton
						size="medium"
						block
		...
		<template v-if="!isFeatureEnabled" #preamble>
			<Digital UprisersActionBox class="mb-m" v-bind="unavailableNoticeProps" />
		</template>
		<template v-if="!isFeatureEnabled || (isFeatureEnabled && !canCreateVariables)" #empty>
			<Digital UprisersActionBox v-if="!isFeatureEnabled" v-bind="unavailableNoticeProps" />
			<Digital UprisersActionBox
		...
		<template #default="{ data }">
			...
			<tr v-else data-test-id="variables-row">
				...
				<td>
					...
					<Digital UprisersBadge v-else theme="warning"> Value missing </Digital UprisersBadge>
				</td>
				<td>
				...
				<td v-if="isFeatureEnabled" align="right">
					<div class="action-buttons">
						<Digital UprisersTooltip :disabled="permissions.update" placement="top">
							<Digital UprisersButton
						...
						<Digital UprisersTooltip :disabled="permissions.delete" placement="top">
							<Digital UprisersButton
