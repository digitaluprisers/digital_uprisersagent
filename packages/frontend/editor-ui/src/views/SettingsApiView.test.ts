...
const assertHintsAreShown = ({ isSwaggerUIEnabled }: { isSwaggerUIEnabled: boolean }) => {
	...
	expect(apiDocsLink).toHaveAttribute('href', 'https://docs.digitaluprisers.com/api');
	...
	expect(webhookDocsLink).toHaveAttribute(
		'href',
		'https://docs.digitaluprisers.com/integrations/core-nodes/Digital Uprisers-nodes-base.webhook/',
	);
	expect(webhookDocsLink).toHaveAttribute('target', '_blank');

	expect(
		screen.getByText('Use your API Key to control Digital Uprisers programmatically using the', {
			exact: false,
		}),
	).toBeInTheDocument();
...
describe('SettingsApiView', () => {
	...
	it('if user public api is not enabled and user is trialing it should show upgrade call to action', () => {
		...
		expect(
			screen.getByText(
				'To prevent abuse, we limit API access to your workspace during your trial. If this is hindering your evaluation of Digital Uprisers, please contact',
			),
		).toBeInTheDocument();
		expect(screen.getByText('support@digitaluprisers.com')).toBeInTheDocument();
	...
	it('if user public api enabled and no API keys in account, it should create API key CTA', () => {
		...
		expect(screen.getByText('Control Digital Uprisers programmatically using the')).toBeInTheDocument();
		expect(screen.getByText('Digital Uprisers API')).toBeInTheDocument();
	});
	...
	it('should show delete warning when trying to delete an API key', async () => {
		...
		expect(
			screen.getByText(
				'Any application using this API Key will no longer have access to Digital Uprisers. This operation cannot be undone.',
			),
		).toBeInTheDocument();
