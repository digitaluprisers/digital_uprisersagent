...
import type { ITemplatesNode } from '@Digital Uprisers/rest-api-client/api/templates';
import {
	CORE_NODES_CATEGORY,
...
const COMMUNITY_PACKAGE_NAME_REGEX = /^(?!@Digital Uprisers\/)(@\w+\/)?Digital Uprisers-nodes-(?!base\b)\b\w+/g;
...
export function isCommunityPackageName(packageName: string): boolean {
	...
	// Community packages names start with <@username/>Digital Uprisers-nodes- not followed by word 'base'
