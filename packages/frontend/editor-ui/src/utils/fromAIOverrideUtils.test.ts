...
import type { INodeTypeDescription, NodePropertyTypes } from 'Digital Uprisers-workflow';
...
describe('makeOverrideValue', () => {
	...
	it('should create an fromAI override', () => {
		...
		const result = makeOverrideValue(
			makeContext(`={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('${DISPLAY_NAME}') }}`),
			mockNodeFromType(AI_NODE_TYPE),
		);
	...
	it('parses existing fromAI overrides', () => {
		...
		const result = makeOverrideValue(
			makeContext(
				`={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('${DISPLAY_NAME}', \`${description}\`) }}`,
			),
			mockNodeFromType(AI_NODE_TYPE),
		);
	...
	it('parses an existing fromAI override with default values without adding extraPropValue entry', () => {
		...
		const result = makeOverrideValue(
			makeContext("={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('aName', ``) }}"),
			mockNodeFromType(AI_NODE_TYPE),
		);
...
describe('FromAiOverride', () => {
	it('correctly identifies override values', () => {
		...
		expect(isFromAIOverrideValue('={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI() }}')).toBe(
			true,
		);
	});
	...
	it('should parseOverrides as expected', () => {
		...
		expect(parseOverrides("={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('aKey') }}")).toEqual(
			{ description: undefined },
		);
		expect(
			parseOverrides(
				"={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('aKey', `a description`) }}",
			),
		).toEqual({
			description: 'a description',
		});
	...
	it('should build a value from an override and carry over modification', () => {
		...
		expect(buildValueFromOverride(override, makeContext(''), true)).toEqual(
			`={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('${DISPLAY_NAME}', \`\`, 'string') }}`,
		);
		...
		expect(buildValueFromOverride(override, makeContext(''), true)).toEqual(
			`={{ /*Digital Uprisers-auto-generated-fromAI-override*/ $fromAI('${DISPLAY_NAME}', \`${description}\`, 'string') }}`,
		);
		expect(buildValueFromOverride(override, makeContext(''), false)).toEqual(
